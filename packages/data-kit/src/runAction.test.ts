...
describe('applyOverlay', () => {
  ...
  it('creates a new stock row only for positive deltas', () => {
    ...
    expect(next.stockLevels.find((s) => s.locationId === 'loc-new')?.quantity).toBe(5);
...
describe('runAction offline queuing (supabase mode)', () => {
  ...
  it('queues a failed floor op, applies the overlay, and resolves true', async () => {
    ...
    const overlay = transferOverlay(
      { productId: 'shirt', fromLocationId: 'loc-wh', toLocationId: 'loc-cebu', quantity: 4 },
      'actor@mwell',
    );
  ...
  it('does NOT queue a non-network (business) failure — resolves false + notifies error', async () => {
    ...
    expect(notifyError).toHaveBeenCalledWith('Insufficient stock in the selected source bin.');
  ...
  it('always refreshes the read model + counters', async () => {
    ...
    await runAction(ctx({ refresh, refreshPending }), 'transfer', async () => {}, undefined, undefined);
...
describe('replay + sync', () => {
  it('replays a queued entry against the repo and removes it on success', async () => {
    ...
    const entry: OutboxEntry = {
      id: 'oq-1',
      method: 'transfer',
      input: { productId: 'shirt', fromLocationId: 'loc-wh', toLocationId: 'loc-cebu', quantity: 3 },
      createdAt: new Date().toISOString(),
      status: 'pending',
    };
    // Seed the memory queue via enqueue-shaped push using runAction's default outbox.
    const removeEntry = vi.fn(async () => {});
    const ok = await replayEntry({ repo, actor: 'actor@mwell', removeEntry }, entry);
    expect(ok).toBe(true);
  ...
  it('marks a genuine (non-network) conflict and keeps the entry', async () => {
    ...
    const entry: OutboxEntry = {
      id: 'oq-2',
      method: 'transfer',
      input: { productId: 'shirt', fromLocationId: 'loc-wh', toLocationId: 'loc-cebu', quantity: 999 },
      createdAt: new Date().toISOString(),
      status: 'pending',
    };
    const ok = await replayEntry(
      { repo, actor: 'actor@mwell', markConflict, removeEntry, isNetworkError: () => false },
      entry,
    );
    expect(ok).toBe(false);
  ...
  it('leaves a transient network failure PENDING (no conflict, retries later)', async () => {
    ...
    const ok = await replayEntry(
      { repo, actor: 'actor@mwell', markConflict, removeEntry, isNetworkError: () => true },
      entry,
    );
    expect(ok).toBe(false);
  ...
  it('syncNow replays FIFO and stops on the first conflict', async () => {
    const repo = new InMemoryRepository(emptyData());
    const order: string[] = [];
    const removeEntry = vi.fn(async (id: string) => void order.push(`removed:${id}`));
    const markConflict = vi.fn(async (id: string) => void order.push(`conflict:${id}`));
    const pending: OutboxEntry[] = [
      {
        id: 'oq-a',
        method: 'transfer',
        input: { productId: 'shirt', fromLocationId: 'loc-wh', toLocationId: 'loc-cebu', quantity: 2 },
      ...
      },
      {
        id: 'oq-c',
        method: 'transfer',
        input: { productId: 'shirt', fromLocationId: 'loc-wh', toLocationId: 'loc-cebu', quantity: 1 },
...
describe('outbox queue lifecycle (memory fallback)', () => {
  it('receiveOverlay produces a positive stock delta + receipt row', () => {
    ...
    expect(overlay.stockDeltas?.[0]).toMatchObject({ productId: 'shirt', delta: 5 });
