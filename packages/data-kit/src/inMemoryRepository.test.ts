...
function miniData(): WarehouseData {
  return {
    ...
    allocations: [],
    events: [
      { id: "e1", name: "Event", type: "corporate", startDate: "2026-01-01" },
    ],
    returns: [],
    cycleCounts: [],
    receipts: [],
...
describe("receiveStock", () => {
  it("creates serialized units and increases availability", async () => {
    await repo.receiveStock({
      ...
      actor: "logi@mwell",
      lines: [
        { productId: "ring", quantity: 3, serialNumbers: ["A", "B", "C"] },
      ],
    });
  ...
  it("creates a lot and links unit cost when supplied", async () => {
    await repo.receiveStock({
      ...
      actor: "logi@mwell",
      lines: [
        { productId: "shirt", quantity: 10, unitCost: 225, lotCode: "LOT-X" },
      ],
    });
    const data = await repo.getData();
    ...
    const mv = data.movements.find(
      (m) => m.type === "receipt" && m.lotId === lot.id,
    );
...
describe("cancelAllocation", () => {
  it("cancels a reserved allocation without changing stock", async () => {
    ...
    const cancelled = await repo.cancelAllocation({
      allocationId: alloc.id,
      actor: "ops@mwell",
    });
...
describe("createProduct", () => {
  it("creates a new SKU and makes it queryable", async () => {
    ...
    expect(data.products.find((p) => p.id === product.id)!.reorderPoint).toBe(
      5,
    );
  });
...
describe("cancelPurchaseOrder", () => {
  it("cancels an open purchase order", async () => {
    ...
    const cancelled = await repo.cancelPurchaseOrder({
      poId: po.id,
      actor: "proc@mwell",
    });
    expect(cancelled.status).toBe("cancelled");
    const data = await repo.getData();
    expect(data.purchaseOrders.find((p) => p.id === po.id)!.status).toBe(
      "cancelled",
    );
  });
...
describe("recordCycleCount", () => {
  it("creates a draft without changing stock or logging a movement", async () => {
    ...
    const data = await repo.getData();
    expect(
      data.movements.find((m) => m.type === "cycle_count"),
    ).toBeUndefined();
    expect(count).toMatchObject({
      status: "draft",
      requestedBy: "finance@mwell",
    });
  });
});
...
describe("deleteLocation", () => {
  it("blocks deleting a location that still holds stock", async () => {
    ...
    await expect(repo.deleteLocation({ locationId: "loc-wh" })).rejects.toThrow(
      /still holds stock/i,
    );
    const data = await repo.getData();
...
describe("storage areas & bins", () => {
  ...
  it("relocates a serialized unit into a bin", async () => {
    ...
    const data = await repo.getData();
    expect(data.units.find((u) => u.serialNumber === "SN1")?.binId).toBe(
      "bin-a",
    );
  });
...
describe("integrity guards (v7)", () => {
  ...
  it("issues non-serialized stock across multiple bins without going negative", async () => {
    ...
    expect(
      availableForProduct(state, "shirt", "loc-wh"),
...
describe("W1 control parity", () => {
  it("keeps held stock on hand but removes it from available inventory", async () => {
    ...
    await controlled.inspectQuality({
      idempotencyKey: "quality-memory-01",
      sourceType: "receipt",
      sourceId: "rcpt-1",
      productId: "shirt",
      quantity: 5,
    ...
    const position = (await controlled.listInventoryPositions({})).rows.find(
      (row) =>
        row.productId === "shirt" && row.locationId === "loc-wh" && !row.binId,
    )!;
    ...
    await controlled.releaseHold({
      idempotencyKey: "release-memory-01",
      holdId: hold.id,
      targetDisposition: "accepted",
      reason: "QC retest passed",
  ...
  it("denies reservations against quantity protected by an active hold", async () => {
    ...
    await controlled.inspectQuality({
      idempotencyKey: "quality-reserved-hold",
      sourceType: "receipt",
      sourceId: "rcpt-reserved-hold",
      productId: "shirt",
      quantity: 20,
      disposition: "hold",
      reason: "Awaiting governed release",
    });
  ...
  it("does not mutate stock until final count approval and posts only once", async () => {
    ...
    const requests = await controlled.submitCycleCount({
      idempotencyKey: "count-memory-001",
      cycleCountId: "cc-1",
    ...
    const decision = {
      idempotencyKey: "decision-memory1",
      requestId: requests[0]!.id,
      decision: "approved" as const,
    };
    const supervisor = {
      actor: "supervisor@mwell",
      capabilities: ["approve_stock_adjustment"] as const,
    ...
    expect(
      data.movements.filter(
        (movement) => movement.reference === requests[0]!.id,
      ),
    ).toHaveLength(1);
  });
  ...
  it("rejects unexpected or duplicate serialized scans", async () => {
    ...
    await expect(
      controlled.submitCycleCount({
        idempotencyKey: "serialized-unexpected-001",
        cycleCountId: unexpected.id,
        reason: "Device count",
      }),
      ...
      lines: [
        {
          ...
          expected: 1,
          counted: 2,
          serialNumbers: ["SN1", "SN1"],
        },
      ],
    });
    await expect(
      controlled.submitCycleCount({
        idempotencyKey: "serialized-duplicate-001",
        cycleCountId: duplicate.id,
        reason: "Device count",
      }),
  ...
  it("marks a missing serialized unit lost only after final approval", async () => {
    ...
    const count = await controlled.recordCycleCount({
      ...
      actor: "counter@mwell",
      lines: [
        { productId: "ring", expected: 2, counted: 1, serialNumbers: ["SN1"] },
      ],
    });
    const [request] = await controlled.submitCycleCount({
      idempotencyKey: "serialized-missing-submit",
      cycleCountId: count.id,
      reason: "Device count",
    });
    expect(
      (await controlled.getData()).units.find(
        (unit) => unit.serialNumber === "SN2",
      )?.status,
    ).toBe("in_stock");
    await controlled.decideStockChange(
      {
        idempotencyKey: "serialized-missing-approve",
  ...
  it("keeps Supervisor, Finance, and requester decisions in their exact memory tiers", async () => {
    ...
    data.cycleCounts = [
      {
        ...
        status: "draft",
        actor: "counter@mwell",
        requestedBy: "counter@mwell",
        createdAt: "2026-07-10T00:00:00Z",
      },
    ];
    const controlled = new InMemoryRepository(data, {
      now: () => "2026-07-10T01:00:00Z",
      id: (prefix) => `${prefix}-tiered`,
    });
    const [request] = await controlled.submitCycleCount({
      idempotencyKey: "tiered-submit-01",
      cycleCountId: "cc-tiered",
      reason: "Material variance",
    });

    await expect(
      controlled.decideStockChange(
        {
          idempotencyKey: "tiered-finance-early",
          requestId: request!.id,
          decision: "approved",
        },
        {
          actor: "finance@mwell",
          capabilities: ["approve_stock_adjustment_finance"],
    ...
    await expect(
      controlled.decideStockChange(
        {
          idempotencyKey: "tiered-requester",
          requestId: request!.id,
          decision: "approved",
        },
        {
    ...
    const afterSupervisor = await controlled.decideStockChange(
      {
        idempotencyKey: "tiered-supervisor",
        requestId: request!.id,
        decision: "approved",
      },
      {
    ...
    await expect(
      controlled.decideStockChange(
        {
          idempotencyKey: "tiered-supervisor-late",
          requestId: request!.id,
          decision: "approved",
        },
        {
  ...
  it("queues a manual stock change and mutates only after a different authorized approval", async () => {
    ...
    await expect(
      controlled.decideStockChange(
        {
          idempotencyKey: "manual-self-approval",
          requestId: request.id,
          decision: "approved",
        },
        {
    ...
    await controlled.decideStockChange(
      {
        idempotencyKey: "manual-supervisor-approval",
        requestId: request.id,
        decision: "approved",
      },
      {
  ...
  it("rejects every serialized manual stock change at submission", async () => {
    const controlled = new InMemoryRepository(miniData(), {
      now: () => "2026-07-10T01:00:00Z",
      id: (prefix) => `${prefix}-serialized-manual`,
    });
    for (const [sourceType, quantityDelta] of [
      ...
      await expect(
        controlled.requestStockChange(
          {
            idempotencyKey: `serialized-${sourceType}`,
            sourceType,
            productId: "ring",
            locationId: "loc-wh",
            quantityDelta,
            reason: "Manual serialized correction",
            evidenceUrls: [],
          },
          {
            actor: "operator-profile-id",
            capabilities: ["manage_inventory"],
            approvalGroups: ["warehouse_operator"],
          },
        ),
  ...
  it("rejects an approved signed delta when locked stock is insufficient", async () => {
    const controlled = new InMemoryRepository(miniData(), {
      now: () => "2026-07-10T01:00:00Z",
      id: (prefix) => `${prefix}-insufficient`,
    });
    const request = await controlled.requestStockChange(
      {
        idempotencyKey: "insufficient-request",
    ...
    await expect(
      controlled.decideStockChange(
        {
          idempotencyKey: "insufficient-approve",
          requestId: request.id,
          decision: "approved",
        },
        {
    ...
    expect((await controlled.listStockChangeRequests({})).rows[0]?.status).toBe(
      "pending_supervisor",
    );
    expect(
      data.movements.filter((movement) => movement.reference === request.id),
    ).toHaveLength(0);
    expect(
      (await controlled.listExceptions({})).rows.find(
        (row) => row.sourceId === request.id,
      )?.status,
    ).toBe("open");
  });
  ...
  it("requires configured groups and distinct requester Supervisor and Finance actors", async () => {
    ...
    const controlled = new InMemoryRepository(data, {
      now: () => "2026-07-10T01:00:00Z",
      id: (prefix) => `${prefix}-distinct-tier`,
    });
    const request = await controlled.requestStockChange(
      {
        idempotencyKey: "distinct-tier-request",
        sourceType: "adjustment",
        productId: "shirt",
        locationId: "loc-wh",
        quantityDelta: -2,
        reason: "Material reconciliation",
        evidenceUrls: [],
      },
      {
        actor: "requester-profile-id",
        capabilities: ["manage_inventory"],
        approvalGroups: ["warehouse_operator"],
      },
    );
    ...
    await expect(
      controlled.decideStockChange(
        {
          idempotencyKey: "all-cap-admin-wrong-group",
          requestId: request.id,
          decision: "approved",
        },
        {
    ...
    const supervisor = await controlled.decideStockChange(
      {
        idempotencyKey: "distinct-tier-supervisor",
        requestId: request.id,
        decision: "approved",
      },
      {
        actor: "supervisor-profile-id",
        capabilities: ["approve_stock_adjustment"],
        approvalGroups: ["warehouse_supervisor"],
      },
    );
    ...
    await expect(
      controlled.decideStockChange(
        {
          idempotencyKey: "same-person-finance",
          requestId: request.id,
          decision: "approved",
        },
        {
    ...
    await controlled.decideStockChange(
      {
        idempotencyKey: "distinct-tier-finance",
        requestId: request.id,
        decision: "approved",
      },
      {
        actor: "finance-profile-id",
        capabilities: ["approve_stock_adjustment_finance"],
        approvalGroups: ["finance"],
      },
    );
  ...
  it("moves a vendor-return hold into supplier custody exactly once", async () => {
    ...
    await controlled.inspectQuality({
      idempotencyKey: "quality-vendor-001",
      sourceType: "receipt",
      sourceId: "rcpt-vendor",
      productId: "shirt",
      quantity: 3,
      disposition: "vendor_return",
      reason: "Supplier packaging failure",
      evidenceUrls: ["memory/damage.jpg"],
    });
    const hold = (await controlled.listHolds({ status: "active" })).rows[0]!;
    const input = {
      idempotencyKey: "vendor-return-001",
      holdId: hold.id,
      supplierId: "sup-1",
      reason: "Rejected at incoming QC",
    ...
    expect((await controlled.listHolds({})).rows[0]).toMatchObject({
  ...
  it("prevents disabling the last active operation route", async () => {
    ...
    await expect(
      controlled.updateOperationRoute({
