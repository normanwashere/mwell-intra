...
describe('warehouse parity vs source roles.ts', () => {
  ...
  it.each(Object.entries(EXPECTED))(
    ...
    (role, caps) => {
      const roleDef = Object.entries(warehouseModule.roles).find(([name]) => name === role)?.[1];
...
describe('capability access classifications', () => {
  it('distinguishes read access, required onboarding writes, and certified live mutations', () => {
    expect(capabilityClassificationFor('core', 'submit_documents')?.access).toBe(
      'onboarding_write',
    );
    expect(capabilityClassificationFor('core', 'manage_own_accreditation_draft')?.access).toBe(
      'onboarding_write',
    );
    expect(requiresLiveCertification('core', 'submit_documents')).toBe(false);
    expect(requiresLiveCertification('core', 'manage_own_accreditation_draft')).toBe(false);
...
describe('hasCapInModule()', () => {
  ...
  it('returns false for an unknown role', () => {
    expect(hasCapInModule('warehouse', 'ghost_role', 'view_dashboard')).toBe(false);
  });
...
describe('provisional procurement/legal/core matrices', () => {
  ...
  it('separates request collaboration, PO approval, and Finance payment duties', () => {
    expect(hasCapInModule('procurement', 'requester', 'cancel_request')).toBe(true);
    expect(
      hasCapInModule('procurement', 'procurement_officer', 'manage_request_collaborators'),
    ).toBe(true);
    expect(hasCapInModule('procurement', 'procurement_officer', 'final_approve_po')).toBe(false);
    expect(hasCapInModule('procurement', 'approver', 'final_approve_po')).toBe(true);
    expect(hasCapInModule('procurement', 'approver', 'author_po')).toBe(false);
    expect(hasCapInModule('procurement', 'finance', 'accept_payment_readiness')).toBe(true);
    expect(hasCapInModule('procurement', 'finance', 'release_payment')).toBe(true);
  });
...
describe('toRoleCapabilityRows() — DB seed shape', () => {
  ...
  it('accounts for every declared grant across all modules', () => {
    const declared = MODULE_LIST.reduce((sum, module) => {
      ...
      return (
        sum + Object.values(definition.roles).reduce((n, role) => n + role.capabilities.length, 0)
      );
    }, 0);
...
describe('events and insights workspace matrices', () => {
  it('exposes the event lifecycle roles', () => {
    ...
    expect(hasCapInModule('events', 'finance_reviewer', 'approve_settlement')).toBe(true);
    expect(hasCapInModule('events', 'finance_reviewer', 'manage_events')).toBe(false);
    expect(hasCapInModule('events', 'viewer', 'approve_settlement')).toBe(false);
    expect(hasCapInModule('events', 'admin', 'approve_settlement')).toBe(false);
  });
...
describe('product governance matrix', () => {
  it('separates contribution, final decision, and operations handoff duties', () => {
    ...
    expect(hasCapInModule('product', 'contributor', 'prepare_readiness')).toBe(true);
    expect(hasCapInModule('product', 'contributor', 'decide_go_live')).toBe(false);
    expect(hasCapInModule('product', 'product_owner', 'decide_go_live')).toBe(true);
    expect(hasCapInModule('product', 'operations_partner', 'acknowledge_operations_handoff')).toBe(
      true,
    );
    expect(hasCapInModule('product', 'operations_partner', 'approve_pricing')).toBe(false);
  });
...
describe('department DOA administration', () => {
  it('grants DOA configuration only to Legal Admin', () => {
    ...
    expect(can({ legal: ['legal_reviewer'] }, 'legal', 'manage_doa')).toBe(false);
  ...
  it('does not turn DOA configuration into procurement approval authority', () => {
    expect(can({ legal: ['admin'] }, 'procurement', 'approve_request')).toBe(false);
    expect(can({ core: ['platform_admin'] }, 'procurement', 'approve_request')).toBe(false);
  });
});
