...
export class GovernedSourcingPglite {
  ...
  async start() {
    await this.db.exec(`
      ...
      insert into core.profiles(id, vendor_id) values ('${procurementActor}', null), ('${reviewerActor}', null), ('${operationsActor}', null), ('${vendorActor}', '${vendorIds[0]}');
  ...
  async rpc(actor: 'procurement' | 'reviewer' | 'operations' | 'vendor', name: string, payload: Record<string, unknown>) {
    const actors = { procurement: [procurementActor, true, false, null], reviewer: [reviewerActor, false, true, null], operations: [operationsActor, false, false, null], vendor: [vendorActor, false, false, vendorIds[0]] } as const;
...
export const governedSourcing = { eventDeadline, procurementActor, reviewerActor, operationsActor, vendorActor };
