...
for (const route of COMMAND_ROUTES) {
  test(`command palette opens ${route.expectedPath}`, async ({ page }, testInfo) => {
    test.skip(memoryFixtureAuthDisabled, memoryFixtureSkipReason);
    await installMemorySession(page, route.fixture);
    ...
    await expect(page).toHaveURL(new RegExp(`${escapeRegex(route.expectedPath)}/?$`));
...
for (const navCase of [
  ...
  test(`${navCase.label} destination is exposed in persistent navigation`, async ({
    ...
    const nav = page.getByRole('navigation', {
      name: testInfo.project.name.includes('mobile') ? 'Primary mobile' : 'Primary',
    });
...
async function visibleBodyTextLength(page: Page): Promise<number> {
  return page.locator('body').innerText().then((text) => text.trim().length);
}

async function assertNoDocumentOverflow(page: Page, path: string): Promise<void> {
...
async function assertNoKnownDeadLinks(page: Page, path: string): Promise<void> {
  ...
  expect(result, `${path} should not expose known-dead or unsafe links`).toEqual([]);
}
...
async function assertControlsAreReachable(
  ...
  const samples = Array.from(
    new Set([
      0,
      ...Array.from(
        { length: Math.ceil(maxY / step) + 1 },
        (_, index) => Math.min(maxY, index * step),
      ),
      maxY,
    ]),
  ).sort((a, b) => a - b);
    ...
    for (const scrollY of samples) {
      ...
      const blockedTargets = visibleElements
        .map((element): BlockedTargetIssue | null => {
          ...
          if (!blocker || element === blocker || element.contains(blocker)) return null;
          if (
          ...
          return { ...describe(element), blocker: describeElement(blocker) };
        })
        .filter((item): item is BlockedTargetIssue => item !== null)
        .slice(0, 12);

      return { visibleTargets, hiddenTargets, smallTargets, blockedTargets };
      ...
      for (const item of result.visibleTargets) {
        if (!blockedKeys.has(targetKey(item))) reachableTargetKeys.add(targetKey(item));
      }
      blockedTargets.push(
  ...
  expect(
    dedupeTargets(
      blockedTargets.filter((item) => !reachableTargetKeys.has(targetKey(item))),
    ).slice(0, 12),
