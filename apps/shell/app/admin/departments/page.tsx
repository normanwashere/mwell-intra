...
function DepartmentAdministration() {
  ...
  const parentOptions = departments.filter(
    (department) =>
      department.is_active && !excludedParents.has(department.id),
  );
    ...
    <div className="space-y-6">
      ...
      {!isLive && (
        <div className="border-l-4 border-amber-500 bg-amber-50 p-4 text-sm text-amber-900">
          Demo mode shows the seeded hierarchy. Connect the live identity service
          to make changes.
        </div>
      )}
      {error && (
              ...
              <div className="flex items-center gap-1">
                ...
                {department.is_active && (
                  <Button
                    ...
                    disabled={
                      !isLive || saving || !department.can_deactivate
                    }
                    onClick={() => void deactivate(department)}
                  >
