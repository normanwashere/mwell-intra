...
describe("Knowledge Base content", () => {
  ...
  it('keeps released return linking and photo preview guidance user-facing', () => {
    ...
    expect(text).not.toMatch(/Local update, release pending/);
  });
  it('describes optional physical-return lineage without conflating intake and disposition', () => {
    ...
    expect(detail).toMatch(/unlinked/i); expect(detail).not.toMatch(/Local candidate/i);
