...
export const EXPLICIT_FEATURE_DETAILS: Record<string, ExplicitFeatureDetails> =
  {
    ...
    "warehouse-fulfillment": {
      ...
      fields: [
        ...
        field(
          "Payment date, RRN, provider method, and status",
          "Preserves the transaction date, gateway retrieval reference, provider method, and result used by Finance or Customer Support to trace the payment.",
          false,
          "Payment date uses YYYY-MM-DD. Each value belongs to the order header and must stay consistent across imported rows.",
        ),
        field(
        ...
        field(
          "Other fees and reported total",
          "Records non-shipping order fees and the old tracker total while Intra calculates subtotal, discounts, order total, net-of-VAT, VAT, and any migration variance.",
          false,
          "Fees and reported totals cannot be negative. A variance is visible for investigation; the calculated app total remains authoritative.",
        ),
        field(
          "Courier, waybill, and delivery link",
          "Carries optional dispatch preparation from order intake into packing and gives Operations, Customer Support, and the customer one tracking reference.",
          false,
          "Packing must confirm all three values before a shipment can become Ready. The delivery link must start with http:// or https://.",
        ),
        field(
          "Order instructions",
