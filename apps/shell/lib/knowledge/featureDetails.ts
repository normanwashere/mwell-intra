...
export const EXPLICIT_FEATURE_DETAILS: Record<string, ExplicitFeatureDetails> =
  {
    "shell-home": {
      controls: [
        ...
        control(
          ...
          "Notifications require a signed-in live session. The unread count covers this latest list, not all historical notifications. In the September 11 UAT release, Warehouse alerts explicitly count active stock and reservation issues, highest priority first; they remain until the source issue is resolved.",
        ...
        control(
          ...
          "The workspace gains room without changing the current record, approval step or permissions. Page and section bands distinguish the same work across internal and vendor screens. Dialog actions remain readable in light and dark mode; View original order still opens the linked original record.",
        ),
        control(
    ...
    "sign-in": {
      controls: [
        control(
          ...
          "A valid session is created and the safe return route opens. The September 11 UAT release preserves supported article, task and record query context after checking destination access. Section fragments alone are not guaranteed across sign-in; use the destination's section links when needed.",
        ),
        control(
          "Send reset link",
    ...
    "admin-users": {
      controls: [
        control(
          "Open profile",
          "Opens the selected user sheet with identity and current role grants. In the September 11 UAT release, Manage preserves the selected user with the directory's search, status, account kind and page in the address.",
      ...
      fields: [
        ...
        field(
          "Module workspace",
          "Identifies the business workspace where access applies, such as Warehouse operations, Procurement, or Events.",
          true,
          "The module must exist in the shared RBAC registry.",
        ),
        field(
          "Role responsibility",
          "Explains what the person may do inside the selected module; similarly named roles in different modules remain separate grants.",
          true,
          "The role must exist in that module and match the approved responsibility.",
        ),
      ],
    },
    "admin-audit": {
      controls: [
        control(
          ...
          "In the September 11 UAT release, enter actor, action, entity or reference text, optionally choose a module and date range, then select Search. Search scans retained authorized events, not only the newest 250.",
    ...
    "warehouse-fulfillment": {
      controls: [
        ...
        control(
          ...
          "A single authoritative fulfillment order enters the Warehouse queue with calculated commercial values and no separate tracker update.",
        ),
        control(
          "Import existing tracker",
          "Migrates legacy tracker CSV rows, recognizes both app and tracker column names, previews validation errors, and groups repeated order numbers into one multi-line order.",
          "Order reference, channel, customer name and contact, complete address, payment status, product SKU, and quantity are required. Existing references, inconsistent order headers, unknown SKUs, invalid dates, email, prices, discounts, fees, totals, delivery links, and quantities block import.",
      ...
      fields: [
        ...
        field(
          "Channel",
          "Identifies the ecommerce marketplace or selling source that accepted the order.",
          false,
          "Required by controlled ecommerce intake and must remain identical across every line for the order.",
        ),
        field(
          "Order date",
          "Records when the ecommerce channel accepted the customer order.",
          false,
          "When supplied, it must use YYYY-MM-DD and remain identical across every line for the order.",
        ),
        field(
          "Customer name",
          "Names the customer who should receive the ecommerce shipment.",
          false,
          "Required by controlled ecommerce intake and must remain identical across every line for the order.",
        ),
        field(
          "Customer contact",
          "Provides the telephone or mobile contact needed for courier coordination.",
          false,
          "Required by controlled ecommerce intake. The queue masks it while the authorized order-detail view reveals only the minimum operational information.",
        ),
        field(
          "Customer email",
          "Provides optional electronic contact associated with the source ecommerce order.",
          false,
          "When supplied, it must be a valid email address and remains masked in the general queue.",
        ),
        field(
          "Delivery address",
          "Provides the street, city, province, and postal code needed for courier handoff.",
          false,
          "All address parts are required by controlled ecommerce intake and must remain identical across every line for the order.",
        ),
        field(
          "Area of delivery",
          "Preserves the courier service area or operational delivery zone used by the ecommerce tracker.",
          false,
          "Use this for routing context; the complete street, city, province, and postal address remains authoritative for delivery.",
        ),
        field(
          "Payment status",
          "Stops unpaid ecommerce demand before stock allocation.",
          false,
          "Only paid, authorized, or COD orders can be imported into the Warehouse queue.",
        ),
        field(
          "Variant",
          "Preserves the channel-facing size, color, or sellable variation selected by the customer.",
          false,
        ...
        field(
          "Discount amount",
          "Preserves the discount reported against the imported ecommerce order line.",
          false,
          "When supplied, it cannot be negative or exceed the line value.",
        ),
        field(
          "Shipping fee",
          "Preserves the non-negative delivery charge reported for the ecommerce order.",
          false,
          "When supplied, it must be numeric and identical across every repeated line for the order.",
        ),
        field(
          "Payment reference",
          "Links the imported order to the channel or payment-provider transaction evidence.",
          false,
          "When supplied, it must remain identical across every repeated line for the order.",
        ),
        field(
          "Sales invoice number",
          "Links fulfillment to the invoice identifier supplied by ecommerce or Finance.",
          false,
          "When supplied, it must remain identical across every repeated line for the order.",
        ),
        field(
          "Payment date",
          "Records the date the ecommerce payment was processed for Finance and Customer Support tracing.",
          false,
          "Use YYYY-MM-DD. The value belongs to the order header and must stay consistent across imported rows.",
        ),
        field(
          "Payment RRN",
          "Stores the payment gateway retrieval reference used to investigate or reconcile the transaction.",
          false,
          "Enter the provider-issued reference exactly as received; do not reuse it for a different order.",
        ),
        field(
          "Payment provider method",
          "Identifies the gateway method used to collect the ecommerce payment.",
          false,
          "Use the method shown by the payment provider so Finance can reconcile like-for-like transactions.",
        ),
        field(
          "Payment provider status",
          "Records the payment result reported by the gateway at order intake.",
          false,
          "Keep the status consistent across every imported row for the same order.",
        ),
        field(
          "Campaign or event name",
          "Links an ecommerce order to the selling campaign or event named in the source tracker without changing physical event custody.",
          false,
          "Use the governed Event demand path instead when stock is physically transferred to an event location.",
        ),
        field(
          "Other fees",
          "Records non-shipping charges while Intra calculates the order subtotal, discount, tax, and final total.",
          false,
          "The value cannot be negative and must represent charges that are not already included in shipping.",
        ),
        field(
          "Reported total",
          "Preserves the total from the former tracker so Intra can display any migration variance against its calculated total.",
          false,
          "The value cannot be negative. Investigate any variance; the calculated app total remains authoritative.",
        ),
        field(
          "Courier",
          "Identifies the delivery provider selected for the shipment.",
          false,
          "Packing must confirm the courier before the shipment can become Ready.",
        ),
        field(
          "Waybill number",
          "Stores the courier reference used by Operations and Customer Support to trace the parcel.",
          false,
          "Packing must confirm a unique waybill before the shipment can become Ready.",
        ),
        field(
          "Delivery link",
          "Provides the courier tracking page shared by Operations, Customer Support, and the customer.",
          false,
          "The link must start with http:// or https:// and must match the confirmed waybill.",
        ),
        field(
          "Order instructions",
          "Preserves operational notes such as recipient or delivery instructions without a separate tracker comment cell.",
          false,
          "Do not place payment secrets or unnecessary personal profile data in free text.",
        ),
        field(
          "Event",
        ...
        field(
          "Scanned rack or bin",
          "Proves where the picker physically obtained each product line.",
          false,
          "When a recommended source bin exists, it must be scanned and must contain the requested quantity or every serialized unit.",
        ),
        field(
          "Courier",
    ...
    "warehouse-finance": {
      controls: [
        ...
        control(
          ...
          "In the September 11 UAT release, the owning PO opens at Payment handoff with current evidence and decision state. Back to Finance returns to the control center; this navigation does not approve or release payment.",
        ),
        control(
        ...
        control(
          "Open source record",
          "Navigates from a payment or activity row to its source. In the September 11 UAT release, Open source record for a warehouse receipt selected in a close draft opens Finance's read-only Receipt evidence summary, with source identity, party, date, amount and registered evidence labels.",
    ...
    "procurement-po-detail": {
      controls: [
        control(
          "Add policy evidence",
          "In the September 11 UAT release, select a named Policy requirement and Evidence format, complete the displayed business fields, and choose an existing Supporting request attachment when available. Select Add policy evidence to submit for review; raw JSON is not required.",
    ...
    "legal-cases": {
      controls: [
        control(
          ...
          "In the September 11 UAT release, selecting Accreditation cases or Vendor lifecycle updates the address while retaining the case filter.",
    ...
    "my-work": {
      controls: [
        ...
        control(
          ...
          "In the September 11 UAT release, opens the specific My Work feature guide. Start at the My Work heading and review the primary assignments queue before the follow-ups below it.",
        ...
        control(
          ...
          "The queue changes without changing any source record. In the September 11 UAT release, No work in this view means no matching assignments were returned; other departments may still be working on your requests.",
        ),
        control(
          "Open record",
    ...
    "insights-workspace": {
      controls: [
        control(
          "Insight view",
          "Filters indicators to an authorized department or executive view. In the September 11 UAT release, the desktop tabs and mobile selector update the area's address.",
        ...
        control(
          ...
          "An open follow-up is created with a stable command identity across uncertain retries. Follow its tracking link to My Work instead of raising a duplicate request. In the September 11 UAT release, a failure remains visible inside the open dialog; correct the issue and retry the same request.",
        ),
        control(
    ...
    "product-governance": {
      controls: [
        ...
        control(
          ...
          "Only an Operations Partner can acknowledge, and the current Product decision must already be approved. In the September 11 UAT release, Kit approval: Not required before handoff means active WMS kit publication is not a prerequisite. Required or unknown kit prerequisites, when shown, still need Product resolution.",
    ...
    "vendor-purchase-orders": {
      controls: [
        control(
          ...
          "In the September 11 UAT release, select Purchase orders from the vendor workspace header. Use Back to vendor portal to return to applications.",
