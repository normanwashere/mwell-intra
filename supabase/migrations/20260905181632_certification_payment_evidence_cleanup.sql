-- Forward-only extension for the exact storage-compatible receipt fixture ID.
-- Keep ordinary retained requests and the existing service-only boundary intact.
...
begin
  ...
  select coalesce(array_agg(id), '{}'::text[]) into ids from (
    ...
    where left(id, length(p_marker) + 1) = p_marker || '-'
       ...
       or id = 'req_' || p_marker || '-receipt-request'
    for update
  ) scoped;
