...
async function createTask3ReceiptFixture(marker, registerTask3Cleanup) {
  ...
  const { data: requesterProfiles, error: requesterError } = await client.schema("core").from("profiles")
    .select("id").eq("email", "intra.test.proc.requester@mwell.com.ph").limit(1);
  if (requesterError || !requesterProfiles?.[0]) throw new Error("Procurement requester profile is required.");
  const { data: reviewerProfiles, error: reviewerError } = await client.schema("core").from("profiles")
    .select("id").eq("email", "intra.test.legal.reviewer@mwell.com.ph").limit(1);
  if (reviewerError || !reviewerProfiles?.[0]) throw new Error("Assigned technical reviewer profile is required.");
  const { data: approverProfiles, error: approverError } = await client.schema("core").from("profiles")
    .select("id").eq("email", "intra.test.proc.approver@mwell.com.ph").limit(1);
  if (approverError || !approverProfiles?.[0]) throw new Error("Procurement amendment approver profile is required.");
  const ids = {
    ...
    amendmentDoaAssignments: [crypto.randomUUID(), crypto.randomUUID(), crypto.randomUUID()],
  };
  ...
  await insertAuditRows(client, "warehouse", "products", [{
    ...
    id: ids.serializedIssueProduct, sku: `${marker}-SERIAL-SKU`,
    name: `${marker} Held Serial Product`, category: "qa", serialized: true,
    unit_cost: 150, reorder_point: 0,
  }]);
  await insertAuditRows(client, "procurement", "requests", [{
    ...
    department: `${marker} Receipt Department`, category: "goods",
    sourcing_method: "rfq", core_vendor_id: ids.vendor,
  ...
  await insertAuditRows(client, "procurement", "acceptance_reviewer_assignments", [{
    request_id: ids.request, reviewer_id: reviewerProfiles[0].id, assigned_by: officerProfiles[0].id,
  }]);
  await insertAuditRows(client, "procurement", "doa_matrices", [{
    id: ids.amendmentDoaMatrix, version: `${marker}-AMENDMENT-DOA-V1`,
    department: `${marker} Receipt Department`, source_document: `${marker} controlled amendment DOA`,
    approved_by_name: "Task 3 governed fixture", approved_at: new Date().toISOString(),
    effective_at: "2026-07-14T00:00:00.000Z", active: true, status: "active",
  }]);
  await insertAuditRows(client, "procurement", "doa_assignments",
    ["dept_head", "procurement_head", "final_approver"].map((tier, index) => ({
      id: ids.amendmentDoaAssignments[index], matrix_id: ids.amendmentDoaMatrix,
      department: `${marker} Receipt Department`, category: "goods", min_amount: 0,
      max_amount: 100000, tier, approver_user_id: approverProfiles[0].id, active: true,
    })));
  await insertAuditRows(client, "warehouse", "events", [{
    id: ids.event, name: `${marker} Hold ATP event`, type: "qa",
    site_location_id: locations[0].id, start_date: "2026-07-15",
  }]);
  const routeRows = await insertAuditRows(client, "procurement", "route_decisions",
  ...
  await insertAuditRows(client, "procurement", "exception_packs", [
    { request_id: ids.policyDirect, exception_type: "direct_award", vendor_id: ids.vendor,
      route_decision_id: routeRows.find((row) => row.request_id === ids.policyDirect).id,
      route_method: "direct_award", request_version: 1,
    ...
    { request_id: ids.policyPetty, exception_type: "petty_cash_non_accredited",
      vendor_id: ids.vendor, route_decision_id: routeRows.find((row) => row.request_id === ids.policyPetty).id,
      route_method: "petty_cash", request_version: 1,
  ...
  await insertAuditRows(client, "procurement", "purchase_orders", [
    ...
    { id: ids.exceptionPo, po_number: `${marker}-PO-EXCEPTION`, request_id: ids.request,
      core_vendor_id: ids.vendor, vendor_name: `${marker} Receipt Vendor`, status: "issued", total: 100 },
    { id: ids.shortPo, po_number: `${marker}-PO-SHORT`, request_id: ids.request,
      core_vendor_id: ids.vendor, vendor_name: `${marker} Receipt Vendor`, status: "issued", total: 300 },
    { id: ids.excessPo, po_number: `${marker}-PO-EXCESS`, request_id: ids.request,
      core_vendor_id: ids.vendor, vendor_name: `${marker} Receipt Vendor`, status: "issued", total: 100 },
  ...
  await insertAuditRows(client, "procurement", "purchase_order_lines", [
    ...
    { id: ids.partialLine, purchase_order_id: ids.partialPo, line_no: 1,
      description: `${marker} partial PO first receipt`, quantity: 1, warehouse_product_id: ids.product },
    { id: ids.concurrentLine, purchase_order_id: ids.partialPo, line_no: 2,
      description: `${marker} concurrent balance`, quantity: 2, warehouse_product_id: ids.product },
    { id: ids.cancelledLine, purchase_order_id: ids.partialPo, line_no: 3,
      description: `${marker} cancelled line`, quantity: 1, warehouse_product_id: ids.product,
      receiving_status: "cancelled" },
    { id: ids.exceptionLine, purchase_order_id: ids.exceptionPo, line_no: 1,
      description: `${marker} quarantine receipt`, quantity: 1, warehouse_product_id: ids.product },
    { id: ids.shortLine, purchase_order_id: ids.shortPo, line_no: 1,
      description: `${marker} short receipt`, quantity: 3, warehouse_product_id: ids.product },
    { id: ids.excessLine, purchase_order_id: ids.excessPo, line_no: 1,
...
async function task3OperatorReceiptTransactions(page, fixture) {
  ...
  const inventoryLedger = async () => {
    const { data: stockRows, error: stockError } = await fixture.client.schema("warehouse").from("stock_levels")
      .select("quantity").eq("product_id", fixture.ids.product).eq("location_id", fixture.locationId);
    if (stockError) throw new Error(`Inventory readback failed: ${stockError.message}`);
    const { count: ledgerCount, error: ledgerError } = await fixture.client.schema("warehouse").from("movements")
      .select("id", { count: "exact", head: true }).eq("product_id", fixture.ids.product);
    if (ledgerError) throw new Error(`Movement ledger readback failed: ${ledgerError.message}`);
    return { inventory: (stockRows ?? []).reduce((total, row) => total + Number(row.quantity ?? 0), 0), ledger: ledgerCount ?? 0 };
  };
  const replayBefore = await inventoryLedger();
  const cleanReplay = await receive(fixture.ids.cleanPo, fixture.ids.cleanLine, 1, "clean");
  const replayAfter = await inventoryLedger();
  if (!cleanReplay.ok || JSON.parse(cleanReplay.body).receipt.id !== JSON.parse(clean.body).receipt.id
      || replayBefore.inventory !== replayAfter.inventory || replayBefore.ledger !== replayAfter.ledger)
    throw new Error("Idempotent receipt replay changed inventory or the movement ledger.");
  ...
  requireRpcFailure(
    await receive(fixture.ids.partialPo, fixture.ids.concurrentLine, 3, "excess"),
  ...
  const concurrent = await Promise.all([
    receive(fixture.ids.partialPo, fixture.ids.concurrentLine, 2, "concurrent-a"),
    receive(fixture.ids.partialPo, fixture.ids.concurrentLine, 2, "concurrent-b"),
  ]);
  ...
  fixture.ids.concurrentReceipt = JSON.parse(concurrent.find((result) => result.ok).body).receipt.id;
  ...
  const exceptionFacts = [
    ...
    { key: "unidentified-escalate", exceptionClass: "unidentified", outcome: "escalate", poId: fixture.ids.unidentifiedPo,
      lineId: fixture.ids.unidentifiedLine, actualQuantity: 2, expectedQuantity: 1,
  ...
  for (const scenario of exceptionFacts) {
    ...
    const exceptionReceipt = await callRpcAsBrowserUser(
      page, "warehouse", "receive_procurement_po_exception", {
        ...
        lines: [{ line_id: scenario.lineId,
          ...(scenario.exceptionClass === "unidentified" ? {} : { product_id: fixture.ids.product }),
    ...
    if (!exceptionReceipt.ok)
      throw new Error(`Operator ${scenario.exceptionClass} receipt failed: ${exceptionReceipt.body}`);
    const result = JSON.parse(exceptionReceipt.body);
    const tracked = { ...scenario, receiptId: result.receipt.id, decisionId: result.decision.id,
      exceptionId: result.exception.id, requestedBy: result.decision.requested_by };
    ...
    if (scenario.exceptionClass === "unidentified" && scenario.actualQuantity > scenario.expectedQuantity) {
      await verifyCheckpoint({ schema: "warehouse", table: "procurement_receipt_excess_custody",
        filters: { decision_id: tracked.decisionId },
        expected: { po_line_id: scenario.lineId, product_id: null,
          ordered_quantity: scenario.expectedQuantity,
          excess_quantity: scenario.actualQuantity - scenario.expectedQuantity,
          status: "pending" },
        select: "decision_id,po_line_id,product_id,ordered_quantity,excess_quantity,status" }, fixture.client);
      const { data: custodyIdentity, error: custodyIdentityError } = await fixture.client
        .schema("warehouse").from("procurement_receipt_excess_custody")
        .select("id").eq("decision_id", tracked.decisionId).single();
      if (custodyIdentityError || !custodyIdentity)
        throw new Error(`Unidentified excess custody identity failed: ${custodyIdentityError?.message ?? "missing"}`);
  ...
  const routineQualityReceipt = await callRpcAsBrowserUser(page, "warehouse", "receive_stock", {
    lots: [], units: [], stock_deltas: [], movements: [],
    receipt: {
  ...
  if (!routineQualityReceipt.ok)
    throw new Error(`Routine public quality receipt failed: ${routineQualityReceipt.body}`);

  const serializedIssueReceipt = await callRpcAsBrowserUser(page, "warehouse", "receive_stock", {
    lots: [],
    units: [{ id: fixture.ids.serializedIssueUnit,
      product_id: fixture.ids.serializedIssueProduct,
      serial_number: fixture.ids.serializedIssueSerial,
      location_id: fixture.locationId, status: "in_stock" }],
    stock_deltas: [],
    movements: [{ id: `${fixture.marker}-serialized-receipt-movement`,
      type: "receipt", product_id: fixture.ids.serializedIssueProduct, quantity: 1,
      to_location_id: fixture.locationId, reference: fixture.ids.serializedIssueReceipt,
      evidence_urls: [`audit/${fixture.marker}/serialized-receipt.jpg`],
      actor: "browser-role Warehouse Operator", created_at: new Date().toISOString() }],
    receipt: {
      id: fixture.ids.serializedIssueReceipt, supplier_id: `proc-${fixture.ids.vendor}`,
      location_id: fixture.locationId,
      lines: [{ productId: fixture.ids.serializedIssueProduct, quantity: 1,
  ...
  if (!serializedIssueReceipt.ok)
    throw new Error(`Serialized issue receipt failed: ${serializedIssueReceipt.body}`);

  const collisionPayload = (suffix) => ({
    idempotency_key: `${fixture.marker}-same-line-${suffix}`,
    po_id: fixture.ids.collisionPo, location_id: fixture.locationId,
    exception_type: "damaged", reason: `${fixture.marker} same-line receipt decision collision`,
    evidence_urls: [`audit/${fixture.marker}/same-line-${suffix}.jpg`],
    lines: [{ line_id: fixture.ids.collisionLine, product_id: fixture.ids.product,
      actual_quantity: 1, expected_quantity: 1 }],
  });
  requireRpcFailure(await callRpcAsBrowserUser(
    page, "warehouse", "receive_procurement_po_exception",
    { ...collisionPayload("expected-drift"), lines: [{ line_id: fixture.ids.collisionLine,
      product_id: fixture.ids.product, actual_quantity: 1, expected_quantity: 2 }] },
  ), /expected quantity drift/i, "expected quantity drift");
  const collisionBefore = await inventoryLedger();
  const collisionResults = await Promise.all([
    callRpcAsBrowserUser(page, "warehouse", "receive_procurement_po_exception", collisionPayload("a")),
    callRpcAsBrowserUser(page, "warehouse", "receive_procurement_po_exception", collisionPayload("b")),
  ]);
  if (collisionResults.filter((result) => result.ok).length !== 1)
    throw new Error(`same-line receipt decision collision expected one success: ${JSON.stringify(collisionResults)}`);
  const collisionAfter = await inventoryLedger();
  if (collisionBefore.inventory !== collisionAfter.inventory || collisionBefore.ledger !== collisionAfter.ledger)
    throw new Error("same-line receipt decision collision posted stock before disposition.");
  const collisionResult = JSON.parse(collisionResults.find((result) => result.ok).body);
  const collisionScenario = {
    exceptionClass: "same-line-collision", outcome: "reject",
    poId: fixture.ids.collisionPo, lineId: fixture.ids.collisionLine,
    actualQuantity: 1, expectedQuantity: 1,
    receiptId: collisionResult.receipt.id, decisionId: collisionResult.decision.id,
    exceptionId: collisionResult.exception.id, requestedBy: collisionResult.decision.requested_by,
  };
  ...
  requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "resolve_procurement_po_exception", {
    idempotency_key: `${fixture.marker}-operator-self-exception-denial`,
    decision_id: fixture.ids.quarantineDecision, decision: "quarantine",
  ...
  requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "adjust_stock", {
    stock_delta: { product_id: fixture.ids.product, location_id: fixture.locationId, delta: 1 },
    movement: { id: `${fixture.marker}-direct-adjustment-probe` },
  }), /retired|permission|not authorized/i, "retired direct stock adjustment");
  ...
  for (const [name, sourceType, quantityDelta] of [
    ...
    const requested = await callRpcAsBrowserUser(page, "warehouse", "request_stock_change", {
      idempotency_key: `${fixture.marker}-manual-${name}`,
      source_type: sourceType, product_id: fixture.ids.product,
      location_id: fixture.locationId, quantity_delta: quantityDelta,
      reason: `${fixture.marker} governed ${name} stock request`,
      evidence_urls: [`audit/${fixture.marker}/manual-${name}.jpg`],
    });
    if (!requested.ok) throw new Error(`Governed ${name} stock request failed: ${requested.body}`);
  ...
  if (stockBeforeRequests.inventory !== stockAfterRequests.inventory
      || stockBeforeRequests.ledger !== stockAfterRequests.ledger)
    throw new Error("A stock-change request mutated inventory before locked approval.");
...
async function task3RequestExcessAmendment(page, fixture) {
  const requested = await callRpcAsBrowserUser(
    page, "procurement", "request_po_line_quantity_amendment", {
  ...
  if (!requested.ok) throw new Error(`Approved amendment request failed: ${requested.body}`);
  const amendment = JSON.parse(requested.body);
  fixture.ids.excessAmendment = amendment.id;
  return { name: "Task 3 approved amendment quantity growth request", ok: true,
    amendmentId: amendment.id, previousQuantity: Number(amendment.previous_quantity),
    amendedQuantity: Number(amendment.amended_quantity) };
}

const TASK3_SIGNATURE_PNG = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=";

async function task3ApproveExcessAmendment(page, fixture) {
  const queue = await callRpcAsBrowserUser(page, "procurement", "purchase_order_amendment_work_items", {});
  if (!queue.ok || !queue.body.includes(fixture.ids.excessAmendment))
    throw new Error(`Scoped amendment approval queue is missing the assigned item: ${queue.body}`);
  let approved;
  const { error: revokeDoaError } = await fixture.client.schema("procurement")
    .from("doa_assignments").update({ active: false })
    .eq("id", fixture.ids.amendmentDoaAssignments[0]);
  if (revokeDoaError) throw new Error(`Current DOA revocation setup failed: ${revokeDoaError.message}`);
  try {
    requireRpcFailure(await callRpcAsBrowserUser(
      page, "procurement", "approve_po_line_quantity_amendment", {
        amendment_id: fixture.ids.excessAmendment, decision: "approved",
        reason: `${fixture.marker} revoked current DOA assignment denial`,
        signature: { signature_png: TASK3_SIGNATURE_PNG,
          signer_name: "Task 3 Procurement Approver", signature_method: "typed",
          signed_at: new Date().toISOString() },
      },
    ), /currently active DOA assignment and matrix/i, "revoked current DOA assignment denial");
  } finally {
    const { error: restoreDoaError } = await fixture.client.schema("procurement")
      .from("doa_assignments").update({ active: true })
      .eq("id", fixture.ids.amendmentDoaAssignments[0]);
    if (restoreDoaError) throw new Error(`Current DOA restoration failed: ${restoreDoaError.message}`);
  }
  for (let step = 1; step <= 3; step += 1) {
    ...
    if (!approved.ok) throw new Error(`Approved amendment step ${step} failed: ${approved.body}`);
  }
  await verifyCheckpoint({ schema: "procurement", table: "purchase_order_amendments",
    filters: { id: fixture.ids.excessAmendment },
    expected: { purchase_order_id: fixture.ids.excessPo, po_line_id: fixture.ids.excessLine,
      previous_quantity: 1, amended_quantity: 2, status: "approved" },
    select: "id,purchase_order_id,po_line_id,previous_quantity,amended_quantity,status,requested_by,approved_by" }, fixture.client);
  await verifyCheckpoint({ schema: "procurement", table: "purchase_order_lines",
    filters: { id: fixture.ids.excessLine }, expected: { quantity: 2 },
    select: "id,quantity,received_quantity" }, fixture.client);
  return { name: "Task 3 approved amendment quantity growth approval", ok: true,
...
async function task3SupervisorTransactions(page, fixture) {
  const inventoryQuantity = async () => {
    const { data, error } = await fixture.client.schema("warehouse").from("stock_levels")
      .select("quantity").eq("product_id", fixture.ids.product).eq("location_id", fixture.locationId);
    if (error) throw new Error(`Stock approval readback failed: ${error.message}`);
    return (data ?? []).reduce((total, row) => total + Number(row.quantity ?? 0), 0);
  };
  const serializedHoldResult = await callRpcAsBrowserUser(page, "warehouse", "inspect_quality", {
    idempotency_key: `${fixture.marker}-held-serial-qc`, source_type: "receipt",
    source_id: fixture.ids.serializedIssueReceipt,
    product_id: fixture.ids.serializedIssueProduct,
    serial_number: fixture.ids.serializedIssueSerial, quantity: 1, disposition: "hold",
  ...
  if (!serializedHoldResult.ok)
    throw new Error(`Serialized hold creation failed: ${serializedHoldResult.body}`);
  const serializedHold = JSON.parse(serializedHoldResult.body).hold;
  if (!serializedHold?.id) throw new Error("Serialized hold readback is missing");
  ...
  await insertAuditRows(fixture.client, "warehouse", "allocations", [{
    id: serializedAllocationId, event_id: fixture.ids.event,
    product_id: fixture.ids.serializedIssueProduct, quantity: 1,
    status: "reserved", promotional: false, created_at: new Date().toISOString(),
  }]);
  requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "issue", {
    allocation_id: serializedAllocationId, unit_ids: [fixture.ids.serializedIssueUnit],
    assigned_to: "Task 3 denied assignee", event_id: fixture.ids.event,
    stock_deltas: [], movement: {
      id: `${fixture.marker}-held-serial-movement`, type: "issue",
      product_id: fixture.ids.serializedIssueProduct, quantity: 1,
      from_location_id: fixture.locationId, event_id: fixture.ids.event,
      reference: serializedAllocationId, evidence_urls: [], actor: "caller-supplied",
      created_at: new Date().toISOString(),
    },
  }), /held serialized units cannot be issued/i, "held serialized unit issue denial");
  await verifyCheckpoint({ schema: "warehouse", table: "inventory_units",
    filters: { id: fixture.ids.serializedIssueUnit }, expected: { status: "in_stock" },
    select: "id,status,serial_number" }, fixture.client);
  await verifyCheckpoint({ schema: "warehouse", table: "allocations",
    filters: { id: serializedAllocationId }, expected: { status: "reserved" },
    select: "id,status,product_id" }, fixture.client);
  const { error: serializedAllocationCleanupError } = await fixture.client.schema("warehouse")
    .from("allocations").delete().eq("id", serializedAllocationId);
  if (serializedAllocationCleanupError)
    throw new Error(`Held serial allocation cleanup failed: ${serializedAllocationCleanupError.message}`);

  requireRpcFailure(await callRpcAsBrowserUser(page, "private", "warehouse_inspect_quality", {
    idempotency_key: `${fixture.marker}-private-quality-denial`, source_type: "receipt",
    source_id: fixture.ids.quarantineReceipt, product_id: fixture.ids.product,
    procurement_po_line_id: fixture.ids.quarantineLine, quantity: 1, disposition: "hold",
    reason: "private quality inspection direct denial", evidence_urls: [`audit/${fixture.marker}/private-denial.jpg`],
  }), /permission|denied|schema|not found|could not find/i, "private quality inspection direct denial");
  requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "inspect_quality", {
    idempotency_key: `${fixture.marker}-wrong-receipt-line`, source_type: "receipt",
    source_id: fixture.ids.quarantineReceipt, product_id: fixture.ids.product,
  ...
  requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "inspect_quality", {
    ...
    source_type: "receipt", source_id: fixture.ids.qualityProbeReceipt,
    product_id: fixture.ids.product, procurement_po_line_id: fixture.ids.qualityProbeLine,
    quantity: 1, disposition: "accepted",
  ...
  const atpBeforeRaceResult = await callRpcArgsAsBrowserUser(page, "warehouse", "available_to_promise", {
    p_product_id: fixture.ids.product,
  });
  if (!atpBeforeRaceResult.ok) throw new Error(`Hold-create race ATP pre-read failed: ${atpBeforeRaceResult.body}`);
  ...
  const raceResults = await Promise.allSettled([
    callRpcAsBrowserUser(page, "warehouse", "inspect_quality", {
      idempotency_key: `${fixture.marker}-hold-create-race`, source_type: "receipt",
      source_id: fixture.ids.cleanReceipt, product_id: fixture.ids.product,
      procurement_po_line_id: fixture.ids.cleanLine, quantity: 1, disposition: "hold",
    ...
    callRpcAsBrowserUser(page, "warehouse", "reserve", {
      product_id: fixture.ids.product, quantity: atpBeforeRace,
      allocation: { id: raceAllocationId, event_id: fixture.ids.event,
        product_id: fixture.ids.product, quantity: atpBeforeRace, status: "reserved",
        promotional: false, created_at: new Date().toISOString() },
    }),
  ]);
  const [holdRace, reserveRace] = raceResults.map((result) => result.status === "fulfilled"
    ? result.value : ({ ok: false, status: 500, body: String(result.reason) }));
  if (Number(holdRace.ok) + Number(reserveRace.ok) !== 1)
    throw new Error(`exactly one concurrent hold or reservation may consume availability: ${JSON.stringify({ holdRace, reserveRace })}`);
  const { data: raceHolds, error: raceHoldError } = await fixture.client.schema("warehouse")
    .from("inventory_holds").select("id,status,quantity,inspection_id")
    .eq("reason", `${fixture.marker} hold creation versus reservation`);
  if (raceHoldError) throw new Error(`Hold-create race hold readback failed: ${raceHoldError.message}`);
  const { data: raceAllocations, error: raceAllocationError } = await fixture.client.schema("warehouse")
    .from("allocations").select("id,status,quantity,product_id").eq("id", raceAllocationId);
  if (raceAllocationError) throw new Error(`Hold-create race allocation readback failed: ${raceAllocationError.message}`);
  if ((raceHolds?.length ?? 0) + (raceAllocations?.length ?? 0) !== 1)
    throw new Error(`Hold-create race authoritative rows diverged: ${JSON.stringify({ raceHolds, raceAllocations })}`);
  await verifyCheckpoint({ schema: "warehouse", table: "receipts",
    filters: { id: fixture.ids.cleanReceipt }, expected: { procurement_po_id: fixture.ids.cleanPo },
    select: "id,quality_status,procurement_po_id" }, fixture.client);
  await verifyCheckpoint({ schema: "procurement", table: "purchase_order_lines",
    filters: { id: fixture.ids.cleanLine }, expected: { received_quantity: 1 },
    select: "id,quantity,received_quantity,receiving_status" }, fixture.client);
  const atpAfterRace = await callRpcArgsAsBrowserUser(page, "warehouse", "available_to_promise", {
    p_product_id: fixture.ids.product,
  });
  const expectedAtpAfterRace = holdRace.ok ? atpBeforeRace - 1 : 0;
  if (!atpAfterRace.ok || Number(JSON.parse(atpAfterRace.body)) !== expectedAtpAfterRace)
    throw new Error(`Hold-create race authoritative ATP readback failed: ${atpAfterRace.body}`);
  if (reserveRace.ok) {
    const cancelled = await callRpcAsBrowserUser(page, "warehouse", "cancel_allocation", {
      allocation_id: raceAllocationId,
    });
    if (!cancelled.ok) throw new Error(`Hold-create race allocation cleanup failed: ${cancelled.body}`);
    const acceptedQc = await callRpcAsBrowserUser(page, "warehouse", "inspect_quality", {
      idempotency_key: `${fixture.marker}-post-race-clean-qc`, source_type: "receipt",
      source_id: fixture.ids.cleanReceipt, product_id: fixture.ids.product,
      procurement_po_line_id: fixture.ids.cleanLine, quantity: 1, disposition: "accepted",
      evidence_urls: [`audit/${fixture.marker}/post-race-clean-qc.jpg`],
    });
    if (!acceptedQc.ok) throw new Error(`Post-race clean QC failed: ${acceptedQc.body}`);
  } else {
    const released = await callRpcAsBrowserUser(page, "warehouse", "release_quality_hold", {
      idempotency_key: `${fixture.marker}-hold-create-race-release`, hold_id: raceHolds[0].id,
      target_disposition: "accepted", reason: `${fixture.marker} race hold released after readback`,
      evidence_urls: [`audit/${fixture.marker}/hold-create-race-release.jpg`],
    });
    if (!released.ok) throw new Error(`Hold-create race hold cleanup failed: ${released.body}`);
  }
  const validPublicQuality = await callRpcAsBrowserUser(page, "warehouse", "inspect_quality", {
    ...
    source_id: fixture.ids.routineQualityReceipt, product_id: fixture.ids.product,
    quantity: 1, disposition: "accepted",
  ...
  await verifyCheckpoint({ schema: "warehouse", table: "quality_inspections",
    filters: { source_id: fixture.ids.routineQualityReceipt, disposition: "accepted" },
  ...
  for (const scenario of fixture.receiptExceptionScenarios) {
    ...
    if (!resolution.ok)
      throw new Error(`Supervisor ${scenario.outcome} decision failed: ${resolution.body}`);
    const replay = await callRpcAsBrowserUser(
      page, "warehouse", "resolve_procurement_po_exception", decisionPayload,
    );
    if (!replay.ok || JSON.parse(replay.body).decision.id !== scenario.decisionId)
      throw new Error(`Supervisor ${scenario.outcome} decision was not idempotent.`);
    await verifyCheckpoint({ schema: "warehouse", table: "procurement_receipt_exception_decisions",
      filters: { id: scenario.decisionId },
      expected: { status: scenario.outcome === "escalate" ? "escalated" : "decided",
        decision: scenario.outcome }, select: "id,status,decision,requested_by,decided_by" }, fixture.client);
    const expectedDisposition = { accept: "accepted", reject: "vendor_return",
      quarantine: "hold", escalate: "pending" }[scenario.outcome];
    if (scenario.exceptionClass === "unidentified") {
      ...
      await verifyCheckpoint({ schema: "warehouse", table: "quality_inspections",
        ...
        select: "id,source_id,disposition,procurement_po_line_id" }, fixture.client);
    }
    ...
    if (scenario.outcome === "escalate") {
      const actionable = await callRpcAsBrowserUser(
        page, "warehouse", "procurement_receipt_exception_work_items", { status: "escalated" },
      );
      ...
      if (scenario.excessCustodyId) {
        await verifyCheckpoint({ schema: "warehouse", table: "procurement_receipt_excess_custody",
          ...
          select: "id,product_id,ordered_quantity,excess_quantity,status" }, fixture.client);
        await verifyCheckpoint({ schema: "warehouse", table: "procurement_receipt_exception_lines",
          filters: { decision_id: scenario.decisionId }, expected: { active: false },
    ...
    if (scenario.outcome === "quarantine") {
      ...
      if (receiptFactError || Number(receiptFact?.lines?.[0]?.actual_quantity) !== scenario.actualQuantity)
        throw new Error("Quarantine did not preserve the full observed receipt fact.");
      const { data: qualityRows, error: qualityError } = await fixture.client.schema("warehouse")
        .from("quality_inspections").select("id,quantity").eq("source_id", scenario.receiptId).limit(1);
      if (qualityError || !qualityRows?.[0]) throw new Error("Quarantine QC readback failed.");
      const postableQuantity = Math.min(scenario.actualQuantity, scenario.expectedQuantity);
      if (Number(qualityRows[0].quantity) !== postableQuantity)
        throw new Error(`bounded quarantine posting expected ${postableQuantity}, got ${qualityRows[0].quantity}.`);
      const { data: holdRows, error: holdError } = await fixture.client.schema("warehouse")
        .from("inventory_holds").select("id,status,quantity,location_id,bin_id,lot_id").eq("inspection_id", qualityRows[0].id)
        .eq("status", "active").limit(1);
      if (holdError || !holdRows?.[0]) throw new Error("Quarantine did not create an active inventory hold.");
      ...
      await insertAuditRows(fixture.client, "warehouse", "allocations", [{
        id: heldLotAllocationId, event_id: fixture.ids.event,
        product_id: fixture.ids.product, quantity: 1, status: "reserved",
        promotional: false, created_at: new Date().toISOString(),
      }]);
      requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "issue", {
        allocation_id: heldLotAllocationId, unit_ids: [], event_id: fixture.ids.event,
        stock_deltas: [{ product_id: fixture.ids.product,
          location_id: holdRows[0].location_id, bin_id: holdRows[0].bin_id,
          lot_id: holdRows[0].lot_id, delta: -1 }],
        movement: { id: `${fixture.marker}-held-lot-movement-${scenario.decisionId}`,
          type: "issue", product_id: fixture.ids.product, quantity: 1,
          from_location_id: holdRows[0].location_id, from_bin_id: holdRows[0].bin_id,
          lot_id: holdRows[0].lot_id, event_id: fixture.ids.event,
          reference: heldLotAllocationId, evidence_urls: [], actor: "caller-supplied",
          created_at: new Date().toISOString() },
      }), /held exact lot stock cannot be issued/i, "held exact lot issue denial");
      await verifyCheckpoint({ schema: "warehouse", table: "allocations",
        filters: { id: heldLotAllocationId }, expected: { status: "reserved" },
        select: "id,status,product_id" }, fixture.client);
      const { error: heldLotAllocationCleanupError } = await fixture.client.schema("warehouse")
        .from("allocations").delete().eq("id", heldLotAllocationId);
      if (heldLotAllocationCleanupError)
        throw new Error(`Held lot allocation cleanup failed: ${heldLotAllocationCleanupError.message}`);
      if (scenario.actualQuantity > scenario.expectedQuantity) {
        await verifyCheckpoint({ schema: "warehouse",
          ...
          filters: { decision_id: scenario.decisionId },
          expected: { product_id: fixture.ids.product, ordered_quantity: 1,
            excess_quantity: 1, status: "held" },
          select: "id,decision_id,product_id,ordered_quantity,excess_quantity,status" }, fixture.client);
        ...
        if (scenario.exceptionClass === "excess") fixture.ids.excessCustody = custodyRows[0].id;
      ...
      requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "reserve", {
        ...
        allocation: { id: `${fixture.marker}-held-allocation-denial`, event_id: fixture.ids.event,
          product_id: fixture.ids.product, quantity: heldStock, status: "reserved",
          promotional: false, created_at: new Date().toISOString() },
      ...
      if (scenario.key === "unidentified-quarantine") {
        ...
        const holdRow = page.getByText(
          `${fixture.marker} exception outcome quarantine ${scenario.key}`, { exact: true },
        ).locator("xpath=ancestor::li[1]");
        await holdRow.getByRole("button", { name: "Review hold" }).click();
        await page.getByRole("tab", { name: "Reject to vendor" }).click();
        await page.getByLabel("Supplier").selectOption(`proc-${fixture.ids.vendor}`);
        await page.getByLabel("Vendor return reference").fill(`${fixture.marker}-atomic-hold-return`);
        await page.getByLabel("Vendor return reason").fill(`${fixture.marker} atomic hold rejection vendor return`);
        await page.getByLabel("Vendor return evidence URL").fill(`audit/${fixture.marker}/atomic-hold-return.jpg`);
        await page.getByRole("button", { name: "Reject and create vendor return" }).click();
        await verifyCheckpoint({ schema: "warehouse", table: "inventory_holds",
          filters: { id: holdRows[0].id }, expected: { status: "vendor_return" },
          select: "id,status,released_by,release_reason" }, fixture.client);
        await verifyCheckpoint({ schema: "warehouse", table: "quality_inspections",
          filters: { id: qualityRows[0].id }, expected: { disposition: "vendor_return" },
          select: "id,disposition,procurement_po_line_id" }, fixture.client);
        await verifyCheckpoint({ schema: "warehouse", table: "vendor_returns",
          filters: { hold_id: holdRows[0].id },
          expected: { supplier_id: `proc-${fixture.ids.vendor}`,
            reference: `${fixture.marker}-atomic-hold-return`, status: "ready" },
          select: "id,hold_id,supplier_id,reference,status,quantity" }, fixture.client);
        await verifyCheckpoint({ schema: "warehouse", table: "procurement_receipt_exception_lines",
          filters: { decision_id: scenario.decisionId }, expected: { active: false },
          select: "decision_id,po_line_id,active" }, fixture.client);
        await verifyCheckpoint({ schema: "procurement", table: "purchase_orders",
          filters: { id: scenario.poId }, expected: { status: "issued" }, select: "id,status" }, fixture.client);
        if (await inventoryQuantity() !== stockBeforeReturn - postableQuantity)
          throw new Error("Atomic hold rejection did not move the held quantity out of Warehouse custody.");
        continue;
      }
      ...
      const [release, firstReservation] = await Promise.all([
        ...
        callRpcAsBrowserUser(page, "warehouse", "reserve", {
          product_id: fixture.ids.product, quantity: heldStock,
          allocation: { id: firstAllocationId, event_id: fixture.ids.event,
            product_id: fixture.ids.product, quantity: heldStock, status: "reserved",
            promotional: false, created_at: new Date().toISOString() },
        }),
      ]);
      ...
      if (!firstReservation.ok) {
        requireRpcFailure(firstReservation,
          /available after active inventory holds|only .* available/i,
          "hold reservation lock ordering");
        allocationId = retryAllocationId;
        finalReservation = await callRpcAsBrowserUser(page, "warehouse", "reserve", {
          product_id: fixture.ids.product, quantity: heldStock,
          allocation: { id: allocationId, event_id: fixture.ids.event,
            product_id: fixture.ids.product, quantity: heldStock, status: "reserved",
            promotional: false, created_at: new Date().toISOString() },
        });
      }
      if (!finalReservation.ok)
        throw new Error(`Authoritative post-release reservation failed: ${finalReservation.body}`);
      await verifyCheckpoint({ schema: "warehouse", table: "inventory_holds",
        filters: { id: holdRows[0].id }, expected: { status: "released" },
        select: "id,status,released_by" }, fixture.client);
      await verifyCheckpoint({ schema: "warehouse", table: "allocations",
        filters: { id: allocationId }, expected: { product_id: fixture.ids.product,
          quantity: heldStock, status: "reserved" },
        select: "id,product_id,quantity,status,event_id" }, fixture.client);
      const atp = await callRpcArgsAsBrowserUser(page, "warehouse", "available_to_promise", {
        p_product_id: fixture.ids.product,
      });
      if (!atp.ok || Number(JSON.parse(atp.body)) !== 0)
        throw new Error(`authoritative hold race readback expected zero ATP: ${atp.body}`);
      await verifyCheckpoint({ schema: "warehouse", table: "receipts",
        filters: { id: scenario.receiptId }, expected: { quality_status: "accepted" },
        select: "id,quality_status,procurement_po_id" }, fixture.client);
      ...
      if (scenario.exceptionClass === "excess") {
        ...
        requireRpcFailure(await callRpcAsBrowserUser(
          page, "warehouse", "resolve_procurement_receipt_excess", {
            ...
            custody_id: fixture.ids.excessCustody, outcome: "accepted_amendment",
        ...
        const workItems = await callRpcAsBrowserUser(
          page, "warehouse", "procurement_receipt_excess_work_items", {},
        );
        if (!workItems.ok || !workItems.body.includes(fixture.ids.excessCustody))
          throw new Error(`authenticated excess custody work items missing: ${workItems.body}`);
  ...
  for (const [name, expectedDelta] of [["positive", 2], ["negative", -1]]) {
    const before = await inventoryQuantity();
    const decided = await callRpcAsBrowserUser(page, "warehouse", "decide_stock_change", {
      idempotency_key: `${fixture.marker}-manual-${name}-approval`,
      request_id: fixture.manualStockRequests[name], decision: "approved",
      note: `${fixture.marker} trusted Supervisor ${name} approval`,
    });
    if (!decided.ok) throw new Error(`Governed ${name} stock approval failed: ${decided.body}`);
    const after = await inventoryQuantity();
    if (after - before !== expectedDelta)
      throw new Error(`Governed ${name} stock approval applied ${after - before}, expected ${expectedDelta}.`);
  }
  requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "decide_stock_change", {
    ...
    request_id: fixture.manualStockRequests.insufficient, decision: "approved",
    note: `${fixture.marker} insufficient stock denial`,
  }), /negative|insufficient/i, "insufficient locked stock-change approval");
...
async function task3SupervisorExcessFinalDisposition(page, fixture) {
  ...
  if (!workItems.ok || !workItems.body.includes(fixture.ids.excessCustody))
    throw new Error(`Authenticated excess custody work item disappeared before disposition: ${workItems.body}`);
  await page.goto(`${baseUrl}/warehouse/purchase-orders?workflow=${Date.now()}`, {
    waitUntil: "domcontentloaded", timeout: 20_000,
  });
  await waitForMeaningfulRoute(page);
  const custodyRow = page.getByText(`${fixture.marker}-PO-EXCESS`, { exact: true })
    .locator("xpath=ancestor::li[1]");
  await custodyRow.getByRole("button", { name: "Review excess custody" }).click();
  await page.getByLabel("Governed outcome").selectOption("accepted_amendment");
  await page.getByLabel("Approved quantity amendment").selectOption(fixture.ids.excessAmendment);
  await page.getByLabel("Decision reason").fill(`${fixture.marker} Supervisor excess custody final disposition`);
  await page.getByLabel("Evidence URL").fill(`audit/${fixture.marker}/accepted-excess-amendment.jpg`);
  await page.getByRole("button", { name: "Record final disposition" }).click();
  await verifyCheckpoint({ schema: "warehouse", table: "procurement_receipt_excess_custody",
    filters: { id: fixture.ids.excessCustody },
    expected: { status: "accepted_amendment", approved_amendment_id: fixture.ids.excessAmendment },
    select: "id,status,approved_amendment_id,ordered_quantity,excess_quantity" }, fixture.client);
  await verifyCheckpoint({ schema: "procurement", table: "purchase_order_lines",
    filters: { id: fixture.ids.excessLine }, expected: { quantity: 2, received_quantity: 2 },
    select: "id,quantity,received_quantity" }, fixture.client);
  await verifyCheckpoint({ schema: "warehouse", table: "procurement_receipt_exception_lines",
    filters: { decision_id: fixture.ids.excessDecision }, expected: { active: false },
    select: "decision_id,po_line_id,active" }, fixture.client);
  await verifyCheckpoint({ schema: "procurement", table: "purchase_orders",
    filters: { id: fixture.ids.excessPo }, expected: { status: "closed" },
    select: "id,status" }, fixture.client);
  const after = await callRpcAsBrowserUser(
    page, "warehouse", "procurement_receipt_excess_work_items", {},
  );
  if (!after.ok || after.body.includes(fixture.ids.excessCustody))
    throw new Error(`Supervisor excess custody final disposition remained actionable: ${after.body}`);
  return { name: "Task 3 Supervisor excess custody final disposition", ok: true,
    custodyId: fixture.ids.excessCustody, amendmentId: fixture.ids.excessAmendment };
}
...
async function task3AllCapabilityAdminWrongStep(page, fixture) {
  requireRpcFailure(await callRpcAsBrowserUser(page, "warehouse", "decide_stock_change", {
    ...
    decision: "approved", note: "all-capability admin wrong-step denial",
...
async function task3GoodsAcceptance(page, fixture, purchaseOrderId, expectedLineId, workflowName) {
  await page.goto(`${baseUrl}/procurement/purchase-orders/${encodeURIComponent(purchaseOrderId)}?workflow=${Date.now()}`, {
    waitUntil: "domcontentloaded", timeout: 20_000,
  });
  await waitForMeaningfulRoute(page);
  ...
  const projection = await callRpcAsBrowserUser(page, "procurement", "acceptance_work_items", {
    purchase_order_id: purchaseOrderId,
  });
  if (!projection.ok) throw new Error(`${workflowName} work item failed: ${projection.body}`);
  const [item] = JSON.parse(projection.body);
  const line = item?.lines?.find((candidate) => candidate.poLineId === expectedLineId);
  if (!line || Number(line.qcAcceptedQuantity) <= 0)
    throw new Error(`${workflowName} did not expose QC-accepted line quantities.`);
  if (/vendor_name|unit_price|core_vendor_id|estimated_amount|"total"/i.test(projection.body))
    throw new Error(`${workflowName} leaked commercial facts.`);
  const accepted = await callRpcAsBrowserUser(page, "procurement", "record_acceptance_pack", {
    purchase_order_id: purchaseOrderId, acceptance_type: "goods",
    accepted_scope: { summary: `${fixture.marker} browser-role acceptance`,
      lines: [{ poLineId: expectedLineId, quantity: Number(line.qcAcceptedQuantity),
        warehouseReceiptId: line.warehouseReceiptId,
        qcInspectionIds: line.qcInspectionIds }] },
    exceptions: [], warehouse_receipt_reference: item.warehouse_receipt_reference,
  });
  if (!accepted.ok) throw new Error(`${workflowName} mutation failed: ${accepted.body}`);
  const pack = JSON.parse(accepted.body);
  fixture.cleanupActivityEntityIds.push(pack.id);
  await verifyCheckpoint({ schema: "procurement", table: "acceptance_packs",
    filters: { id: pack.id }, expected: { purchase_order_id: purchaseOrderId },
    select: "id,purchase_order_id,request_id,accepted_by,status" }, fixture.client);
  return { name: workflowName, ok: true, purchaseOrderId, acceptancePackId: pack.id };
}

async function task3CumulativePartialAcceptance(page, fixture) {
  const { data: exactQuality, error: exactQualityError } = await fixture.client.schema("warehouse")
    .from("quality_inspections")
    .select("source_id,procurement_po_line_id,quantity,disposition")
    .in("source_id", [fixture.ids.partialReceipt, fixture.ids.concurrentReceipt])
    .eq("disposition", "accepted");
  const exactByLine = new Map((exactQuality ?? []).map((row) => [
    row.procurement_po_line_id, { receiptId: row.source_id, quantity: Number(row.quantity) },
  ]));
  if (exactQualityError
      || exactByLine.get(fixture.ids.partialLine)?.receiptId !== fixture.ids.partialReceipt
      || exactByLine.get(fixture.ids.partialLine)?.quantity !== 1
      || exactByLine.get(fixture.ids.concurrentLine)?.receiptId !== fixture.ids.concurrentReceipt
      || exactByLine.get(fixture.ids.concurrentLine)?.quantity !== 2)
    throw new Error(`same-product PO-line quality isolation failed: ${exactQualityError?.message ?? JSON.stringify(exactQuality)}`);
  const latest = await task3GoodsAcceptance(
    page, fixture, fixture.ids.partialPo, fixture.ids.concurrentLine,
    "Task 3 assigned-reviewer goods acceptance latest partial",
  );
  const earlier = await task3GoodsAcceptance(
    page, fixture, fixture.ids.partialPo, fixture.ids.partialLine,
    "Task 3 assigned-reviewer goods acceptance earlier partial",
  );
  const { data: packs, error } = await fixture.client.schema("procurement")
    .from("acceptance_packs").select("id,warehouse_receipt_reference,status")
  ...
  if (new Set(fixture.cumulativeAcceptancePackIds).size !== 2)
    throw new Error("distinct active acceptance packs were not preserved per receipt evidence pack.");
...
async function task3InvalidateReadinessWithAcceptanceChange(page, fixture) {
  const replacement = await task3GoodsAcceptance(
    page, fixture, fixture.ids.partialPo, fixture.ids.partialLine,
    "Task 3 replacement acceptance evidence",
  );
  fixture.ids.replacementAcceptancePack = replacement.acceptancePackId;
  const { data: po, error: poError } = await fixture.client.schema("procurement")
    .from("purchase_orders").select("acceptance_evidence_version")
    .eq("id", fixture.ids.partialPo).single();
  if (poError || Number(po?.acceptance_evidence_version) <= 0)
    throw new Error(`Acceptance evidence version did not advance: ${poError?.message ?? JSON.stringify(po)}`);
  await verifyCheckpoint({ schema: "procurement", table: "payment_readiness_packs",
    filters: { id: fixture.ids.paymentReadinessPack }, expected: { status: "accepted", evidence_stale: true },
    select: "id,status,evidence_stale,evidence_stale_at,acceptance_evidence_version,acceptance_pack_ids" }, fixture.client);
  await verifyCheckpoint({ schema: "procurement", table: "payment_readiness_staleness_events",
    filters: { payment_readiness_pack_id: fixture.ids.paymentReadinessPack },
    expected: { prior_status: "accepted" },
    select: "id,payment_readiness_pack_id,prior_status,prior_acceptance_evidence_version,acceptance_evidence_version" }, fixture.client);
  return { name: "Task 3 stale payment readiness invalidation", ok: true,
    replacementAcceptancePackId: replacement.acceptancePackId,
    acceptanceEvidenceVersion: Number(po.acceptance_evidence_version) };
}

async function task3FinalizePaymentReadiness(page, fixture) {
  const accepted = await callRpcAsBrowserUser(page, "procurement", "review_payment_readiness", {
    id: fixture.ids.paymentReadinessPack, status: "accepted",
    note: `${fixture.marker} finalized Finance decision before later acceptance evidence`,
  });
  if (!accepted.ok) throw new Error(`Finance readiness finalization failed: ${accepted.body}`);
  await verifyCheckpoint({ schema: "procurement", table: "payment_readiness_packs",
    filters: { id: fixture.ids.paymentReadinessPack }, expected: { status: "accepted", evidence_stale: false },
    select: "id,status,evidence_stale,acceptance_pack_ids,accepted_quantity" }, fixture.client);
  return { name: "Task 3 finalized Finance readiness preservation", ok: true,
    readinessPackId: fixture.ids.paymentReadinessPack };
}

async function task3RejectStalePaymentReadiness(page, fixture) {
  requireRpcFailure(await callRpcAsBrowserUser(page, "procurement", "review_payment_readiness", {
    id: fixture.ids.paymentReadinessPack, status: "accepted",
  ...
  return { name: "Task 3 stale Finance readiness denial", ok: true,
    readinessPackId: fixture.ids.paymentReadinessPack };
}
...
async function assertTask3ZeroResidualRows(fixture) {
  ...
  const { poIds } = fixture;
  const checks = [
    ["core.vendors", client.schema("core").from("vendors").select("id", { count: "exact", head: true }).in("id", [ids.vendor, ids.expiredVendor])],
    ["core.activity_log", client.schema("core").from("activity_log").select("id", { count: "exact", head: true }).like("entity_id", `${marker}%`)],
    ["core.documents", client.schema("core").from("documents").select("id", { count: "exact", head: true }).like("storage_path", `audit/${marker}/%`)],
    ["procurement.requests", client.schema("procurement").from("requests").select("id", { count: "exact", head: true }).like("id", `${marker}%`)],
    ["procurement.purchase_orders", client.schema("procurement").from("purchase_orders").select("id", { count: "exact", head: true }).in("id", poIds)],
    ["procurement.purchase_order_lines", client.schema("procurement").from("purchase_order_lines").select("id", { count: "exact", head: true }).in("purchase_order_id", poIds)],
    ["procurement.route_decisions", client.schema("procurement").from("route_decisions").select("id", { count: "exact", head: true }).like("request_id", `${marker}%`)],
    ...
    ["procurement.payment_readiness_staleness_events", client.schema("procurement").from("payment_readiness_staleness_events").select("id", { count: "exact", head: true }).in("purchase_order_id", poIds)],
    ["procurement.purchase_order_amendments", client.schema("procurement").from("purchase_order_amendments").select("id", { count: "exact", head: true }).in("purchase_order_id", poIds)],
    ["procurement.purchase_order_amendment_steps", client.schema("procurement").from("purchase_order_amendment_steps").select("id", { count: "exact", head: true }).eq("amendment_id", ids.excessAmendment)],
    ["procurement.doa_assignments Task 3 amendment", client.schema("procurement").from("doa_assignments").select("id", { count: "exact", head: true }).in("id", ids.amendmentDoaAssignments)],
    ["procurement.doa_matrices Task 3 amendment", client.schema("procurement").from("doa_matrices").select("id", { count: "exact", head: true }).eq("id", ids.amendmentDoaMatrix)],
    ...
    ["warehouse.receipts", client.schema("warehouse").from("receipts").select("id", { count: "exact", head: true }).in("procurement_po_id", poIds)],
    ["warehouse.routine quality receipt", client.schema("warehouse").from("receipts").select("id", { count: "exact", head: true }).eq("id", ids.routineQualityReceipt)],
    ["warehouse.vendor_returns", client.schema("warehouse").from("vendor_returns").select("id", { count: "exact", head: true }).like("reference", `${marker}%`)],
    ["warehouse.command_log", client.schema("warehouse").from("command_log").select("id", { count: "exact", head: true }).like("idempotency_key", `${marker}%`)],
    ["warehouse.movements", client.schema("warehouse").from("movements").select("id", { count: "exact", head: true }).eq("product_id", ids.product)],
    ["warehouse.stock_levels", client.schema("warehouse").from("stock_levels").select("product_id", { count: "exact", head: true }).eq("product_id", ids.product)],
    ["warehouse.stock_change_requests", client.schema("warehouse").from("stock_change_requests").select("id", { count: "exact", head: true }).in("source_id", [ids.cycleCount, ids.selfCycleCount])],
    ["warehouse.cycle_counts", client.schema("warehouse").from("cycle_counts").select("id", { count: "exact", head: true }).in("id", [ids.cycleCount, ids.selfCycleCount])],
    ["warehouse.suppliers", client.schema("warehouse").from("suppliers").select("id", { count: "exact", head: true }).eq("id", `proc-${ids.vendor}`)],
  ];
  if (fixture.cleanupReceiptIds?.length) checks.push(
    ["warehouse.quality_inspections", client.schema("warehouse").from("quality_inspections").select("id", { count: "exact", head: true }).in("source_id", fixture.cleanupReceiptIds)],
    ["warehouse.exceptions", client.schema("warehouse").from("exceptions").select("id", { count: "exact", head: true }).in("source_id", [...fixture.cleanupReceiptIds, ...(fixture.cleanupStockRequestIds ?? [])])]);
  if (fixture.cleanupQualityIds?.length) checks.push(["warehouse.inventory_holds", client.schema("warehouse").from("inventory_holds").select("id", { count: "exact", head: true }).in("inspection_id", fixture.cleanupQualityIds)]);
  if (fixture.cleanupHoldIds?.length) checks.push(
    ["warehouse.inventory_holds by UUID", client.schema("warehouse").from("inventory_holds").select("id", { count: "exact", head: true }).in("id", fixture.cleanupHoldIds)],
    ["core.activity_log hold-release activity", client.schema("core").from("activity_log").select("id", { count: "exact", head: true }).in("entity_id", fixture.cleanupHoldIds)]);
  if (fixture.cleanupStockRequestIds?.length) checks.push(["core.approvals", client.schema("core").from("approvals").select("id", { count: "exact", head: true }).eq("entity_type", "warehouse_stock_change").in("entity_id", fixture.cleanupStockRequestIds)]);
...
async function cleanupTask3ReceiptFixture(fixture) {
  ...
  const receiptIds = [...new Set([
    ...(receiptRows ?? []).map((row) => row.id), ids.quarantineReceipt, ids.routineQualityReceipt,
    ids.serializedIssueReceipt,
  ].filter(Boolean))];
  let qualityRows = [];
  if (receiptIds.length) {
    const { data, error } = await client.schema("warehouse").from("quality_inspections")
      .select("id").in("source_id", receiptIds);
    if (error) throw new Error(`Quality cleanup lookup failed: ${error.message}`);
    qualityRows = data ?? [];
  }
  const qualityIds = qualityRows.map((row) => row.id);
  let holdRows = [];
  if (qualityIds.length) {
    const { data, error } = await client.schema("warehouse").from("inventory_holds")
      .select("id").in("inspection_id", qualityIds);
    if (error) throw new Error(`Hold cleanup lookup failed: ${error.message}`);
    holdRows = data ?? [];
  }
  const holdIds = [...new Set([
    ...holdRows.map((row) => row.id), ...(fixture.cleanupHoldIds ?? []),
  ])];
  ...
  const stockRequestIds = [...new Set([
    ...(requestRows ?? []).map((row) => row.id),
    ...(fixture.cleanupStockRequestIds ?? []),
  ])];
  const exceptionSourceIds = [...receiptIds, ...stockRequestIds.map(String), ...qualityIds.map(String)];
  let exceptionRows = [];
  if (exceptionSourceIds.length) {
    const { data, error } = await client.schema("warehouse").from("exceptions")
      .select("id").in("source_id", exceptionSourceIds);
    if (error) throw new Error(`Exception cleanup lookup failed: ${error.message}`);
    exceptionRows = data ?? [];
  }
  const exceptionIds = [...new Set([...exceptionRows.map((row) => row.id), ...(fixture.cleanupExceptionIds ?? [])])];
  let decisionRows = [];
  if (receiptIds.length) {
    const { data, error } = await client.schema("warehouse")
      .from("procurement_receipt_exception_decisions").select("id").in("receipt_id", receiptIds);
    if (error) throw new Error(`Receipt decision cleanup lookup failed: ${error.message}`);
    decisionRows = data ?? [];
  }
  const decisionIds = [...new Set([...decisionRows.map((row) => row.id), ...(fixture.cleanupDecisionIds ?? [])])];
  ...
  fixture.cleanupActivityEntityIds = [...new Set([
    ...(fixture.cleanupActivityEntityIds ?? []), ...stockRequestIds.map(String),
    ...qualityIds.map(String), ...holdIds.map(String),
    ...exceptionIds.map(String), ...decisionIds.map(String),
  ])];
  ...
  if (holdIds.length) await remove("warehouse", "vendor_returns", (query) => query.in("hold_id", holdIds));
  if (holdIds.length) await remove("warehouse", "inventory_holds", (query) => query.in("id", holdIds));
  if (decisionIds.length) await remove("warehouse", "procurement_receipt_exception_decisions", (query) => query.in("id", decisionIds));
  ...
  if (exceptionIds.length) await remove("warehouse", "exceptions", (query) => query.in("id", exceptionIds));
  await remove("core", "documents", (query) => query.like("storage_path", `audit/${marker}/%`));
  await remove("warehouse", "command_log", (query) => query.like("idempotency_key", `${marker}%`));
  await remove("warehouse", "movements", (query) => query.in("product_id",
    [ids.product, ids.serializedIssueProduct]));
  if (receiptIds.length) {
  ...
  if (stockRequestIds.length) await remove("warehouse", "stock_change_requests", (query) => query.in("id", stockRequestIds));
  ...
  await remove("warehouse", "inventory_units", (query) => query.in("product_id",
    [ids.product, ids.serializedIssueProduct]));
  ...
  await remove("procurement", "payment_readiness_staleness_events", (query) => query.in("purchase_order_id", poIds));
  ...
  if (ids.excessAmendment) await remove("procurement", "purchase_order_amendment_steps", (query) => query.eq("amendment_id", ids.excessAmendment));
  await remove("procurement", "purchase_order_amendments", (query) => query.in("purchase_order_id", poIds));
  await remove("procurement", "purchase_order_lines", (query) => query.in("purchase_order_id", poIds));
  await remove("procurement", "purchase_orders", (query) => query.in("id", poIds));
  ...
  await remove("procurement", "doa_assignments", (query) => query.in("id", ids.amendmentDoaAssignments));
  await remove("procurement", "doa_matrices", (query) => query.eq("id", ids.amendmentDoaMatrix));
  await remove("warehouse", "products", (query) => query.in("id",
    [ids.product, ids.serializedIssueProduct]));
...
try {
  ...
  if (allowMutations) {
    for (const viewport of transactionViewports) {
      ...
      workflows.push(await runWorkflow(
        browser, viewport, { email: "intra.test.proc.approver@mwell.com.ph" },
      ...
      workflows.push(await runWorkflow(
        ...
        { name: "Task 3 Supervisor excess custody final disposition",
          run: (page) => task3SupervisorExcessFinalDisposition(page, task3Fixture) },
      ));
      workflows.push(await runWorkflow(
        browser, viewport, { email: "intra.test.proc.officer@mwell.com.ph" },
      ...
      workflows.push(await runWorkflow(
        browser, viewport, { email: "intra.test.proc.requester@mwell.com.ph" },
        { name: "Task 3 requester goods acceptance", run: (page) => task3GoodsAcceptance(
          page, task3Fixture, task3Fixture.ids.cleanPo, task3Fixture.ids.cleanLine,
      ...
      workflows.push(await runWorkflow(
        browser, viewport, { email: "intra.test.proc.finance@mwell.com.ph" },
      ...
      workflows.push(await runWorkflow(
        ...
        { name: "Task 3 stale payment readiness invalidation",
          run: (page) => task3InvalidateReadinessWithAcceptanceChange(page, task3Fixture) },
      ));
      workflows.push(await runWorkflow(
        browser, viewport, { email: "intra.test.proc.finance@mwell.com.ph" },
        { name: "Task 3 stale Finance readiness denial",
          run: (page) => task3RejectStalePaymentReadiness(page, task3Fixture) },
      ));
      workflows.push(
        await runWorkflow(
          browser,
