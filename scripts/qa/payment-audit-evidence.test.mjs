...
for (const [label, change] of [
  ...
  ['cancelled PO', h => { h.po.status = 'cancelled'; }],
  ['approved but unissued PO', h => { h.po.status = 'approved'; }],
  ['pending approval PO', h => { h.po.status = 'pending_approval'; }],
  ['missing PO status', h => { delete h.po.status; }],
  ['unknown PO status', h => { h.po.status = 'received'; }],
