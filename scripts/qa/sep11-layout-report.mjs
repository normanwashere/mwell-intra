...
const report=JSON.parse(await readFile(path.join(root,'layout-final/results.json'),'utf8'));
...
for(const key of Object.keys(screens)) for(const width of [1440,390]) await access(path.join(root,`layout-final/${key}-${width}.png`));
