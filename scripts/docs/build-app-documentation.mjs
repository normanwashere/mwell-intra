...
function taskArticle(guide, guideById) {
  ...
  const stageEvidence = (stage) => {
    ...
    const mobileScreenshotSource = embeddedImage(stage.screenshot.mobilePath, "");
    const label = stage.screenshot.target.label;
    return `<figure class="doc-image step-screenshot" data-screen-evidence="certified" data-certified-screenshot data-screenshot-binding="${escapeHtml(stage.screenshot.bindingId)}"><button class="screenshot-trigger" type="button" data-open-screenshot-surface="${escapeHtml(screenshotId)}" aria-controls="screenshot-viewer" aria-expanded="false"><picture><source media="(max-width: 767px)" srcset="${escapeHtml(mobileScreenshotSource)}" data-screen-variant="mobile"><img src="${escapeHtml(screenshotSource)}" alt="${escapeHtml(label)}" loading="lazy" data-screen-variant="desktop"></picture><span class="screenshot-action">View full screen</span></button><figcaption>${escapeHtml(label)}. Certified interaction target: ${escapeHtml(stage.screenshot.target.landmark)}. Route <code>${escapeHtml(stage.route)}</code>; role ${escapeHtml(roleName(stage.performingRole))}.</figcaption></figure>`;
  };
...
<body id="top">
  <header class="topbar" data-stable-region="header">
    <div class="topbar-first-row"><div class="brand"><strong>mwell</strong><span>Intra handbook</span></div><div class="search-wrap"><input id="search" type="search" placeholder="What do you need help with?" aria-label="What do you need help with?" autocomplete="off"></div><button class="drawer-trigger menu-trigger" type="button" data-open-drawer="contents" aria-controls="contents-rail" aria-expanded="false">Menu</button></div>
  </header>
