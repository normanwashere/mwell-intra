...
const mermaidBundleFile = path.join(
  root,
...
const brandLogoFile = path.join(root, "apps/shell/public/mwell-wordmark.png");
...
function resolveDocumentLink(href, sourceFile, sourceIds, sourceRoutes) {
  ...
  const target = normalize(
    path.relative(root, path.resolve(root, path.dirname(sourceFile), filePart)),
  );
  const id = sourceIds.get(target);
  const route =
    sourceRoutes.get(`${target}#${slug(fragment ?? "")}`) ??
    sourceRoutes.get(target);
  return id && route ? canonicalGuideHash(route) : href;
}

function flowDiagramMetadata(text) {
  const directive = String(text).match(
    /^\s*%%\s*handbook-flow:\s*([^\n]+)\n?/i,
  );
  if (!directive)
    return { source: text, workflow: null, view: null, stages: [] };
  ...
  const view = ["overview", "role", "decision"].includes(values.view)
    ? values.view
    : "overview";
  return {
...
function renderMarkdown(markdown, sourceFile, sourceIds, sourceRoutes) {
  ...
  renderer.image = ({ href, title, text }) => {
    ...
    const screenshotId = `${sourceIds.get(sourceFile) ?? slug(sourceFile)}-screenshot-${++screenshotIndex}`;
    return `<figure class="doc-image"><button class="screenshot-trigger" type="button" data-open-screenshot-surface="${escapeHtml(screenshotId)}" aria-controls="screenshot-viewer" aria-expanded="false"><img src="${escapeHtml(source)}" alt="${escapeHtml(text)}" loading="lazy"${title ? ` title="${escapeHtml(title)}"` : ""}><span class="screenshot-action">View full screen</span></button><figcaption id="${escapeHtml(`${screenshotId}-caption`)}">${escapeHtml(text)}</figcaption></figure>`;
  };
  renderer.link = function ({ href, title, tokens }) {
    const target = resolveDocumentLink(
      href,
      sourceFile,
      sourceIds,
      sourceRoutes,
  ...
  renderer.code = ({ text, lang }) => {
    if (lang !== "mermaid")
      return `<pre class="code-block"${lang ? ` data-language="${escapeHtml(lang)}"` : ""}><code>${escapeHtml(text)}</code></pre>`;
...
function disclosureDefaultOpen(document, heading, index, section) {
  ...
  return (
    ...
    /overview|at a glance|completion|success criteria|done when|outcome/i.test(
      heading,
    )
  );
}
...
function decorateArticleHtml(document, html) {
  ...
  const withDiagramIds = html.replace(
    /<figure class="diagram-shell"([^>]*)>([\s\S]*?)<\/figure>/g,
    (match, attributes, contents) => {
      ...
      const source =
        contents.match(/<div class="mermaid">([\s\S]*?)<\/div>/)?.[1] ??
        contents;
      const baseId =
        workflow && view
  ...
  const flowPattern =
    /<figure\b(?=[^>]*\bclass="diagram-shell")(?=[^>]*\bdata-flow-workflow="([^"]+)")(?=[^>]*\bdata-flow-view="([^"]+)")(?=[^>]*\bdata-flow-stages="([^"]*)")[^>]*>[\s\S]*?<\/figure>/g;
  ...
  for (let index = 0; index < diagrams.length;) {
    ...
    while (
      ...
      diagrams[end][1] === workflow &&
      withDiagramIds
        .slice(
          diagrams[end - 1].index + diagrams[end - 1][0].length,
          diagrams[end].index,
        )
        .trim() === ""
    )
    ...
    if (!["overview", "role", "decision"].every((view) => views.has(view))) {
      index = end;
      continue;
    }
    ...
    const groupEnd =
      group[group.length - 1].index + group[group.length - 1][0].length;
    ...
    const controls = [
      ...
      .map(
        ([view, label]) =>
          `<button type="button" data-diagram-view-control="${view}" aria-pressed="${view === "overview"}">${label}</button>`,
      )
      .join("");
    const ribbon = stages
      .map(
        (stage, stageIndex) =>
          `<li data-process-stage="${stageIndex}">${escapeHtml(stage)}</li>`,
      )
      .join("");
    const figures = group
      .map((diagram) =>
        diagram[0].replace(
          "<figure ",
          `<figure data-diagram-view="${diagram[2]}" `,
        ),
      )
      .join("");
  ...
  return groupedHtml
    ...
    .map((section) => {
      ...
      const open = disclosureDefaultOpen(
        document,
        plainMarkdownText(headingHtml),
        sectionIndex,
        section,
      );
      sectionIndex += 1;
...
function extractSourceFragment(sourceRecord, heading) {
  if (
    path.extname(sourceRecord.file).toLowerCase() === ".csv" ||
    heading == null
  ) {
  ...
  if (!selected)
    throw new Error(
      `Missing exact source heading ${heading} in ${sourceRecord.file}.`,
    );
  const end =
    headings.find(
      ({ index, depth }) => index > selected.index && depth <= selected.depth,
    )?.index ?? lines.length;
  return lines.slice(selected.index, end).join("\n");
}

export function loadDocumentationSources(sourceFiles = documentationSources()) {
  const { documents: catalogDocuments, errors } =
    resolveHandbookCatalog(sourceFiles);
  if (errors.length) {
    throw new Error(
      `Handbook source registry validation failed:\n${errors.join("\n")}`,
    );
  }
  const catalogBySource = new Map(
    catalogDocuments.map((document) => [document.source, document]),
  );
...
export function resolveHandbookModel(sourceInput, options = {}) {
  ...
  if (validation.errors.length) {
    throw new Error(
      `Handbook guide model validation failed:\n${validation.errors.join("\n")}`,
    );
  }
  ...
  for (const source of sources) {
    const articleRoute = legacyRoutes.find(
      (route) =>
    ...
    for (const { text } of markdownHeadingEntries(source.sourceText)) {
      ...
      const headingRoute = legacyRoutes.find(
        (route) =>
          route.legacyTabId === source.primaryTab &&
          route.legacyArticleId === source.legacyArticleId &&
      ...
      if (headingRoute)
        sourceRoutes.set(`${source.file}#${slug(text)}`, headingRoute);
    }
  }
...
export function composeHandbookGuides(model) {
  ...
  const librarySectionByFile = new Map(
    sourceLibrary.sourceSections.map((sourceSection) => [
  ...
  return model.guides.map((guide) => {
    ...
    const sourceIds = new Map(
      model.sources.map((source) => [source.file, source.legacyArticleId]),
    );
    const sourceReferences = guide.sourceSections.map((sourceSection) => {
      const source = model.sourceByFile.get(sourceSection.source);
      if (!source)
        throw new Error(
          `${guide.id} references unavailable source ${sourceSection.source}.`,
        );
      ...
      if (!librarySection)
        throw new Error(
          `${source.file} has no canonical System source-library section.`,
        );
      const rootHeading = source.sourceText.match(/^#\s+(.+)$/m)?.[1]?.trim();
      const canonicalTargetId =
        sourceSection.heading && sourceSection.heading !== rootHeading
      ...
      return {
        ...
        reviewDate:
          guide.lastReviewedDate ?? "Generated with the current handbook",
...
const CONTROLLED_SEARCH_TERMS = Object.freeze({
  "finance-readiness-evidence:Step:step-2": [
    "three-way match",
    "match purchase order receipt invoice",
  ],
  "procurement-request-approval:Task:outcome": [
    "approve request",
    "RFQ",
  ...
  "stock-receiving-putaway:Task:outcome": [
    "receive stock",
    "receiving",
    "receipt",
  ],
  "stock-receiving-putaway:Step:step-3": [
    "report damaged item",
    "damaged stock",
    "quarantine",
  ],
  "platform_administrator:Troubleshooting:negative-and-recovery-scenario": [
    "reset password",
    "invalid login",
    "access denied",
    "password recovery",
  ...
  "vendor-accreditation-renewal:Task:outcome": [
    "vendor renewal",
    "renew vendor",
  ],
  "ecommerce-fulfillment-delivery:Task:outcome": ["pick and pack"],
  "returns-replacements-refunds-rma:Task:outcome": ["refund"],
  "event-stock-custody:Decision:decision-2": [
    "lost event stock",
    "damaged event stock",
  ],
  "inventory-count-variance:Task:outcome": [
    "cycle count",
    "cycle count variance",
  ],
  "department-doa-activation:Task:outcome": ["DOA", "delegation of authority"],
  "platform_administrator:Role:role-purpose-and-department": [
    "security",
    "admin",
    "administration",
    "configuration",
  ],
  "administration-configuration:System reference:overview": [
    "DOA",
    "delegation",
    "delegation of authority",
    "authority",
...
function guideSearchRecord({
  ...
  const controlledTerms =
    CONTROLLED_SEARCH_TERMS[`${guide.id}:${type}:${headingId}`] ?? [];
  return {
    type,
    modeId,
    guideId: guide.id,
    ...
    searchText: plainMarkdownText(
      [searchText, controlledTerms].flat().join(" "),
    ),
    tabId,
    tabIds: [tabId],
...
export function buildGuideSearchIndex(guides) {
  const guideById = new Map(guides.map((guide) => [guide.id, guide]));
  const roleLabel = (id) =>
    guideById.get(id)?.canonicalName ?? guideById.get(id)?.title ?? id;
  const records = [];
  ...
  for (const guide of guides.filter(({ type }) => type === "task")) {
    ...
    records.push(
      guideSearchRecord({
        ...
        searchText: [guide.recovery, guide.denialChecks, guide.decisionLabels]
          .flat()
          .join(" "),
        audience: roles,
        source: sources,
  ...
  for (const guide of guides.filter(({ type }) => type === "role")) {
    const modules = [
      ...new Set(guide.workspaceMap.map(({ module }) => module)),
    ];
    records.push(
      guideSearchRecord({
        type: "Role",
        ...
        searchText: [
          guide.canonicalName,
          guide.displayedAliases,
          guide.purpose,
          guide.departmentAndScope,
          modules,
          guide.permittedActions,
          guide.linkedTasks,
        ]
          .flat()
          .join(" "),
    ...
    records.push(
      guideSearchRecord({
        ...
        searchText: [
          guide.denialChecks,
          guide.escalationAndRecovery,
          guide.guidedSimulation.negativeScenario,
          guide.guidedSimulation.recovery,
        ]
          .flat()
          .join(" "),
  ...
  for (const guide of guides.filter(({ type }) => type === "system")) {
    records.push(
      guideSearchRecord({
        ...
        keywords: guide.keywords,
        searchText: [guide.title, guide.summary, guide.audience, guide.keywords]
          .flat()
          .join(" "),
    ...
    for (const reference of guide.id === "source-references"
...
function documentControls(
  ...
  references,
  {
    sectionId = "document-controls",
    label = "Technical metadata and governed sources",
    addressableReferences = true,
  } = {},
) {
...
function taskFlow(guide, roleName = (id) => id) {
  const lines = [
    "flowchart LR",
    `START([${mermaidLabel(guide.startCondition)}])`,
  ];
  const stageNodes = new Map(
    guide.steps.map((stage, index) => [stage.id, `S${index + 1}`]),
  );
  const decisionNodes = new Map(
    guide.decisionPoints.map((decision, index) => [
      decision.id,
      `D${index + 1}`,
    ]),
  );
  const decisionIdsWithInboundBranches = new Set(
    guide.decisionPoints
      .flatMap((decision) => [decision.yesBranch, decision.noBranch])
      .filter(({ target }) => target.type === "decision")
      .map(({ target }) => target.id),
  );
  const decisionsAt = (position, stageId) =>
    guide.decisionPoints.filter(
      (decision) =>
  ...
  guide.steps.forEach((stage, index) =>
    lines.push(`S${index + 1}["${index + 1}. ${mermaidLabel(stage.label)}"]`),
  );
  guide.decisionPoints.forEach((decision, index) =>
    lines.push(`D${index + 1}{"${mermaidLabel(decision.question)}"}`),
  );
  ...
  if (firstStage) {
    ...
    lines.push(
      `START --> ${beforeFirst.length ? decisionNodes.get(beforeFirst[0].id) : stageNodes.get(firstStage.id)}`,
    );
  }
  for (const [index, stage] of guide.steps.entries()) {
    ...
    if (after.length) {
      for (const decision of after)
        lines.push(`${stageNode} --> ${decisionNodes.get(decision.id)}`);
    } else if (index + 1 < guide.steps.length) {
      const nextStage = guide.steps[index + 1];
      const beforeNext = decisionsAt("before", nextStage.id);
      lines.push(
        `${stageNode} --> ${beforeNext.length ? decisionNodes.get(beforeNext[0].id) : stageNodes.get(nextStage.id)}`,
      );
    } else {
  ...
  for (const decision of guide.decisionPoints) {
    for (const branch of [decision.yesBranch, decision.noBranch]) {
      ...
      lines.push(
        `${decisionNodes.get(decision.id)} -->|${mermaidLabel(branch.label)}| ${decisionTargetNode(branch.target, stageNodes, decisionNodes)}`,
      );
    }
  }
  for (const outcome of outcomes) {
    const label = outcome
      ...
      .replace(/\b\w/g, (letter) => letter.toUpperCase());
    lines.push(
      `O_${outcome.replace(/[^a-z0-9]+/gi, "_").toUpperCase()}([${label}])`,
    );
  }
  const source = lines.join("\n");
  const stages = guide.steps.map(({ label }) => label).join("|");
  const stageRibbon = `<ol class="task-flow-ribbon" aria-label="Task stages">${guide.steps
    .map((stage, index) => {
      const decisions = guide.decisionPoints.filter(
        ({ placement }) => placement.stageId === stage.id,
      );
      return `<li><a href="${escapeHtml(canonicalGuideHash({ modeId: "tasks", guideId: guide.id, headingId: stage.id }))}" data-route-link><span>${index + 1}</span><div><strong>${escapeHtml(stage.label)}</strong><small>${escapeHtml(roleName(stage.performingRole))}${decisions.length ? ` · ${decisions.length} decision${decisions.length === 1 ? "" : "s"}` : ""}</small></div></a></li>`;
    })
    .join("")}</ol>`;
  const textEquivalent = `<div class="flow-text-equivalent"><h3>Flow text equivalent</h3><ol>${guide.steps
    .map(
      (stage, index) =>
        `<li>${index + 1}. ${escapeHtml(stage.label)} by ${escapeHtml(stage.performingRole)}.${guide.decisionPoints
          .filter(({ placement }) => placement.stageId === stage.id)
          .map((decision) => ` Decision: ${decision.question}`)
          .join("")}</li>`,
    )
    .join("")}</ol><p><strong>Branches:</strong> ${escapeHtml(
    guide.decisionPoints
      .flatMap((decision) => [decision.yesBranch, decision.noBranch])
      .map((branch) => `${branch.label}: ${branch.outcome}`)
      .join(" "),
  )}</p></div>`;
  const decisionMap = `<details class="full-decision-map"><summary><span>Explore the complete decision map</span><small>${guide.decisionPoints.length} governed decision${guide.decisionPoints.length === 1 ? "" : "s"}, including recovery and terminal outcomes</small></summary><figure class="diagram-shell" data-diagram-id="${escapeHtml(`${guide.id}:flow:overview`)}" data-flow-workflow="${escapeHtml(guide.id)}" data-flow-view="overview" data-flow-stages="${escapeHtml(stages)}"><div class="diagram-toolbar" aria-label="Diagram zoom controls"><button type="button" data-diagram-fit aria-label="Fit diagram to available space">Fit</button><button type="button" data-diagram-zoom="reset" aria-label="Show diagram at 100 percent">100%</button><button type="button" data-diagram-zoom="out" aria-label="Zoom diagram out">−</button><button type="button" data-diagram-zoom="in" aria-label="Zoom diagram in">+</button></div><div class="diagram-viewport" data-diagram-viewport role="region" tabindex="0" aria-label="Task flow diagram"><div class="diagram-canvas"><div class="mermaid">${escapeHtml(source)}</div></div></div><figcaption>Complete task flow with decision and terminal outcomes.</figcaption></figure>${textEquivalent}</details>`;
  return `<div class="flow-overview"><p>Follow the four operating stages in order. Open a stage to see the exact screen, authority boundary, retained evidence, and next owner.</p>${stageRibbon}${decisionMap}</div>`;
}
...
function taskArticle(guide, guideById) {
  const roleName = (id) =>
    guideById.get(id)?.canonicalName ?? guideById.get(id)?.title ?? id;
  const policyReferences = guide.sourceReferences.filter(
    ({ purpose }) => purpose === "policy-basis",
  );
  const supportReferences = guide.sourceReferences.filter(
    ({ purpose }) => purpose !== "policy-basis",
  );
  const involved = guide.participatingRoles
    .map((id) => {
      const role = guideById.get(id);
      return role
        ? `<li>${guideLink(role, { label: roleName(id), summary: "" })}</li>`
        : `<li>${escapeHtml(roleName(id))}</li>`;
    })
    .join("");
  ...
  const stageNavigation = (index) => {
    const previous = guide.steps[index - 1];
    const next = guide.steps[index + 1];
    return `<nav class="stage-navigation" aria-label="Step ${index + 1} navigation">${previous ? `<a href="${escapeHtml(canonicalGuideHash({ modeId: "tasks", guideId: guide.id, headingId: previous.id }))}" data-route-link>Previous: ${escapeHtml(previous.label)}</a>` : '<span aria-hidden="true"></span>'}${next ? `<a href="${escapeHtml(canonicalGuideHash({ modeId: "tasks", guideId: guide.id, headingId: next.id }))}" data-route-link>Next: ${escapeHtml(next.label)}</a>` : `<a href="${escapeHtml(canonicalGuideHash({ modeId: "tasks", guideId: guide.id, headingId: "completion-checklist" }))}" data-route-link>Next: Completion checklist</a>`}</nav>`;
  };
  const stageEvidence = (stage) => {
    if (stage.screenshot.status !== "certified") {
      return `<aside class="screen-evidence-pending" data-screen-evidence="pending" data-screenshot-binding="${escapeHtml(stage.screenshot.bindingId)}" role="status"><strong>Screen evidence pending review</strong><p>The procedure remains usable while the stage-specific application evidence is reviewed.</p></aside>`;
    }
    const screenshotId = slug(stage.screenshot.bindingId);
    const screenshotSource = embeddedImage(stage.screenshot.path, "");
    const mobileScreenshotSource = embeddedImage(
      stage.screenshot.mobilePath,
      "",
    );
    const label = stage.screenshot.target.label;
    const hotspot = (variant, viewport) => {
      const box = variant?.targetBox;
      if (!box || !variant.width || !variant.height) return "";
      const style = `--target-left:${(box.x / variant.width) * 100}%;--target-top:${(box.y / variant.height) * 100}%;--target-width:${(box.width / variant.width) * 100}%;--target-height:${(box.height / variant.height) * 100}%`;
      const marker = stage.screenshot.assertions?.numberedCallout
        ? ""
        : "<b>1</b>";
      return `<span class="screen-hotspot screen-hotspot-${viewport}" style="${escapeHtml(style)}" aria-hidden="true">${marker}</span>`;
    };
    const desktop = stage.screenshot.variants.find(
      ({ viewport }) => viewport === "desktop",
    );
    const mobile = stage.screenshot.variants.find(
      ({ viewport }) => viewport === "mobile",
    );
    return `<figure class="doc-image step-screenshot" data-screen-evidence="certified" data-certified-screenshot data-screenshot-binding="${escapeHtml(stage.screenshot.bindingId)}"><div class="screen-instruction"><span>1</span><div><strong>Select ${escapeHtml(label)}</strong><small>${escapeHtml(stage.screenshot.target.landmark)}</small></div></div><button class="screenshot-trigger" type="button" data-open-screenshot-surface="${escapeHtml(screenshotId)}" aria-controls="screenshot-viewer" aria-expanded="false"><span class="screenshot-media"><picture><source media="(max-width: 767px)" srcset="${escapeHtml(mobileScreenshotSource)}" data-screen-variant="mobile"><img src="${escapeHtml(screenshotSource)}" alt="${escapeHtml(label)}" loading="lazy" data-screen-variant="desktop"></picture>${hotspot(desktop, "desktop")}${hotspot(mobile, "mobile")}</span><span class="screenshot-action">Enlarge screen</span></button><figcaption><strong>Expected target:</strong> ${escapeHtml(label)}. Route <code>${escapeHtml(stage.route)}</code>; signed in as ${escapeHtml(roleName(stage.performingRole))}.</figcaption></figure>`;
  };
  const stages = `<ol class="task-stages">${guide.steps.map((stage, index) => `<li id="${escapeHtml(`${guide.id}-${stage.id}`)}" data-task-stage="${escapeHtml(stage.id)}"><header><span>Step ${index + 1}</span><h3>${escapeHtml(stage.label)}</h3><p>${escapeHtml(roleName(stage.performingRole))} | ${escapeHtml(stage.module)} | <code>${escapeHtml(stage.route)}</code></p></header><p>${escapeHtml(stage.instruction)}</p>${stageEvidence(stage)}<dl><dt>Expected visible result</dt><dd>${escapeHtml(stage.expectedResult)}</dd><dt>Data read</dt><dd>${escapeHtml(stage.dataRead.join("; "))}</dd><dt>Data written</dt><dd>${escapeHtml(stage.dataWritten.join("; "))}</dd><dt>Evidence retained</dt><dd>${escapeHtml(stage.evidenceRetained.join("; "))}</dd><dt>Next handoff</dt><dd>${escapeHtml(roleName(stage.nextHandoff))}</dd></dl>${stageNavigation(index)}</li>`).join("")}</ol>`;
  ...
  const related = guide.relatedTasks
  ...
  const jumpControls = `<nav class="guide-jump-controls" aria-label="Task guide controls"><span>${guide.steps.length} steps · ${guide.decisionPoints.length} decisions</span><a href="${escapeHtml(canonicalGuideHash({ modeId: "tasks", guideId: guide.id, headingId: "step-1" }))}" data-route-link>Start at step 1</a><button type="button" data-open-drawer="toc" aria-controls="page-toc" aria-expanded="false">In this guide</button></nav>`;
  const roles = guide.participatingRoles.map(roleName);
  return `<article id="guide-${escapeHtml(guide.id)}" data-guide data-document data-guide-id="${escapeHtml(guide.id)}" data-guide-type="task" data-mode="tasks" data-tab="workflows" data-related-tabs="" data-category="Tasks" data-search="${escapeHtml([guide.title, guide.summary, guide.keywords].flat().join(" ").toLowerCase())}" hidden><header class="article-header"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="#mode=tasks" data-route-link>Tasks</a><span aria-hidden="true">/</span><span>${escapeHtml(guide.module)}</span></nav><div><span class="category">${escapeHtml(guide.module)} task</span><h1>${escapeHtml(guide.title)}</h1><p>${escapeHtml(guide.summary)}</p><dl class="guide-facts"><div><dt>Primary roles</dt><dd>${escapeHtml(roles.join(", "))}</dd></div><div><dt>Completion</dt><dd>${escapeHtml(guide.outcome)}</dd></div></dl></div></header>${jumpControls}<div class="article-body">${guideSection(guide, "outcome", "Outcome", `<p>${escapeHtml(guide.outcome)}</p>`)}${guideSection(guide, "flow", "Process at a glance", taskFlow(guide, roleName))}${guideSection(guide, "who-is-involved", "Who is involved", `<ul>${involved}</ul><p><strong>Accountable closer:</strong> ${escapeHtml(roleName(guide.steps.at(-1)?.performingRole ?? guide.participatingRoles.at(-1)))}</p>`)}${guideSection(guide, "before-you-start", "Before you start", prerequisites)}${guideSection(guide, "steps", "Follow the steps", stages)}${guideSection(guide, "decisions-and-exceptions", "Decisions and exceptions", decisions)}${guideSection(guide, "completion-checklist", "Completion checklist", completion)}${guideSection(guide, "related-tasks", "Related tasks", related.length ? `<div class="article-list">${related.map((item) => guideLink(item, { related: true })).join("")}</div>` : "<p>No direct continuation or recovery guide.</p>")}${policy}${documentControls(guide, supportReferences)}</div></article>`;
}

function roleArticle(guide, guideById) {
  const linkedTasks = guide.linkedTasks
    .map((id) => guideById.get(id))
    .filter(Boolean);
  ...
  return `<article id="guide-${escapeHtml(guide.id)}" data-guide data-document data-guide-id="${escapeHtml(guide.id)}" data-guide-type="role" data-mode="roles" data-tab="roles" data-related-tabs="" data-category="Roles" data-search="${escapeHtml([guide.title, guide.summary, guide.keywords].flat().join(" ").toLowerCase())}" hidden><header class="article-header"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="#mode=roles" data-route-link>Roles</a><span aria-hidden="true">/</span><span>${escapeHtml(guide.departmentAndScope.split(/[.;]/)[0])}</span></nav><div><span class="category">Role guide</span><h1>${escapeHtml(guide.canonicalName)}</h1><p>${escapeHtml(guide.summary)}</p><dl class="guide-facts"><div><dt>Department and scope</dt><dd>${escapeHtml(guide.departmentAndScope)}</dd></div><div><dt>Assignment owner</dt><dd>${escapeHtml(guide.assignmentOwner)}</dd></div></dl></div></header><div class="article-body">${guideSection(guide, "role-purpose-and-department", "Role purpose and department", `<p>${escapeHtml(guide.purpose)}</p><p><strong>Department and scope:</strong> ${escapeHtml(guide.departmentAndScope)}</p><p><strong>Assignment owner:</strong> ${escapeHtml(guide.assignmentOwner)}</p>`)}${guideSection(guide, "your-workspace", "Your workspace", `${workspace}<h3>Required access</h3>${renderItems(guide.requiredAccess)}`)}${guideSection(guide, "work-queue-and-priorities", "Work queue and priorities", renderItems(guide.workQueueOrStartConditions))}${guideSection(guide, "permitted-actions", "What you can do", renderItems(guide.permittedActions))}${guideSection(guide, "decisions-and-approval-authority", "Your decisions and approval authority", renderItems(guide.authorityLimits))}${guideSection(guide, "prohibited-actions", "What stays with another owner", renderItems(guide.prohibitedActions))}${guideSection(guide, "handoffs-received-and-sent", "Handoffs you receive and send", renderItems(guide.handoffs))}${guideSection(guide, "guided-simulation", "Practice your primary workflow", simulation)}${guideSection(guide, "negative-and-recovery-scenario", "Practice a recovery scenario", recovery)}${guideSection(guide, "escalation-and-support", "Escalation and support", `<p>${escapeHtml(guide.escalationAndRecovery)}</p>`)}${guideSection(guide, "completion-evidence-and-training-sign-off", "Training completion", `<h3>Evidence responsibilities</h3>${renderItems(guide.evidenceResponsibilities)}<h3>Training sign-off</h3>${renderItems(guide.trainingReadiness)}<h3>Related tasks</h3><div class="article-list">${linkedTasks.map((task) => guideLink(task)).join("")}</div>`)}${documentControls(guide, guide.sourceReferences, { sectionId: "capability-codes-and-document-controls", label: "Technical access and document metadata" })}</div></article>`;
}

function systemArticle(guide) {
  const ownsGovernedBodies = guide.id === "source-references";
  const references = guide.sourceReferences
    .map((reference) =>
      ownsGovernedBodies
        ? sourceReferenceBody(guide, reference)
        : sourceReferenceLink(guide, reference),
    )
    .join("");
...
function homeGuide(guide, guides, guideById) {
  ...
  const frequentTasks = frequentIds
    .map((id) => guideById.get(id))
    .filter(Boolean);
  const roles = guides.filter(({ type }) => type === "role");
  const systemGuides = guides
    .filter(({ type }) => type === "system")
    .slice(0, 4);
  return `<section class="hero home-guide" data-guide data-guide-id="home" data-guide-type="home" data-mode="home"><div class="home-intro"><span class="category">Mwell Intra operating handbook</span><h1>Find the answer, then do the work.</h1><p>Start with a task when you know the outcome. Start with your role when you are learning what you own.</p><nav class="home-paths" aria-label="Handbook starting points"><a href="#mode=tasks" data-route-link><strong>Browse tasks</strong><span>Step-by-step operating guides</span></a><a href="#mode=roles" data-route-link><strong>Learn a role</strong><span>Authority, workspace, and practice</span></a><a href="#mode=system" data-route-link><strong>Run the system</strong><span>Architecture, controls, and support</span></a></nav></div><section id="home-start-a-task" data-home-section="start-a-task"><div class="section-heading"><div><span>Common work</span><h2>Start a task</h2></div><a href="#mode=tasks" data-route-link>View all tasks</a></div><div class="article-list frequent-tasks">${frequentTasks.map((task) => guideLink(task)).join("")}</div></section><section id="home-learn-my-role" data-home-section="learn-my-role"><div class="section-heading"><div><span>Personal guidance</span><h2>Learn my role</h2></div><a href="#mode=roles" data-route-link>Compare roles</a></div><div class="role-picker"><label for="role-entry">Choose your assigned role</label><div><select id="role-entry" data-role-entry><option value="">Select a role</option>${roles.map((role) => `<option value="${escapeHtml(role.id)}">${escapeHtml(role.canonicalName)}</option>`).join("")}</select><button type="button" data-open-role>Open role guide</button></div></div></section><section id="home-manage-support" data-home-section="manage-support"><div class="section-heading"><div><span>For owners and support teams</span><h2>Manage or support Mwell Intra</h2></div><a href="#mode=system" data-route-link>View system library</a></div><div class="article-list">${systemGuides.map((item) => guideLink(item)).join("")}</div></section><section id="home-recent-guides" data-home-section="recent-guides"><h2>Continue reading</h2><ol data-recent-guides><li>No recent guides yet.</li></ol></section>${documentControls(guide, guide.sourceReferences, { label: "Document controls" })}</section>`;
}

function modePanel(mode, guides) {
  const tabId = MODE_TAB_ALIASES[mode.id];
  const modeGuides = guides.filter(
    (guide) => guide.modeId === mode.id && guide.type !== "home",
  );
  return `<section id="mode-panel-${escapeHtml(mode.id)}" class="mode-panel-shell"><div role="tabpanel" id="panel-${escapeHtml(tabId)}" aria-labelledby="mode-${escapeHtml(mode.id)}" data-mode-panel data-mode="${escapeHtml(mode.id)}" data-tab-panel${mode.id === "home" ? "" : " hidden"} tabindex="0"><header class="panel-header"><h2>${escapeHtml(mode.label)}</h2><p>${escapeHtml(mode.summary)}</p></header>${modeGuides.length ? `<div class="article-list compact-guide-list">${modeGuides.map((guide) => guideLink(guide, { summary: "" })).join("")}</div>` : "<p>Choose an outcome, role, or system responsibility from Home.</p>"}</div></section>`;
}

function modeLanding(mode, guides) {
  if (mode.id === "home") return "";
  const modeGuides = guides.filter(
    (guide) => guide.modeId === mode.id && guide.type !== "home",
  );
  const labelFor = (guide) =>
    guide.type === "task"
      ? `${guide.steps.length} steps · ${guide.module}`
      : guide.type === "role"
        ? guide.departmentAndScope
        : (guide.audience ?? []).join(", ");
  const cards = modeGuides
    .map(
      (guide) =>
        `<a class="mode-guide-card" href="${escapeHtml(canonicalGuideHash({ modeId: guide.modeId, guideId: guide.id }))}" data-guide-link data-article-link data-mode="${escapeHtml(guide.modeId)}" data-guide-id="${escapeHtml(guide.id)}" data-tab="${escapeHtml(MODE_TAB_ALIASES[guide.modeId])}" data-article="guide-${escapeHtml(guide.id)}"><span>${escapeHtml(labelFor(guide))}</span><strong>${escapeHtml(guide.title)}</strong><p>${escapeHtml(guide.summary)}</p><small>Open guide</small></a>`,
    )
    .join("");
  const noun =
    mode.id === "tasks"
      ...
      : mode.id === "roles"
        ? "role guides"
        : "system guides";
  return `<section class="mode-landing" data-mode-landing data-mode="${escapeHtml(mode.id)}" hidden><header><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="#mode=home&amp;guide=home" data-route-link>Handbook</a><span aria-hidden="true">/</span><span>${escapeHtml(mode.label)}</span></nav><span class="category">${escapeHtml(mode.label)}</span><h1>${escapeHtml(mode.label === "Tasks" ? "What are you trying to complete?" : mode.label === "Roles" ? "What work are you responsible for?" : "Operate and govern Mwell Intra")}</h1><p>${escapeHtml(mode.summary)}</p><div class="landing-count"><strong>${modeGuides.length}</strong><span>${escapeHtml(noun)}</span></div></header><div class="mode-guide-grid">${cards}</div></section>`;
}

export function renderHandbookShell({
  model,
  guides,
  searchIndex,
  styles,
  ...
  const articles = guides
    ...
    .map((guide) =>
      guide.type === "task"
        ? taskArticle(guide, guideById)
        : guide.type === "role"
          ? roleArticle(guide, guideById)
          : systemArticle(guide),
    )
    .join("\n")
...
<body id="top">
  <header class="topbar" data-stable-region="header">
    <div class="topbar-first-row"><a class="brand" href="#mode=home&amp;guide=home" data-route-link aria-label="Mwell Intra operating handbook home"><img src="${brandLogo}" alt=""><span class="brand-copy"><strong>Intra</strong><small>Operating handbook</small></span></a><div class="search-wrap"><input id="search" type="search" placeholder="Search tasks, roles, controls, and policies" aria-label="Search tasks, roles, controls, and policies" autocomplete="off"></div><button class="drawer-trigger menu-trigger" type="button" data-open-drawer="contents" aria-controls="contents-rail" aria-expanded="false">Menu</button></div>
  </header>
  <div class="handbook-shell">
    <aside id="contents-rail" class="contents-rail navigation-rail" aria-labelledby="contents-title" data-stable-region="navigation"><div class="drawer-heading"><h2 id="contents-title">Menu</h2><button type="button" data-close-drawer="contents" aria-label="Close menu">Close</button></div><nav class="tab-rail" role="tablist" aria-label="Handbook modes">${model.modes.map((mode, index) => `<button role="tab" id="mode-${escapeHtml(mode.id)}" aria-controls="panel-${escapeHtml(MODE_TAB_ALIASES[mode.id])}" aria-selected="${index === 0}" tabindex="${index === 0 ? 0 : -1}" type="button" data-mode-button data-mode="${escapeHtml(mode.id)}" data-tab-button data-tab="${escapeHtml(MODE_TAB_ALIASES[mode.id])}">${escapeHtml(mode.label)}</button>`).join("")}</nav><div class="toolbar"><span data-current-mode data-current-mode-label>Home</span><button id="theme" type="button" aria-label="Toggle color theme">Theme</button><div class="print-control"><button type="button" data-print-trigger aria-controls="print-menu" aria-expanded="false" aria-haspopup="dialog">Print</button></div></div><div class="summary"><strong>Find a guide</strong><div class="result-count" id="result-count" aria-live="polite">Choose a task, role, or system guide</div></div><div class="search-scope" role="group" aria-label="Search scope"><button type="button" data-search-scope="tab" aria-pressed="false">This mode</button><button type="button" data-search-scope="all" aria-pressed="true">All guides</button></div><section class="search-results" id="search-results" aria-label="Search results" hidden></section><section class="empty" id="empty" aria-labelledby="empty-title" hidden><h2 id="empty-title">No direct answer found</h2><p role="status" aria-live="polite">Try a related term or start from a frequent task.</p><div class="article-list" aria-label="Controlled search suggestions"><a href="#mode=home&amp;guide=home&amp;q=receive+stock&amp;scope=all" data-route-link data-search-suggestion="receive stock"><span>Receive stock</span><small>Frequent task</small></a><a href="#mode=home&amp;guide=home&amp;q=approve+request&amp;scope=all" data-route-link data-search-suggestion="approve request"><span>Approve a request</span><small>Frequent task</small></a><a href="#mode=home&amp;guide=home&amp;q=cycle+count&amp;scope=all" data-route-link data-search-suggestion="cycle count"><span>Cycle count</span><small>Frequent task</small></a></div><div class="article-list" aria-label="Other handbook paths"><a href="#mode=roles" data-route-link>Browse role guides</a><a href="#mode=system&amp;guide=source-references&amp;scope=mode" data-route-link data-no-result-system>Search System references</a></div></section>${model.modes.map((mode) => modePanel(mode, guides)).join("")}</aside>
    <main class="reading-canvas" tabindex="-1" data-stable-region="reading">
      ...
      ${model.modes.map((mode) => modeLanding(mode, guides)).filter(Boolean).join("\n")}
      ${articles}
    </main>
  ...
  <div id="print-menu" class="print-menu" role="dialog" aria-label="Print options" tabindex="-1" hidden><button type="button" data-print-scope="guide">Current guide</button><button type="button" data-print-scope="mode">Current mode</button><button type="button" data-print-scope="all">Complete handbook</button></div>
  <section id="screenshot-viewer" class="screenshot-viewer" data-screenshot-surface aria-labelledby="screenshot-viewer-title" hidden><header><div><span class="category">Screen evidence</span><h2 id="screenshot-viewer-title">Screenshot viewer</h2></div><button type="button" data-close-screenshot-surface aria-label="Close screenshot">Close</button></header><figure><div class="screenshot-viewport"><img data-screenshot-viewer-image alt=""></div><figcaption data-screenshot-viewer-caption></figcaption></figure></section>
...
export function buildDocumentationHtml(sourceFiles = documentationSources()) {
  ...
  const mermaidBundle = readFileSync(mermaidBundleFile, "utf8").replace(
    /[ \t]+$/gm,
    "",
  );
  const styles = normalizeText(
    readFileSync(handbookStylesFile, "utf8"),
  ).replace(/[ \t]+$/gm, "");
  const runtime = normalizeText(readFileSync(handbookRuntimeFile, "utf8"))
    .replace(/[ \t]+$/gm, "")
    .replaceAll("</script", "<\\/script");
  const brandLogo = `data:${mimeFor(brandLogoFile)};base64,${readFileSync(brandLogoFile).toString("base64")}`;
  return renderHandbookShell({
    model,
    guides,
    searchIndex,
    styles,
...
export function writeDocumentationHtml({ check = false } = {}) {
  ...
  if (check) {
    if (!existsSync(absolute) || readFileSync(absolute, "utf8") !== html) {
      throw new Error(
        `Consolidated documentation is stale. Run pnpm docs:build and commit ${outputFile}.`,
      );
    }
  ...
  console.log(
    `Wrote ${outputFile} from ${documentationSources().length} source documents.`,
  );
}
