...
const repoUrl = `https://github.com/normanwashere/mwell-intra/blob/${commit}/`;
...
const pinned = p => {
  if (!snapshot) return git('show', `${commit}:${p}`);
...
const subtitles = { index: 'September 22, 2026 | Data, engineering and controlled UAT coordination', data: 'Proposed reporting contract, recipient decisions and future integration acceptance', technical: 'Source-bound operating context, release gaps and receiving-team responsibilities', testers: 'Role-based sessions, saved-result checks and cross-team handoffs' };
...
const ledger = { sourceCommit: commit, sourceReviewOnly: true, noLiveChecks: true, reportingApiRouteAbsentInPinnedTree: true, references: sourceReview };
const commonFiles = {
