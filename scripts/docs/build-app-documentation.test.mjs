...
function parseProcurementOverviewStages(process) {
  const overview = markdownSection(
    process,
    3,
    "Canonical 13-step procurement-to-payment overview",
  );
  ...
  const sequence = Array.from(
    { length: EXPECTED_PROCUREMENT_STAGE_LABELS.length },
    (_, index) => index + 1,
  );

  assert.deepEqual(
    nodes.map(({ nodeNumber }) => nodeNumber),
    sequence,
    "procurement overview step nodes must be sequential",
  );
  assert.deepEqual(
    nodes.map(({ displayedNumber }) => displayedNumber),
    sequence,
    "procurement overview step numbers must be sequential",
  );
  ...
  return nodes.map(({ displayedNumber, mermaidLabel }) => {
    ...
    assert.notEqual(
      sourceIndex,
      -1,
      `unrecognized procurement overview stage ${displayedNumber}: ${mermaidLabel}`,
    );
    assert.equal(
      sourceIndex,
      displayedNumber - 1,
      `procurement overview stage ${displayedNumber} has the wrong meaning`,
    );
...
function assertTask11Structure(process, manual) {
  const operatingExtract = markdownSection(
    process,
    2,
    "Procurement Policy Operating Extract",
  );
  const operatingH3 = [...operatingExtract.matchAll(/^### (.+)$/gm)].map(
    (match) => match[1],
  );
  assert.deepEqual(
    operatingH3,
  ...
  const metadata = manual.match(
    /%% handbook-flow: workflow=procurement-to-payment; view=overview; stages=([^\n]+)/,
  );
  ...
  const roleHeadings = [...roleSection.matchAll(/^### (.+)$/gm)].map(
    (match) => match[1],
  );
  assert.deepEqual(
    roleHeadings,
    EXPECTED_PROCUREMENT_ROLE_HEADINGS,
    "procurement role headings must match exactly",
  );
  ...
  for (const role of EXPECTED_PROCUREMENT_ROLE_HEADINGS) {
    ...
    for (const field of EXPECTED_PROCUREMENT_ROLE_FIELDS) {
      ...
      const count = (
        procedure.match(new RegExp(`^- \\*\\*${escapedField}:\\*\\*`, "gm")) ??
        []
      ).length;
...
const AFFIRMATIVE_RELEASE_CLAIMS = Object.freeze([
  {
    ...
    pattern:
      /\b(?:live|uat)(?:\s*\/\s*(?:live|uat))?(?:\s+(?:testing|approval|pass|certification|completion|deployment|activation))?\s+(?:(?:is|are|was|were|has|have|had|been|now|already|successfully|fully|formally)\s+)*(?:approved|passed|certified|complete|completed|deployed|activated|successful|done|claimed)\b/i,
  },
  {
    name: "deployment/activation status",
    pattern:
      /\b(?:deployment|activation)\s+(?:(?:to|in|on)\s+(?:live|uat)\s+)?(?:(?:is|are|was|were|has|have|had|been|now|already|successfully|fully|formally)\s+)*(?:approved|passed|certified|complete|completed|successful|done)(?:\s+(?:in|to|on)\s+(?:live|uat))?\b/i,
  },
  {
    name: "migration-applied status",
    pattern:
      /\b(?:(?:live|uat)\s+)?migration\s+(?:(?:is|are|was|were|has|have|had|been|now|already|successfully|fully|formally)\s+)*applied(?:\s+(?:in|to|on)\s+(?:live|uat))?\b/i,
  },
]);
...
function assertNoAffirmativeReleaseClaims(markdown) {
  ...
  for (const sentence of sentences) {
    for (const claim of AFFIRMATIVE_RELEASE_CLAIMS) {
      ...
      assert.fail(
        `prohibited affirmative release claim (${claim.name}): ${sentence}`,
      );
    }
  }
}
...
function runtimeFunction(name, nextFunction, bindings = {}) {
  ...
  const source = match[0].slice(
    0,
    match[0].lastIndexOf(`function ${nextFunction}`),
  );
  return new Function(...Object.keys(bindings), `${source}; return ${name};`)(
...
function generatorFunction(name, nextFunction, bindings = {}) {
  ...
  assert.ok(
    match,
    `could not extract ${name} from the documentation generator`,
  );
  const source = match[0].slice(
    0,
    match[0].lastIndexOf(`function ${nextFunction}`),
...
function embeddedWindowValue(html, name) {
  const match = html.match(
    new RegExp(`window\\.${name} = ([\\s\\S]*?);(?: window\\.|</script>)`),
  );
...
function articleHtml(html, guideId) {
  const match = html.match(
    new RegExp(`<article[^>]+data-guide-id="${guideId}"[\\s\\S]*?</article>`),
  );
...
test("exports deterministic outcome-first generator phases", () => {
  ...
  const sources = documentationGenerator.loadDocumentationSources(
    documentationSources(),
  );
...
test("renders the four public modes and an outcome-first Home before metadata", () => {
  ...
  assert.doesNotMatch(
    html,
    /<button[^>]+data-mode-button[^>]*>(?:Start Here|Workflows|Roles &amp; Training|Architecture|Infrastructure|Security &amp; Governance|Release &amp; QA)<\/button>/,
  );

  const question = html.indexOf("Find the answer, then do the work.");
  ...
  const recentGuides = html.indexOf("Continue reading", systemEntry);
  ...
  assert.equal(
    html.slice(question, documentControls).includes("Source checksum"),
    false,
  );
  assert.match(
    html.slice(question, documentControls),
    /data-guide-id="stock-receiving-putaway"/,
  );
  ...
  assert.match(
    html,
    /<table tabindex="0"><thead><tr><th scope="col">Module<\/th>/,
  );
});
...
test("renders all canonical task, role, and System guides in contract order", () => {
  const html = buildDocumentationHtml();
  assert.equal(
    (html.match(/<article[^>]+data-guide-type="task"/g) ?? []).length,
    13,
  );
  assert.equal(
    (html.match(/<article[^>]+data-guide-type="role"/g) ?? []).length,
    11,
  );
  assert.equal(
    (html.match(/<article[^>]+data-guide-type="system"/g) ?? []).length,
    8,
  );

  const task = articleHtml(html, "procurement-request-approval");
  const taskSections = [...task.matchAll(/data-guide-section="([^"]+)"/g)].map(
    (match) => match[1],
  );
  assert.deepEqual(taskSections, TASK_SECTION_IDS);
  assert.match(
    task,
    /data-task-stage="step-1"[\s\S]*?General Employee[\s\S]*?\/procurement\/requests\/new/,
  );
  ...
  const roleSections = [...role.matchAll(/data-guide-section="([^"]+)"/g)].map(
    (match) => match[1],
  );
  assert.deepEqual(roleSections, ROLE_SECTION_IDS);
  assert.match(role, /Practice your primary workflow/);
  assert.match(role, /Practice a recovery scenario/);
  assert.match(role, /Training completion/);
});

test("renders substantive landing pages for every non-Home mode", () => {
  const html = buildDocumentationHtml();

  assert.equal(
    (html.match(/<section class="mode-landing" data-mode-landing/g) ?? [])
      .length,
    3,
  );
  assert.match(
    html,
    /data-mode-landing data-mode="tasks"[^>]*hidden[\s\S]*?What are you trying to complete\?/,
  );
  assert.match(
    html,
    /data-mode-landing data-mode="roles"[^>]*hidden[\s\S]*?What work are you responsible for\?/,
  );
  assert.match(
    html,
    /data-mode-landing data-mode="system"[^>]*hidden[\s\S]*?Operate and govern Mwell Intra/,
  );
  assert.match(
    html,
    /modeLandings\.forEach\(\(landing\) => \{ landing\.hidden = Boolean\(activeRoute\.query\)/,
  );
});

test("renders certified responsive evidence for every implemented task stage", () => {
  const html = buildDocumentationHtml();
  const taskStages = HANDBOOK_GUIDES.filter(
    ({ type }) => type === "task",
  ).flatMap(({ steps }) => steps);

  assert.equal(taskStages.length, 52);
  assert.equal(
    (html.match(/data-screen-evidence="certified"/g) ?? []).length,
    taskStages.length,
  );
  assert.equal(
    (html.match(/data-certified-screenshot/g) ?? []).length,
    taskStages.length,
  );
  assert.equal(
    (html.match(/data-screen-variant="desktop"/g) ?? []).length,
    taskStages.length,
  );
  assert.equal(
    (html.match(/data-screen-variant="mobile"/g) ?? []).length,
    taskStages.length,
  );
  assert.equal(
    (html.match(/class="screen-hotspot screen-hotspot-desktop"/g) ?? []).length,
    taskStages.length,
  );
  assert.equal(
    (html.match(/class="screen-hotspot screen-hotspot-mobile"/g) ?? []).length,
    taskStages.length,
  );
  assert.match(html, /Select Continue/);
...
test("collapses policy, source, capability, and document controls while preserving governed content", () => {
  ...
  assert.match(
    task,
    /<details[^>]+data-guide-section="policy-basis"(?![^>]*\bopen\b)/,
  );
  assert.match(
    task,
    /<details[^>]+data-guide-section="document-controls"(?![^>]*\bopen\b)/,
  );
  assert.match(
    role,
    /<details[^>]+data-guide-section="capability-codes-and-document-controls"(?![^>]*\bopen\b)/,
  );
  assert.equal(
  ...
  for (const source of documentationSources()) {
    assert.match(
      sourceLibrary,
      new RegExp(
        `data-source-file="${source.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`,
      ),
    );
  }
...
test("renders decisions at their true stage with explicit recovery and terminal destinations", () => {
  ...
  assert.ok(
    procurement.indexOf("S1 --&gt; D1") < procurement.indexOf("S3 --&gt; D3"),
  );
  ...
  assert.match(
    fulfillment,
    /D1 --&gt;\|Insufficient or held\| O_CONTROLLED_HOLD/,
  );
  for (const label of [
    "Owner",
    "Failed condition",
...
test("renders governed source bodies once and links every other source control to the library", () => {
  ...
  assert.equal(
    (html.match(/data-source-body/g) ?? []).length,
    sourceFiles.length,
  );
  assert.equal(
    (sourceLibrary.match(/data-source-body/g) ?? []).length,
    sourceFiles.length,
  );
  for (const source of sourceFiles) {
    ...
    assert.equal(
      (
        html.match(
          new RegExp(`data-source-body[^>]+data-source-file="${escaped}"`, "g"),
        ) ?? []
      ).length,
      1,
      source,
  ...
  for (const guide of HANDBOOK_GUIDES.filter(
    ({ id }) => id !== "source-references",
  )) {
    const rendered =
      guide.type === "home"
        ? (html.match(
            /<section class="hero home-guide"[\s\S]*?<\/section>\s*<article id="guide-/,
          )?.[0] ?? "")
  ...
  for (const linkTag of html.matchAll(
    /<a[^>]+data-canonical-source-link[^>]*>/g,
  )) {
    const targetId = tagAttributes(linkTag[0])["data-heading"];
    assert.ok(
      ids.has(targetId),
      `canonical source link targets addressable #${targetId}`,
    );
  }
});
...
test("gives disclosures and DOM IDs globally unique keys and renders every legacy target", () => {
  ...
  const disclosureKeys = [...html.matchAll(/data-section-id="([^"]+)"/g)].map(
    (match) => match[1],
  );
  ...
  assert.equal(
    new Set(ids).size,
    ids.length,
    "every DOM ID is globally unique",
  );
  assert.equal(
    new Set(disclosureKeys).size,
    disclosureKeys.length,
    "every disclosure state key is globally unique",
  );
  for (const route of LEGACY_ROUTES) {
    const targetId = `${route.guideId}-${route.headingId}`;
    assert.ok(
      ids.includes(targetId),
      `${route.legacyArticleId} targets addressable #${targetId}`,
    );
  }
});
...
test("embeds typed search records and every legacy route", () => {
  ...
  assert.ok(
    index.some(
      ({ type, guideId }) =>
        type === "Task" && guideId === "stock-receiving-putaway",
    ),
  );
  assert.ok(
    index.some(
      ({ type, guideId, headingId }) =>
        type === "Step" &&
  ...
  assert.ok(
    index.some(
      ({ type, guideId }) =>
        type === "Role" && guideId === "operations_associate",
    ),
  );
...
test("builds one self-contained handbook from every canonical source", () => {
  ...
  assert.match(html, /Search tasks, roles, controls, and policies/);
...
test("keeps one incorporated MPIC source-library reference with separated sections in required order", () => {
  ...
  assert.equal(
    (
      sourceLibrary.match(
        /data-source-file="docs\/policy\/MPIC_PROCUREMENT_POLICY_FEBRUARY_2025\.md"/g,
      ) ?? []
    ).length,
    1,
  );
  assert.equal(
    documentationSources().filter(
      (file) => file === "docs/policy/MPIC_PROCUREMENT_POLICY_FEBRUARY_2025.md",
    ).length,
    1,
  );
  ...
  assert.match(
    markdownSection(source, 2, "Direct requirements"),
    /Department Head[\s\S]*Group Controller/,
  );
  assert.match(
    markdownSection(source, 2, "Mwell mapping"),
    /Solicitation document[\s\S]*Procurement mode[\s\S]*Governance tier/,
  );
});
...
test("locks the exact procurement stage order and six decision-tree semantics", () => {
  ...
  assert.match(
    quorum,
    /current, independently approved pre-issue invitation-target exception/i,
  );
  assert.match(
    quorum,
...
test("blocks obsolete routing, certification, named variance authority, and working-day extension claims", () => {
  ...
  const byFile = Object.fromEntries(
    files.map((file) => [file, repositoryFile(file)]),
  );
  ...
  const extract =
    byFile["docs/policy/MPIC_PROCUREMENT_POLICY_FEBRUARY_2025.md"];
  ...
  assert.doesNotMatch(
    operating,
    /(?:recommendation variance|recommendation differs|differing recommendation)[\s\S]{0,260}(?:Department Head|Group Controller|Finance Controller|independent Controller)/i,
  );
  ...
  assert.match(
    conflicts,
    /current Mwell code[\s\S]*`dept_head`[\s\S]*`finance`[\s\S]*unresolved/i,
  );
  assert.match(conflicts, /seven calendar days/i);
  assert.match(extract, /sourceDocumentStatus[\s\S]*updated_visual_draft/i);
  assert.doesNotMatch(
    markdownSection(extract, 2, "Active profile"),
    /inherits[^\n]*all[^\n]*MPIC controls/i,
  );
  assert.match(
    traceability,
    /Neutral recommendation-variance decisions[\s\S]*Unresolved local authority mapping/i,
  );
...
test("rejects representative Task 11 structural and certification mutations", () => {
  ...
  assert.throws(
    () => assertTask11Structure(alteredStage, manual),
    /overview stage/i,
  );
  ...
  assert.throws(
    () => assertTask11Structure(duplicateTree, manual),
    /operating extract headings/i,
  );
  ...
  assert.throws(
    () => assertTask11Structure(process, duplicateRole),
    /role headings/i,
  );
  ...
  assert.throws(
    () => assertTask11Structure(process, duplicateField),
    /Requester Start condition/i,
  );
  ...
  for (const affirmativeClaim of [
    ...
    assert.throws(
      () => assertNoAffirmativeReleaseClaims(`${process}\n${affirmativeClaim}`),
      /affirmative release claim/i,
    );
  }

  assert.doesNotThrow(() =>
    assertNoAffirmativeReleaseClaims(
      [
...
test("embeds local presentation assets without external requests", () => {
  ...
  assert.match(
    html,
    /class="brand"[^>]+aria-label="Mwell Intra operating handbook home"/,
  );
  assert.match(html, /class="brand"[\s\S]*?<img src="data:image\/png;base64,/);
  assert.match(
    html,
    /<span class="brand-copy"><strong>Intra<\/strong><small>Operating handbook<\/small><\/span>/,
  );
  assert.doesNotMatch(html, /<script\s+src=/i);
  assert.doesNotMatch(html, /<link\s+[^>]*rel=["']stylesheet/i);
  assert.doesNotMatch(html, /cdn\.prod\.website-files\.com/i);
});
...
test("renders four accessible mode tabs and every canonical guide once", () => {
  ...
  const controls = [
    ...html.matchAll(/<button[^>]+data-mode-button[^>]*>/g),
  ].map((match) => tagAttributes(match[0]));
  for (const control of controls) {
    const panelTag = html.match(
      new RegExp(`<[^>]+id="${control["aria-controls"]}"[^>]*>`),
    )?.[0];
    ...
    assert.equal(
      ...
      control.id,
      `${control.id} is the panel label`,
    );
  }
  assert.equal(
    (html.match(/data-guide(?:\s|>)/g) ?? []).length,
    HANDBOOK_GUIDES.length,
  );
});
...
test("renders canonical guide navigation and in-place route support", () => {
  ...
  assert.match(html, /matchMedia\('\(min-width: 1536px\)'\)/);
});
...
test("renders progressive disclosures with stable state keys", () => {
  ...
  assert.match(
    html,
    /<details[^>]+data-section-id="procurement-request-approval:policy-basis"/,
  );
  assert.match(
    html,
    /<details[^>]+data-section-id="procurement-request-approval:document-controls"/,
  );
  assert.match(html, /<summary\b/);
  assert.match(html, /mwell-intra-handbook:v3/);
  assert.doesNotMatch(
    html,
    /data-section-id="procurement-request-approval:(?:policy-basis|document-controls)"[^>]+open/,
  );
});
...
test("embeds guide-level typed search records with match metadata", () => {
  ...
  assert.ok(
    index.some(
      (record) =>
        record.type === "Step" &&
        /create and validate the request/i.test(record.searchText),
    ),
    "structured task stages remain searchable without exposing source fences",
  );
...
test("indexes operational language against canonical task and role destinations", () => {
  ...
  const model = documentationGenerator.resolveHandbookModel(sources);
  const index = documentationGenerator.buildGuideSearchIndex(
    documentationGenerator.composeHandbookGuides(model),
  );
  const searchableText = (record) =>
    normalizeWhitespace(
      [record.title, record.heading, record.keywords, record.searchText]
        .flat()
        .join(" "),
    ).toLowerCase();
  const cases = [
  ...
  for (const [query, guideId] of cases) {
    const match = index.find(
      (record) =>
        ...
        searchableText(record).includes(query.toLowerCase()),
    );
    assert.ok(match, `${query} must index an operational answer in ${guideId}`);
    assert.match(
      match.href,
      new RegExp(`^#mode=(?:tasks|roles)&guide=${guideId}&heading=`),
    );
    assert.ok(
      match.role || match.module,
      `${query} must expose role or module context`,
    );
    assert.ok(
      match.excerpt.length > 20,
      `${query} must expose an actionable excerpt`,
    );
  }
});
...
test("publishes the maintained governance search intent vocabulary", () => {
  ...
  const html = buildDocumentationHtml();
  assert.deepEqual(
    embeddedWindowValue(html, "__HANDBOOK_SYSTEM_INTENTS__"),
    requiredIntents,
  );
});
...
test("renders maintained flow views before workflow prose", () => {
  ...
  for (const workflow of workflows) {
    const group = html.match(
      new RegExp(
        `<section class="diagram-group"[^>]+data-workflow-id="${workflow}"[\\s\\S]*?</section>`,
      ),
    );
    assert.ok(group, `renders ${workflow} as a maintained diagram group`);
    for (const view of ["overview", "role", "decision"])
      assert.match(group[0], new RegExp(`data-diagram-view="${view}"`));
    assert.match(
      group[0],
      /--&gt;\|[^|]+\|/,
      `${workflow} includes labeled decision edges`,
    );
    const nextGroup = html.indexOf(
      '<section class="diagram-group"',
      group.index + group[0].length,
    );
    const completion = html.indexOf(
      "<strong>Completion criteria:</strong>",
      group.index + group[0].length,
    );
    assert.ok(
      completion > group.index && (nextGroup < 0 || completion < nextGroup),
      `${workflow} keeps completion criteria after its diagram group`,
    );
  }
...
test("opens task decision maps at readable scale rather than shrinking the complete tree", () => {
  const html = buildDocumentationHtml();

  assert.equal(
    (html.match(/shell\.closest\('\.full-decision-map'\) \? 1 : 'fit'/g) ?? [])
      .length,
    2,
  );
  assert.match(
    articleHtml(html, "procurement-request-approval"),
    /<details class="full-decision-map">/,
  );
});

test("normalizes v3 per-guide state without accepting malformed values", () => {
  const normalizeStoredState = runtimeFunction(
  ...
  assert.deepEqual(
    normalizeStoredState({
      activeRoute: {
        modeId: "system",
        guideId: "technical-architecture",
        headingId: "overview",
        query: "  tenant  ",
        scope: "mode",
      },
      ...
      disclosures: {
        "technical-architecture": [
          "technical-architecture:runtime-architecture",
          4,
        ],
        broken: "open",
      },
      diagramViews: {
        "doc-technical-and-functional-specification-md:diagram-1": {
          left: 18,
          top: 32,
        },
      },
      diagramZoom: {
        "doc-technical-and-functional-specification-md:diagram-1": 1.4,
        fit: "fit",
        broken: "big",
      },
      diagramModes: {
        "procurement-to-payment": "decision",
        broken: "sideways",
      },
      theme: "dark",
    }),
  ...
  const migrated = migrateV2State(
    {
      ...
      scope: "tab",
      expandedIds: [
        "procurement-request-approval:policy-basis",
        "other-guide:document-controls",
      ],
...
test("does not restore an unmapped compatibility article during v2 migration", () => {
  ...
  assert.deepEqual(migrated.activeRoute, {
    modeId: "home",
    guideId: "home",
    headingId: null,
    query: "stock",
...
test("keeps semantic diagram state IDs stable when diagrams are inserted or reordered", () => {
  const decorateArticleHtml = generatorFunction(
    "decorateArticleHtml",
    "markdownHeadingEntries",
    {
  ...
  const overview =
    '<figure class="diagram-shell" data-flow-workflow="procurement-to-payment" data-flow-view="overview" data-flow-stages="Request|Pay"><div class="mermaid">flowchart TD\nA[Request] --&gt; B[Pay]</div></figure>';
  const role =
    '<figure class="diagram-shell" data-flow-workflow="procurement-to-payment" data-flow-view="role" data-flow-stages="Request|Pay"><div class="mermaid">flowchart TD\nA[Requester] --&gt; B[Finance]</div></figure>';
  const decision =
    '<figure class="diagram-shell" data-flow-workflow="procurement-to-payment" data-flow-view="decision" data-flow-stages="Request|Pay"><div class="mermaid">flowchart TD\nA{Approved?} --&gt;|Yes| B[Pay]</div></figure>';
  const fallback =
    '<figure class="diagram-shell"><div class="mermaid">flowchart TD\nA[Stable fallback] --&gt; B[Done]</div></figure>';
  const inserted =
    '<figure class="diagram-shell"><div class="mermaid">flowchart TD\nX[Inserted] --&gt; Y[Other]</div></figure>';
  const original = decorateArticleHtml(
    document,
    `${overview}${role}${decision}${fallback}`,
  );
  const reordered = decorateArticleHtml(
    document,
    `${inserted}${fallback}${overview}${role}${decision}`,
  );
  const idFor = (html, marker) =>
    html.match(
      new RegExp(
        `data-diagram-id="([^"]+)"[^>]*>(?:(?!<figure)[\\s\\S])*?${marker}`,
      ),
    )?.[1];

  assert.equal(
    idFor(original, "Request] --&gt; B\\[Pay"),
    idFor(reordered, "Request] --&gt; B\\[Pay"),
  );
  assert.equal(
    idFor(original, "Stable fallback"),
    idFor(reordered, "Stable fallback"),
  );
  assert.match(
    original,
    /data-diagram-id="doc-example:flow:procurement-to-payment:overview"/,
  );
});
...
test("restores a saved per-guide position through the document viewport", () => {
  ...
  const withoutSmoothScroll = runtimeFunction(
    "withoutSmoothScroll",
    "visibleStickyChromeBottom",
    { document },
  );
  const restoreStoredPosition = runtimeFunction(
...
test("serializes and parses only canonical handbook routes", () => {
  ...
  assert.equal(
    hash,
    "#mode=tasks&guide=procurement-request-approval&heading=steps&q=three-way+match&scope=mode",
  );
  ...
  const defaultScopeHash = routeHash({
    modeId: "home",
    guideId: "home",
    headingId: null,
    query: "",
...
test("stores query and scope only inside the five-field active route", () => {
  const currentStoredState = runtimeFunction(
    "currentStoredState",
    "schedulePersistence",
    {
      activeRoute: {
...
test("translates an exact legacy route but does not guess an unknown legacy target", () => {
  ...
  assert.deepEqual(
    parseRoute(
      "#tab=workflows&article=doc-manual-mwell-intra-user-manual-md&heading=doc-manual-mwell-intra-user-manual-md-warehouse-flow",
    ),
    {
      modeId: "tasks",
      guideId: "stock-receiving-putaway",
...
test("renders canonical guide hashes while preserving compatibility data hooks", () => {
  const html = buildDocumentationHtml();
  const match = html.match(
    /href="(#mode=tasks&amp;guide=stock-receiving-putaway)"[^>]+data-guide-link[^>]+data-mode="tasks"[^>]+data-tab="workflows"[^>]+data-article="guide-stock-receiving-putaway"/,
  );
  assert.ok(match);
