...
function launchBlockerMigration() {
  ...
  assert.equal(matches.length, 1, "expected one forward-only Operations migration");
...
test("separates receipt capture from inspection and exception creation from closure", () => {
  ...
  assert.match(sql, /before update of disposition on warehouse\.quality_inspections/i);
  assert.match(
    sql,
    /function warehouse\.receive_procurement_po\(payload jsonb\)[\s\S]*set quality_status = 'pending'/i,
  );
  assert.match(sql, /defer_independent_receipt_inspection[\s\S]*'on'[\s\S]*warehouse_receive_procurement_po[\s\S]*'off'/i);
...
test("creates and submits an evidenced cycle count in one idempotent transaction", () => {
  ...
  assert.match(sql, /function warehouse\.create_and_submit_cycle_count\(payload jsonb\)/i);
  assert.match(sql, /core\.has_live_cap\('warehouse',\s*'cycle_count'\)/i);
  assert.match(sql, /private\.begin_idempotent_command\([\s\S]*create_and_submit_cycle_count/i);
  ...
  assert.match(sql, /grant execute on function warehouse\.create_and_submit_cycle_count\(jsonb\)/i);
});
