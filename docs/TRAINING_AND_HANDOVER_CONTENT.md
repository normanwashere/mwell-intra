...
The event seller drill uses a clearly labelled UAT test account with only the seller role and optional staff read baseline, never a shared operational account. Record the actual automated-review provenance for test training publication without claiming human review; the learner still completes all six governed checkpoints through the application. After approved release setup, demonstrate owner enablement before issue, time-limited assignment, multi-line demand, approval, release, acknowledgment, own sales/giveaways, duplicate prevention, reversal, unsold return, independent inspection, Product-approved recovery and Finance readback. Do not substitute a simulated screenshot for a saved live transaction. The September 20 candidate guide lists the remaining release gates. SMTP remains excluded.
