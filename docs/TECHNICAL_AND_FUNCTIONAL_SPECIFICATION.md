...
The sourcing workspace first verifies the request-bound sourcing read. An explicit `event: null` is a valid not-started state and does not trigger event-specific exception/evaluation reads. Invalid or denied reads show retry guidance instead of editable stale state. Existing-event evaluation authority and all sourcing mutations remain unchanged; later-stage sourcing certification is separate.
