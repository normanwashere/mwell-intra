...
The UAT web build is `9ec7311`; the vendor upload database correction `20260920164950_vendor_document_upload_authorization` is now installed. It removes the incorrect Legal-review prerequisite for a current vendor uploading to their own case, while checking the vendor and requirement bindings. Local authorization regressions pass; browser Storage, final submission and Legal handoff need fresh hosted evidence. Uploading a document does not submit or approve the application. These follow-up handbook/KB edits and audit fixes remain local until the next deployment is recorded.

CI #202 finished with desktop 1440, desktop 1280 and tablet route checks passing. Mobile route probes and both transaction jobs failed; both independent cleanup jobs passed. The focused corrections address mobile scroll-settling measurement, the Events Product 1 selector and the real vendor-upload authorization defect. Do not count skipped or failed checks as passed, and do not call the mobile probe correction a redesign.

The separate September 21 synthetic event is `uat-evt-20260921-13ec2166-93be-4902-b323-bacc857dad5d-event`, using the existing named test seller. Marketing requested three watches and two materials; a different Warehouse lead approved request/order `c5ce7cc6-6352-4d4c-af13-2bbee8ad5d19`. Both lines remain intact and no stock, serials or allocations were created by setup or approval. The full replenishment-to-seller-to-Finance chain remains open. September 20 records and existing tester stock are unchanged. SMTP remains excluded.
...
The event cannot be cancelled while released stock still awaits acknowledgment, and settlement cannot replace missing stock with invented return or loss quantities. Complete physical handovers and recorded returns first. Submitted or approved settlement prevents new demand from changing its reconciled balance. Reusable event watch variants stay active; Product decides when to retire them. Inspection and an approved Product recipe remain required to recover a returned watch to its base product; this does not retire the variant SKU or remove its history. Post-event corrections remain undecided; the candidate keeps existing correction restrictions and strict assignment dates.
