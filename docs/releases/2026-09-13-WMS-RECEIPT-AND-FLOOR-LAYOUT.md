...
Status: UAT receipt-upload migration applied; the latest application and KB are deployed as `63a52fc42d2bdaa2c219640a0b580a5970be9387`, replacing `75e733b`. Deployment `dpl_JDBT8w9tMZGMJ7WUZASoET1rFNhe` is ready and its public health response confirms the exact build and UAT database. Six new desktop/mobile smoke cases and 32 fresh ecommerce checkpoints passed. Full warehouse certification remains open.

Released follow-up: Packing confirmation now uses row/filter feedback, the pick heading keeps its full reference in an expandable section, the Quality reminder is collapsible, and new validation errors scroll into view without changing focus or scanned data. Independent review found no blockers. This change passed 158 fulfillment tests, 30 local browser cases across six widths and 96 KB checks. The exact changed packing and wrong-bin screens were then reviewed at 1440px and 390px on live UAT. No additional database migration was required.
...
Marketing, Operations and Admin passed the new read-only checks at 1440px and 390px with zero recorded page/server errors (`outputs/sep13-pack-pick-release-smoke`). Fresh run `fbceedf7-9ffd-49dd-be36-f1d7ff4f66f3` completed two uninterrupted ecommerce journeys on `63a52fc`; independent database readback confirms correct stock and private POD evidence. Ten supplemental proof/timeline screenshots were captured without page errors or business writes; selected desktop/mobile images were visually reviewed. Earlier department and ecommerce evidence remains tied to its original builds. Other ecommerce branches, receiving/putaway, returns, role coverage, concurrency, real hardware and actual participant acceptance remain open. SMTP is excluded. See the [live readout](../audits/2026-09-13-WMS-LIVE-READOUT.md) and [pilot pack](../audits/2026-09-13-WMS-PILOT-PACK.md).
