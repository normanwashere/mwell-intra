...
**Fresh-account certification driver repair:** Run #172 stopped before the route and transaction matrix because the driver missed receiving controls and the required pause/resume checkpoint, skipped answers while the server was still evaluating a prior choice, and lacked the current Marketing assessment answers. The driver now waits for confirmed step changes, operates the receiving practice controls using the UAT product selection, and accepts an auto-closed assessment only when its exact requirement reads Complete. Browser regressions keep missing transitions and unknown assessments failing. App permissions, training requirements and business workflows are unchanged; a replacement full CI run is still required.

**Repair validation:** The isolated critical accounts now pass 11/11 mobile onboarding completion checks and 11/11 fresh-session desktop readbacks on UAT `911d8b8`, with role screenshots saved under `outputs/sep12-performance/ci172-repair`. These are completion and recovery checks on the repaired accounts, not 22 pristine first-time journeys. The driver regression suite passes 119 tests with one existing skip. New viewport identities and the full transaction/visual matrix still require the replacement CI run.
