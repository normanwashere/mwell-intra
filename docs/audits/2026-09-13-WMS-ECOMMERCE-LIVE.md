...
Assistant visual review of both `wrong-bin-denied` and `packed-independent-release-required` captures confirms the full error below the compact header, a visible scan action and non-overlay packing guidance. Supplemental `evidence-review-2026-09-13T04-44-14-128Z` captured ten images without page errors or business writes; selected proof and timeline captures show the full synthetic image opening in a lightbox and the failure/retry/delivery history remaining readable. This is not a human floor pilot or certification of every control. The two objects and scoped live fixtures have now been removed after archive verification; the historical test evidence remains preserved.
...
The earlier `55b66a58` ecommerce batch remains at Received and still needs scoped cleanup. Testers should leave WMS-ECOM fixtures untouched. Existing tester data is outside this scope.

The latest completed batch `fbceedf7` is now cleaned up. Its two photos were archived and removed through the Storage API, with one successful DELETE per object. Immediate cached reads were not accepted as deletion proof. Separate Storage readback confirmed absence at `2026-09-13T06:11:03.876Z`; the scoped database cleanup finished at `06:13:55.843Z`, followed by an independent zero-row check at `06:14:51.416Z`. Evidence under the recorded attempt: `ecommerce-storage-cleanup/storage-verification.json`, `cleanup-db-apply-01.execution.json`, and `cleanup-db-independent-postcleanup-01.report.json`. The API receipt SHA-256 is `20f74c19d7d088f58a137befec0605b990c364267fc22aef3469a0a3007af4f1`. Parent read-only checks also confirmed the exact two orders, products and delivery objects were absent. This does not claim global UAT cleanup.

The older completed batch `acd6711b` is now cleaned up. Both actual photos were downloaded, hash-checked and archived before Storage API deletion. Independent preflight scanned 186 application relations with no foreign references found. The saved API receipt is `attempt-2026-09-13T04-08-52-389Z-aca4e68c/ecommerce-storage-cleanup/storage-verification.json` under `outputs/wms-signoff/sep13-ecommerce-governed-bootstrap`. Separate Storage readback at `2026-09-13T05:52:16.680Z` found no remaining objects. The guarded database transaction then removed the exact batch records; `cleanup-execution-result.json` and the separate `independent-postcleanup.json` confirm zero scoped rows across ten target tables and zero Storage objects. This is scoped cleanup, not a claim that all UAT residue or remote/encoded references were inspected.

Cleanup tooling was corrected for actual catalog coverage, the exact received create-order cache's JSON-null address, and Supabase's `400 / NoSuchKey / 404` missing-object response. Recovery now validates the saved successful removal, administrative principal, chronology, archive hash and archived preflight before contacting the backend. Independent verification passed 76 Storage executor tests, including 12 invalid-proof cases with zero resumed backend calls. The earlier full combined run passed 159 tests before that additional recovery safeguard. One immediate post-delete read still served a cached copy; it was not accepted as absence. After cache invalidation, the same authenticated read returned the missing-object response. Historical failed attempts remain recorded; no successful deletion was repeated and no authorization or business workflow changed.

A separate clean-bulk receiving/independent inspection/putaway journey has completed on the same build; see [Inbound live evidence](2026-09-13-WMS-INBOUND-LIVE.md) for its continuation and review limits. Physical return disposition, customer return resolution and replacement delivery, other stock types, receiving exceptions, partial/cancel cases, all isolated roles, concurrency, wider recovery, hardware and named-user pilot acceptance remain open. Historical scripts and rollback return probes do not certify those requirements on this build.
