...
| P16 | Operations recommends replenishment for the prepared product. The Procurement participant accepts it and creates the linked draft request; use a different prepared recommendation to test dismissal. | Operations can recommend without viewing supplier/spend details, but cannot make a Procurement decision without its separate permission. Accepted or handed-off details cannot be silently rewritten. Handoff creates one linked draft, not an order or stock movement. Mark this scenario blocked until the recommendation entry and database correction are released and retested. |
