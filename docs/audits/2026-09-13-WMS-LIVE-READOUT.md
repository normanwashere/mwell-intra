...
**Latest ecommerce result:** Run `acd6711b-6480-4bf9-83f0-896c5174974f` completed desktop and mobile shipment delivery on build `75e733b`. The final attempt passed 24 recorded checks with no failures, linked to eight earlier desktop checks by hashes; desktop resumed at a verified released boundary after a test-only actor-label mismatch, while mobile ran continuously from intake. Independent readback confirms two completed/delivered orders, eight units remaining in each isolated bin, one two-unit movement and one delivery audit per order, and one private POD object per order. No human delivery or acceptance is claimed. Details and open UI findings: [Ecommerce live evidence](2026-09-13-WMS-ECOMMERCE-LIVE.md). This supersedes the older ecommerce-pending notes below, not their historical evidence. The two ecommerce batches still require scoped cleanup.
