...
An additive executor is now implemented and independently reviewed offline, with 71 tests passing and no skips. Three review findings were fixed: approval expiry at the write boundary, explicit ordinary Auth-role validation, and tests depending on ignored audit files. See the [executor review](2026-09-13-WMS-ROLE-EXECUTOR-REVIEW.md) for exact scope and remaining transport/coordination checks. No live execution approval or new accounts exist yet; a complete WMS role-matrix runner remains open. **Do not run `node scripts/qa/provision-uat-intra-test-users.mjs` unchanged for this plan:** its CLI reconciles the selected existing roster, can delete mismatched roles/scopes, and can retire obsolete identities. Existing journey runners also hardcode combined job personas; do not count their runs as isolated-role evidence.
