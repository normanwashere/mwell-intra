...
begin
  ...
  if not exists(select 1 from core.role_capabilities where module='procurement' and role='admin' and cap='review_payment_readiness')
  ...
  select cv.* into strict b from learning.curriculum_versions cv join learning.curricula root on root.id=cv.curriculum_id
    where root.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and root.audience='internal' and root.governance_owner='platform' and root.status='active'
  ...
  if (select md5(jsonb_build_object('curriculum',to_jsonb(c),'version',to_jsonb(cv),
    ...
    where c.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and cv.version=1) is distinct from '0a4376c84b5a76f0823ffa177edf54ce' then raise exception 'Reviewed baseline drift'; end if;
  ...
  select rv.* into strict old_r from learning.requirement_versions rv join learning.requirements root on root.id=rv.requirement_id
    ...
    where root.requirement_key='internal.role.procurement.admin.capability-practice.v1' and rv.version=1 and rv.status='published' and rv.requirement_kind='scenario'
    and rv.effective_at<=statement_timestamp() and (rv.expires_at is null or rv.expires_at>statement_timestamp()) for share of rv;
  if exists(select 1 from learning.requirements r where r.requirement_key='internal.role.procurement.admin.payment-readiness.v1')
  ...
  new_root := jsonb_populate_record(new_root,jsonb_build_object('id',gen_random_uuid(),'requirement_key','internal.role.procurement.admin.payment-readiness.v1',
  ...
  new_r := jsonb_populate_record(old_r,jsonb_build_object('id',gen_random_uuid(),'requirement_id',new_root.id,'version',1,'status','draft',
    ...
    'source_references',old_r.source_references||jsonb_build_array('{"type":"draft_mapping_content","candidate":"06c9b80bc6c09c343800756ebcadc0efdb88619f","review_reference":"docs/audits/2026-09-07-publication-bindings/admin.review.json","reviewed_at":"2026-09-07T03:31:51Z","baseline_fingerprint":"0a4376c84b5a76f0823ffa177edf54ce","simulation_id":"procurement-payment-readiness-review-v1","content_sha256":"901e4ee97aec872dd7efd4cc2d7975c59f15dfdedd8a29f986201d3072c6ac2b","source":"modules/learning/src/scopedReadinessCandidates.ts","rules_sha256":"deeb1dff8e5fa2998e59655bb66511a78a20becb5e01f8fafaaa8fd631f65a98","runtime_status":"registered_in_exact_reviewed_candidate","registered_runtime_candidate":"06c9b80bc6c09c343800756ebcadc0efdb88619f","publication_state":"reviewed_publication_inactive_no_role_mapping","review_mode":"automated","author_agent_id":"01a06fa0-d12a-73d0-a143-5e6c7f3a745e","reviewer_agent_id":"01a06fa2-a909-7bd0-84a1-90c820ec9c5f","review_artifact_sha256":"64f6752581be5f44df17fac687f0f39f96cb9c6ebe6f4c1614bbef252649c6cb","user_authorization_reference":"2026-09-07 current task explicit automated independent review and per-key publication binding request; no human signoff","custodian_attribution":"Existing UAT test profiles are audit custodians only; neither performed human review.","human_review_claimed":false}'::jsonb),
  ...
  if (select md5(jsonb_build_object('curriculum',to_jsonb(c),'version',to_jsonb(cv),
    ...
    where c.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and cv.version=1) is distinct from '0a4376c84b5a76f0823ffa177edf54ce' then raise exception 'Baseline changed'; end if;
...
select cv.status,rv.version as requirement_version,rv.simulation_id,rv.pass_rules,rv.source_references,
  ...
  join learning.requirement_versions rv on rv.id=cr.requirement_version_id
  where c.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and cv.version=2 and rv.requirement_id in
    (select id from learning.requirements where requirement_key='internal.role.procurement.admin.payment-readiness.v1');
rollback;
