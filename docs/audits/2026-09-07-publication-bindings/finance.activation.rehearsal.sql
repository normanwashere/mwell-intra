...
begin
  select cv.id into strict source_id from learning.curriculum_versions cv join learning.curricula c on c.id=cv.curriculum_id
    where c.catalog_key='internal.role.procurement.finance.capability-practice.v1.curriculum' and cv.version=1 and cv.status='published';
  select cv.id into strict target_id from learning.curriculum_versions cv join learning.curricula c on c.id=cv.curriculum_id
    where c.catalog_key='internal.role.procurement.finance.capability-practice.v1.curriculum' and cv.version=2 and cv.status='published' and cv.audience='internal'
  ...
  if (select md5(jsonb_build_object('curriculum',to_jsonb(c),'version',to_jsonb(cv),
    ...
    where c.catalog_key='internal.role.procurement.finance.capability-practice.v1.curriculum' and cv.version=1) is distinct from 'deb60d391e7857938b0edc12ac346a71' or
    ...
    where c.catalog_key='internal.role.procurement.finance.capability-practice.v1.curriculum' and cv.version=2) is distinct from '6581abf370e11dae569816bc977b2225' then raise exception 'Activation baseline drift'; end if;
  ...
  insert into learning.role_curricula(module,role,curriculum_version_id,audience,department_id,effective_at,expires_at,created_by)
    ...
    from learning.role_curricula rc where rc.curriculum_version_id=source_id and rc.module='procurement' and rc.role='finance'
