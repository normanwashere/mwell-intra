...
begin
  select cv.id into strict source_id from learning.curriculum_versions cv join learning.curricula c on c.id=cv.curriculum_id
    where c.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and cv.version=1 and cv.status='published';
  select cv.id into strict target_id from learning.curriculum_versions cv join learning.curricula c on c.id=cv.curriculum_id
    where c.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and cv.version=2 and cv.status='published' and cv.audience='internal'
  ...
  if (select md5(jsonb_build_object('curriculum',to_jsonb(c),'version',to_jsonb(cv),
    ...
    where c.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and cv.version=1) is distinct from '0a4376c84b5a76f0823ffa177edf54ce' or
    ...
    where c.catalog_key='internal.role.procurement.admin.capability-practice.v1.curriculum' and cv.version=2) is distinct from 'abc130ab2d04227046c03d7b12b0fe3f' then raise exception 'Activation baseline drift'; end if;
  ...
  insert into learning.role_curricula(module,role,curriculum_version_id,audience,department_id,effective_at,expires_at,created_by)
    ...
    from learning.role_curricula rc where rc.curriculum_version_id=source_id and rc.module='procurement' and rc.role='admin'
