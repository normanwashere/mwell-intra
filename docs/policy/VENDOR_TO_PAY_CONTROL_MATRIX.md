...
**Activation is blocked:** current code maps MPIC's named recommendation-variance stages to local code roles without recorded Mwell authorization, and it changes the source's seven-calendar-day extension cap to seven working days. Operating controls therefore use neutral first/second independent variance decisions and the seven-calendar-day source cap until owners authorize a local mapping or the code is corrected. The unapplied migration must remain unapplied.
...
| Sourcing and evaluation | Three to four accredited vendors invited; equal package/deadline/clarifications; at least three usable sealed-bid responses or governed failed-bid recovery; technical and commercial evaluation; understandable best-value recommendation | Procurement plus technical reviewer | Versioned RFQ/RFP package, invitation delivery, current independently approved pre-issue invitation-target exception when needed, acknowledgment/clarification timers, response quorum, failed-bid/requote/insufficient-bids paths, tabulation, technical evaluation, and no automatic winner. Do not activate the unapproved working-day extension unit | Invitations or pre-issue exception evidence, communications, responses, failed-bid decision when applicable, tabulation, technical decision, award recommendation |
| Award variance | MPIC directly names written Requestor justification, Department Head approval, then Group Controller decision | Local decision-stage authorization is unresolved | Require written justification plus neutral first and second independent variance decisions. Block use until Mwell policy/DOA owners authorize the stages and effective assignments; current code labels are implementation facts only | Justification, two independently attributable authorized decisions, approval source, effective DOA, and immutable timestamps |
