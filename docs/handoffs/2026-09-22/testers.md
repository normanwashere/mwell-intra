...
Open [live UAT](https://mwell-intra-uat.vercel.app/), use your assigned test account, and work only on records allocated to your test run. **This is UAT, not production. The release is available for controlled testing but is not fully certified.**

The current changes are live on UAT. We confirmed the assigned final DOA approval through the app and checked the saved result. The wider CI206 run is tracked in Release Status. Confirm your session's build with the coordinator, and do not switch to a more powerful account to get around a blocker.
...
**Mobile issues still on this live build:** long approval emails and attachment details can be clipped on a narrow request page. A Department hierarchy target also failed the 390px accessibility check. Both fixes passed local browser checks, but are not live yet. Record the affected page and width instead of treating horizontal clipping or an undersized control as expected behavior.
