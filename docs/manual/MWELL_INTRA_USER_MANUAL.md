...
**Plain-language error update, local candidate:** Error messages explain what happened and the next step, without database terms. A PO item awaiting a receiving decision needs an independent Warehouse Supervisor in **Receive and inspect > Controlled receipt decisions**; do not receive the same units again. Stock **waiting for inspection** is not automatically damaged: an authorized person other than the receiver must inspect it in **Quality Control > Pending**. For other holds, ask the warehouse supervisor to review the reason. Remove a held unit from the move selection to move other eligible units; never remove the hold just to continue. A connection error does not prove nothing was saved: reopen the record and check its history before trying again. This wording update has not yet been deployed.
