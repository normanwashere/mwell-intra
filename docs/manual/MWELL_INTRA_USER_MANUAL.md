...
UAT serves application commit `9989a0d20fd6126f4ce61ebe5c9e38b9bda0feb1`. Both scoped DOA migrations are installed. The assigned Operations Lead completed the existing synthetic PHP340 request's final approval through the live UI; both earlier decisions stayed unchanged, and an unassigned Procurement Lead was denied eligibility. No role, active-policy or SMTP setting changed. This is automated UAT evidence, not human approval or production acceptance. Use the dated team handoff Release Status for full CI206 and the remaining seller-to-Finance journey. Earlier dated candidate notes below retain their historical evidence limits.

## September 21 Follow-Up

**September 22 approval follow-up (live UAT `9989a0d`):** Final approval follows the effective Delegation of Authority (DOA), not a requirement to hold the Procurement administrator role. The named pending approver must be an active employee with a current Procurement approval permission and the required training. Exactly one current matrix and one matching assignment must cover the request's department, category and amount, including the amount-band endpoints; the pending step must retain the matching matrix version. A name on a draft, training alone or administrator access does not grant authority, and requesters cannot approve their own requests. Ask Legal or the policy administrator to review a mismatch; do not grant extra roles or repeat completed training to force approval. The earlier inactive Procurement Lead draft is not recommended or activated. This candidate changes no recorded decisions, account roles, active policy or SMTP settings. Deployment and a targeted final-DOA live UI retest passed; full CI206 and downstream journey completion remain separate.
...
**Vendor submission candidate:** Complete the declarations and capture your authorized signature, then select **Sign and submit**. The candidate fixes a case where the button stayed unavailable even after signing because the signing date was not included in the readiness check. **Saved securely** confirms the draft, not submission. Wait for the submitted confirmation and check the case status; if it stays a draft, report the case reference. This fix still needs deployment and live retesting.
