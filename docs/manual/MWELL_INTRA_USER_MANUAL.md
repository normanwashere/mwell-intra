...
**Candidate layout update, pending UAT deployment:** Allocation and start-picking confirmations appear beside the affected order instead of covering the scan form. Receipt capture opens as a single work surface; closing it returns you to the request. Expand **Order reference** to read or copy the full reference when the heading is shortened. Required evidence, approval, release and receipt steps are unchanged.
