...
In Onboarding, read the module and role underneath each certification capability. Similar capability names can belong to different authorities; they are not automatically duplicate permissions. Active and historical entries keep their own status and validity. If **Role context unavailable** appears, ask the role owner to review the historical curriculum; the label does not create or remove access.
