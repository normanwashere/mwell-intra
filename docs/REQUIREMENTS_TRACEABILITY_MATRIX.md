...
| Invite three to four accredited vendors; sealed bids require at least three usable responses or governed failed-bid recovery | Direct MPIC requirement with local recovery mapping | Versioned package, invitation/communication evidence, quorum blocker, failed-bid state, equal-notice extension/requote, insufficient-bids decision | Task 5 sourcing tests and decision-tree review; live transaction evidence remains Task 12 work |
...
| Differing recommendation requires written justification, Department Head approval, and independent Controller decision | Direct MPIC requirement; Mwell authority resolved only through effective DOA | Versioned variance rationale and independent decision path without MPIC-name substitution | Task 6 variance tests and handbook flow; active named authority remains DOA-controlled |
