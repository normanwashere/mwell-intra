...
The onboarding task chooser starts closed. Your progress and next required action come first; use the compact Start, Resume or Try again button beside its title. Expand Choose a task or Change task to browse and search the full eligible list using the page scroll. The return-to-work link remains available, but does not bypass prerequisites, permissions or certification requirements.
