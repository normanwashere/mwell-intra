...
**September 22 approval follow-up (live UAT `9989a0d`):** Final approval follows the approved DOA. The requesting department head can approve when that policy names them for the request's department, category and amount; they do not need Procurement Admin access. They still need current approval permission and training, and cannot approve their own request. Check the request's actual department, which may differ from the event owner. Ask Legal or the policy administrator to review a policy mismatch; do not grant broader access or repeat completed training to force approval. Keep the earlier Procurement Lead replacement draft inactive. No roles, active policy, approval history or SMTP settings were changed. See the September 22 release note for test evidence and the deployment boundary.

If the request, assigned approver or approval step changes during review, the confirmation closes and clears the note and signature. Reopen it and review the current details before signing. Use **Retry requests** if refresh fails. A policy-update message means nothing was saved and you can try again shortly. If a save outcome is uncertain, check the recorded decision before trying again. Normal profile-title or last-active updates leave the confirmation open.

On small screens, the app wraps approval emails and attachment details and places Download below the file information when needed. Read the recorded decision and signer details together; a missing preview is not an invitation to approve again. Report the request reference instead of replacing historical evidence. Administration tables retain keyboard scrolling even when only their header is present.

Vendor submission: after completing required facts and declarations, capture a fresh authorized signature and select **Sign and submit**. Readiness now considers that captured signature's date; it does not invent one or save it before submission. **Saved securely** means the draft is saved, not that Legal received it. Confirm submitted status and the separate Legal handoff. CI208 passed this maintained application-to-Legal workflow on desktop and mobile. Invitation email delivery remains excluded.
