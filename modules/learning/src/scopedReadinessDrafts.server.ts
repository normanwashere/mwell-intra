...
// Review candidates only. Never imported by the active catalog or choice authority.
// IDs are new; existing published definitions and assignment versions stay unchanged.
export const SCOPED_READINESS_DRAFTS: readonly SimulationDefinition[] = [
  {
    id: "warehouse-quality-inspection-review-draft-v1", version: 1,
    ...
    embeddedSteps: [
      ...
      { checkpointId: "confirm-handoff", outcomeId: "confirm-handoff", title: "Confirm the result and retain custody",
        instruction: "A failed inspection response is not a recorded decision. Recheck the same source and retain its custody restrictions; release or vendor return follows its own authorized workflow.",
        context: "Submission failed. A colleague proposes moving the held stock to available inventory or dispatching a vendor return immediately.", question: "What should happen?",
        choices: [{ id: "release-anyway", label: "Release the units because the inspection was attempted" }, { id: "dispatch-return", label: "Dispatch a return without its separate authority" }, { id: "verify-route", label: "Verify the same source, resolve the failure and hand off disposition through authorized custody controls" }] },
    ],
  },
  {
    id: "procurement-payment-readiness-review-draft-v1", version: 1,
...
// Proposed review keys, not installed evaluator authority or completion evidence.
export const SCOPED_READINESS_DRAFT_KEYS: Readonly<Record<string, string>> = {
  "confirm-source": "exact-source", "hold-with-evidence": "complete-hold", "confirm-handoff": "verify-route",
  "separate-decisions": "return-owner", "return-stale-pack": "return-current", "review-not-release": "readback-handoff",
};
