...
const RECEIVING_QUESTIONS: readonly AssessmentQuestion[] = [
  {
    ...
    options: [
      { id: "capture-identifiers", label: "Delivery date, batch, and every unit serial" },
      { id: "quantity-only", label: "Only the total delivered quantity" },
      { id: "supplier-only", label: "Only the supplier and courier" },
    ],
    explanation: "Delivery, batch, and unit identity preserve end-to-end traceability.",
  },
  {
    id: "receiving-exception",
    prompt: "What should happen when received stock is damaged or unidentified?",
    options: [
      { id: "available-stock", label: "Place it in available stock" },
      { id: "controlled-quality", label: "Keep it in controlled quality custody" },
      { id: "discard-immediately", label: "Discard it without a record" },
    ],
    explanation: "Exceptions remain in controlled custody until an authorized disposition is recorded.",
  },
];
...
const RECEIVING_POLICY: ControlledPolicyDocument = {
  ...
  summary: "These controls keep every inbound item attributable, traceable, and unavailable until custody is valid.",
  sections: [
  ...
  evidenceHash: "9b13c375513649ddab0af15ce7188a22fcbcefe7d861a7002e759cefb88e0cc0",
...
/** Demo-only scoring. Live answer authority remains in the learning service. */
export function scorePreviewAssessment(input: AssessmentSubmission): number {
  const expected = new Map([
    ["receiving-identifiers", "capture-identifiers"],
    ["receiving-exception", "controlled-quality"],
  ]);
  const correct = input.answers.filter(
    (answer) => expected.get(answer.questionId) === answer.answerId,
  ).length;
  return expected.size === 0 ? 0 : Math.round((correct / expected.size) * 100);
}
