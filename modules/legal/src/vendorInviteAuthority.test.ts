...
describe('vendor invitation authority client', () => {
  it('reads only a complete positive pending invitation generation', () => {
    ...
    expect(readPendingVendorInvitation({ pending_vendor_invite: 'inv-1' })).toBeNull();
  });

  it('requires link-bound evidence that matches the pending generation', () => {
    const pending = { inviteId: 'inv-1', linkGeneration: 3 };
    const token = 'a'.repeat(64);
    expect(readVendorInvitationEvidence(`?invite_id=inv-1&generation=3&acceptance_token=${token}`, pending)).toEqual({
      ...pending,
      acceptanceToken: token,
    });
    expect(readVendorInvitationEvidence(`?invite_id=inv-1&generation=2&acceptance_token=${token}`, pending)).toBeNull();
    expect(readVendorInvitationEvidence('?invite_id=inv-1&generation=3', pending)).toBeNull();
  });
