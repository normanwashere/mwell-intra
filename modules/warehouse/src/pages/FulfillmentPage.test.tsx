...
describe("FulfillmentPage", () => {
  ...
  ] as const)('gates contextual physical-return links by intake authority for $role (allowed=$allowed)', async ({ role, allowed }) => {
  ...
  it('shows original-order physical return action and linked history without requiring completion', async () => {
    ...
    expect(within(dialog).getByRole('link', { name: 'physical-return' })).toHaveAttribute('href', '/returns#return-physical-return');
    expect(within(dialog).getByRole('link', { name: 'physical-return' })).toHaveClass('block', 'min-h-11', 'min-w-11', 'max-w-full');
  });
