    ...
    <div className="space-y-4">
      ...
      {event && (
        <section className="space-y-3 border-t border-line pt-4">
          ...
          {canManage && event.status === 'draft' && <div className="grid gap-2 sm:grid-cols-[1fr_auto]"><select className="input" value={vendorId} onChange={(e) => setVendorId(e.target.value)}><option value="">Select vendor to invite</option>{availableVendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.legalName}</option>)}</select><button type="button" className="btn-outline w-full sm:w-auto" disabled={busy || !vendorId} onClick={() => void inviteVendor()}><Icon name="plus" className="h-4 w-4" />Record invitation</button></div>}
          ...
          {canManage && event.status === 'issued' && <div className="grid gap-3 rounded-lg bg-inset p-3 sm:grid-cols-2">
            <label className="block text-sm font-semibold text-ink">Vendor<select className="input mt-1.5" value={vendorId} onChange={(e) => setVendorId(e.target.value)}><option value="">Select invited vendor</option>{event.responses.filter((response) => !response.receivedAt).map((response) => <option key={response.vendorId} value={response.vendorId}>{response.vendorName}</option>)}</select></label>
          ...
          {canManage && event.status === 'issued' && received.length > 0 && <div className="grid gap-3 rounded-lg border border-line p-3 sm:grid-cols-2">
            <label className="block text-sm font-semibold text-ink">Recommended vendor<select className="input mt-1.5" value={selectedVendorId} onChange={(e) => setSelectedVendorId(e.target.value)}><option value="">Select received response</option>{received.map((response) => <option key={response.vendorId} value={response.vendorId}>{response.vendorName}</option>)}</select></label>
