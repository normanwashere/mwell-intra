...
import type { AwardRecommendation, CommercialTabulation, FailedBidReason, ProcurementPolicyControls, ProcurementVendor, RecommendationVarianceDecision, SourcingEventStatus, SourcingMethod, TechnicalEvaluation, VarianceReviewEligibility } from '../types';
...
export function SourcingWorkspace({ requestId, method, canManage, canApprove, client, vendors, onChanged }: {
  ...
  const controlSources = event?.policyControlSources ?? MWELL_OPERATING_PROFILE.controlSources;
  ...
  const inviteVendor = () => event && vendorId ? (event.status === 'failed_bid'
    ? run(() => call('transition_sourcing_event', { id: event.id, action: 'source_additional_and_requote', vendor_id: vendorId, submission_deadline: deadline ? new Date(deadline).toISOString() : null, package_version: packageVersion.trim(), package_hash: packageHash.trim() }).then(() => undefined), 'Additional vendor sourced and equal requote issued')
    : run(() => call('invite_sourcing_vendors', { sourcing_event_id: event.id, vendor_ids: [vendorId] }).then(() => undefined), 'Controlled vendor invitation recorded')
  ).finally(() => setVendorId('')) : undefined;
  ...
  return <section className="space-y-4" aria-label="Governed competitive sourcing">
    ...
    {event && <>
      ...
      {canManage && canInvite && <section className="space-y-3 rounded-lg border border-line p-3"><p className="text-sm text-muted">{event.status === 'failed_bid' ? 'Source one additional accredited vendor, version the package, and send the same requote notice to every invitee.' : 'Invite accredited vendors to the controlled package.'}</p>{event.status === 'failed_bid' && <div className="grid gap-3 md:grid-cols-3"><label className="block text-sm font-semibold text-ink">Requote deadline<input aria-label="Requote deadline" type="datetime-local" className="input mt-1.5" value={deadline} onChange={(e) => setDeadline(e.target.value)} /></label><label className="block text-sm font-semibold text-ink">Requote package version<input aria-label="Requote package version" className="input mt-1.5" value={packageVersion} onChange={(e) => setPackageVersion(e.target.value)} /></label><label className="block text-sm font-semibold text-ink">Requote package SHA-256<input aria-label="Requote package SHA-256" className="input mt-1.5" value={packageHash} onChange={(e) => setPackageHash(e.target.value)} /></label></div>}<div className="grid gap-2 sm:grid-cols-[1fr_auto]"><select aria-label="Vendor to invite" className="input" value={vendorId} onChange={(e) => setVendorId(e.target.value)}><option value="">Select accredited vendor to invite</option>{availableVendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.legalName}</option>)}</select><button type="button" className="btn-outline min-h-11 w-full sm:w-auto" disabled={busy || !vendorId || (event.status === 'failed_bid' && (!deadline || !packageVersion.trim() || !packageHash.trim()))} onClick={() => void inviteVendor()}><Icon name="plus" className="h-4 w-4" />{event.status === 'failed_bid' ? 'Source and requote' : 'Record invitation'}</button></div></section>}
      ...
      {canManage && (event.status === 'issued' || event.status === 'failed_bid') && <section className="grid gap-3 rounded-lg border border-line p-3 md:grid-cols-[1fr_auto]"><label className="block text-sm font-semibold text-ink">Extend deadline (calendar days)<input aria-label="Extension calendar days" type="number" min={1} max={controls.maxExtensionCalendarDays} className="input mt-1.5" value={extensionDays} onChange={(e) => setExtensionDays(Math.min(controls.maxExtensionCalendarDays, Math.max(1, Number(e.target.value) || 1)))} /></label><button type="button" className="btn-outline min-h-11 w-full md:w-auto" disabled={busy || extensionDays > controls.maxExtensionCalendarDays} onClick={() => void communicate('extension')}><Icon name="arrowRight" className="h-4 w-4" />Extend and notify all</button><p className="text-xs text-muted md:col-span-2">An extension is limited to {controls.maxExtensionCalendarDays} calendar days and sends the same notice to every invitee. Source: {controlSources.maxExtensionCalendarDays ?? 'request-bound policy profile'}.</p></section>}
