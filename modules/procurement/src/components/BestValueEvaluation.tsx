  ...
  return (
    <section className="space-y-4 border-t border-line pt-4" aria-label="Best-value evaluation">
      ...
      <section className="space-y-3 rounded-lg border border-line p-3" aria-label="Commercial tabulation">
        ...
        <div className="overflow-x-auto rounded border border-line"><table className="min-w-[42rem] w-full text-left text-sm"><thead className="bg-inset text-xs text-muted"><tr><th className="px-3 py-2">Vendor</th><th className="px-3 py-2">Quote</th><th className="px-3 py-2">Evidence</th></tr></thead><tbody className="divide-y divide-line">{received.map((response) => <tr key={response.vendorId}><td className="px-3 py-2 font-medium text-ink">{response.vendorName}</td><td className="px-3 py-2 text-ink">{response.commercial?.amount ? money(response.commercial.amount) : 'Not recorded'}</td><td className="px-3 py-2 text-muted">{tabulation?.evidenceReference ?? 'Pending tabulation evidence'}</td></tr>)}</tbody></table></div>
        {tabulation && <p className="text-xs text-muted">Submitted by {tabulation.submittedByName ?? tabulation.submittedByEmail ?? 'governed reviewer'} on {tabulation.submittedAt ? new Date(tabulation.submittedAt).toLocaleString() : 'pending'}{tabulation.comments ? ` · ${tabulation.comments}` : ''}</p>}
      ...
      <section className="space-y-3 rounded-lg border border-line p-3" aria-label="Technical evaluation">
        ...
        {technicalEvaluations.length > 0 && <div className="overflow-x-auto rounded border border-line"><table className="min-w-[50rem] w-full text-left text-sm"><thead className="bg-inset text-xs text-muted"><tr><th className="px-3 py-2">Vendor</th><th className="px-3 py-2">Score</th><th className="px-3 py-2">Reviewer</th><th className="px-3 py-2">Evidence</th><th className="px-3 py-2">Due / submitted</th><th className="px-3 py-2">SLA</th></tr></thead><tbody className="divide-y divide-line">{technicalEvaluations.map((item) => <tr key={item.id}><td className="px-3 py-2 text-ink">{received.find((response) => response.vendorId === item.vendorId)?.vendorName ?? item.vendorId}</td><td className="px-3 py-2 font-semibold text-ink">{item.totalScore.toFixed(1)}</td><td className="px-3 py-2 text-muted">{item.reviewerName ?? item.reviewerEmail ?? 'Governed reviewer'}</td><td className="px-3 py-2 text-muted">{item.evidenceReference}{item.comments ? ` · ${item.comments}` : ''}</td><td className="px-3 py-2 text-muted">{new Date(item.dueAt).toLocaleDateString()} / {item.submittedAt ? new Date(item.submittedAt).toLocaleDateString() : 'Pending'}</td><td className="px-3 py-2"><Badge tone={item.escalationStatus === 'on_track' ? 'emerald' : 'amber'}>{item.escalationStatus.replaceAll('_', ' ')}</Badge></td></tr>)}</tbody></table></div>}
      ...
      <section className="space-y-3 rounded-lg border border-line p-3" aria-label="Award recommendation">
        ...
        {recommendation && <div className="rounded bg-inset p-3 text-sm text-ink"><p><strong>Recommended vendor:</strong> {received.find((response) => response.vendorId === recommendation.recommendedVendorId)?.vendorName ?? recommendation.recommendedVendorId}</p><p className="mt-1 text-muted">{recommendation.rationale}</p>{pendingVariance && <p className="mt-2 text-xs text-muted">Next variance stage: {varianceStageLabel}. {varianceEligibility?.doaMatrixVersion ? `DOA matrix ${varianceEligibility.doaMatrixVersion}` : 'The server will confirm active DOA authority.'}</p>}</div>}
        {pendingVariance && (event.varianceDecisions?.length ?? 0) > 0 && <ul className="space-y-1 text-xs text-muted">{event.varianceDecisions?.map((decision) => <li key={decision.id}>{decision.decisionType.replaceAll('_', ' ')}: {decision.decision} by {decision.decidedByName ?? decision.decidedByEmail ?? 'governed approver'} · DOA assignment {decision.doaAssignmentId ?? 'recorded'}{decision.doaMatrixVersion ? ` (${decision.doaMatrixVersion})` : ''}</li>)}</ul>}
        {pendingVariance && varianceEligibility?.canReview && <div className="grid gap-3 border-t border-line pt-3 md:grid-cols-[1fr_auto_auto]"><p className="text-sm font-semibold text-ink md:col-span-3">Variance review: {varianceStageLabel}</p><label className="block text-sm font-semibold text-ink md:col-span-3">Independent decision note<textarea aria-label="Variance approval note" className="input mt-1.5" rows={2} value={varianceNote} onChange={(event) => setVarianceNote(event.target.value)} /></label><button type="button" className="btn-outline min-h-11" disabled={busy || !varianceNote.trim()} onClick={() => void run(() => call('review_recommendation_variance', { award_recommendation_id: recommendation.id, expected_version: recommendation.version, decision: 'rejected', note: varianceNote.trim() }), 'Variance recommendation rejected')}><Icon name="x" className="h-4 w-4" />Reject variance</button><button type="button" className="btn-primary min-h-11" disabled={busy || !varianceNote.trim()} onClick={() => void run(() => call('review_recommendation_variance', { award_recommendation_id: recommendation.id, expected_version: recommendation.version, decision: 'approved', note: varianceNote.trim() }), `Variance ${varianceStageLabel} decision recorded`)}><Icon name="check" className="h-4 w-4" />Record {varianceStageLabel} approval</button></div>}
      </section>
    </section>
  );
}
