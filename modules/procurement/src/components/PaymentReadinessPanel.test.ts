...
it('enables Finance preparation when the preview binds every active acceptance and aggregate quantity', () => {
  ...
  const pack: PaymentReadinessPack = {
    ...
    invoiceNumber: 'INV-1', invoiceDate: '2026-07-15', invoiceAmount: 10_000,
  ...
  const html = renderToStaticMarkup(createElement(PaymentReadinessPanel, {
    ...
    purchaseOrderAmount: 10_000, acceptanceType: 'goods',
    onAccept: vi.fn(), onPrepare: vi.fn(), onReview: vi.fn(), onRelease: vi.fn(),
  }));
  ...
  expect(html).not.toMatch(/<button[^>]*disabled[^>]*>[^<]*(?:<[^>]+>)*Validate match and send to Finance/);
});
...
it('shows amount-based service acceptance without requiring Warehouse lines', () => {
  const html = renderToStaticMarkup(createElement(PaymentReadinessPanel, {
    acceptances: [], canAccept: true, canPrepare: false, canReview: false,
    purchaseOrderAmount: 75_000, acceptanceType: 'service',
    onAccept: vi.fn(), onPrepare: vi.fn(), onReview: vi.fn(), onRelease: vi.fn(),
  }));
...
it('shows a governed payment release action only after Finance acceptance', () => {
  const pack: PaymentReadinessPack = {
    id: 'pack-release', purchaseOrderId: 'po-1', acceptancePackId: 'accept-1',
    poMatch: true, invoiceOrSiReference: 'invoice.pdf', milestoneSupportReference: 'delivery.pdf',
    taxWithholdingSupportReference: 'tax.pdf', status: 'accepted', preparedAt: '2026-07-15T12:00:00Z',
    invoiceNumber: 'INV-1002', invoiceAmount: 50_000, releasedAmount: 10_000,
  };
  const html = renderToStaticMarkup(createElement(PaymentReadinessPanel, {
    acceptances: [], pack, canAccept: false, canPrepare: false, canReview: true,
    purchaseOrderAmount: 50_000, acceptanceType: 'goods',
    onAccept: vi.fn(), onPrepare: vi.fn(), onReview: vi.fn(), onRelease: vi.fn(),
  }));
