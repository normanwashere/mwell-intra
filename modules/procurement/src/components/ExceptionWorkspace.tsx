...
export function ExceptionWorkspace({
  ...
  const needsEvidence = !pack || pack.status === 'rejected' || pack.status === 'superseded';
  return <section className="space-y-4" aria-label="Governed exception workspace">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <Badge tone={statusTone(pack?.status ?? 'under_review')}>{pack?.status?.replaceAll('_', ' ') ?? 'Evidence needed'}</Badge>
    </div>
    ...
    {needsEvidence && activeActions.canSubmit && <section className="space-y-3 rounded-lg border border-line p-3">
      <div><h3 className="font-semibold text-ink">Submit evidence for review</h3><p className="text-sm text-muted">Submitting replaces an outdated or rejected pack. The server derives and locks any policy facts it can verify.</p></div>
