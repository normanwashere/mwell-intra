...
export function PurchaseOrdersPage() {
  ...
  const requestAmendment = async () => {
    ...
    const { error: rpcError } = await supabaseClient
      ...
      .rpc('request_po_line_quantity_amendment', {
        payload: {
          purchase_order_id: selected.po.id,
  ...
  const decideAmendment = async (item: AmendmentWorkItem, decision: 'approved' | 'rejected') => {
    ...
    if (!decisionReason) {
      error('Enter the approver decision rationale first');
      return;
    }
    ...
    const { error: rpcError } = await supabaseClient
      .schema('procurement')
      .rpc('approve_po_line_quantity_amendment', {
        payload: {
    ...
    setAmendmentDecisionReasons((current) => ({
      ...current,
      [item.amendment_id]: '',
    }));
    if (!(await refreshAmendmentQueue())) return;
  ...
  const visibleRows = useMemo(() => {
    switch (filter) {
      case 'authoring':
        return rows.filter((r) => r.status === 'draft' || r.status === 'pending_approval');
      case 'active':
        return rows.filter((r) => r.status === 'approved' || r.status === 'issued');
      case 'closed':
  ...
  if (!canViewPurchaseOrders && amendmentQueueLoading) {
    return (
      <p className="p-4 text-sm text-muted" aria-live="polite">
        Loading assigned amendment work...
      </p>
    );
  }
        ...
        <section className="space-y-3" aria-label="PO quantity amendment queue">
            ...
            <div className="grid gap-3 border-y border-line py-4 md:grid-cols-2">
              <label className="text-sm font-semibold text-ink">
                ...
                >
                  ...
                  {rows
                    .filter((po) => po.status === 'approved' || po.status === 'issued')
                    .flatMap((po) =>
                      po.lines.map((line) => (
                        <option key={line.id} value={line.id}>
                          {po.poNumber} · {line.description} · {line.quantity} ordered
                        </option>
                      )),
                    )}
                </select>
          ...
          {amendmentItems.length === 0 ? (
            <p className="text-sm text-muted">
              No requester-owned or assigned amendment work is active.
            </p>
          ) : (
                ...
                >
                  <div className="min-w-0 space-y-1">
                    <p className="font-semibold text-ink">
                      {item.po_number} / {item.line_description}
                    </p>
                    <p className="text-sm text-muted">
                      {item.previous_quantity} to {item.amended_quantity} units / {item.status}
                      {item.next_tier ? ` / next: ${item.next_tier}` : ''}
                    </p>
                    <p className="text-sm text-ink">
                      <span className="font-semibold">Request reason:</span> {item.reason}
                    </p>
                  ...
                  {item.can_decide && (
                    <div className="grid min-w-0 gap-2 sm:min-w-80">
                      <label className="text-sm font-semibold text-ink">
                        Decision rationale
                        <textarea
                          className="input mt-1.5 min-h-20 resize-y"
                          value={amendmentDecisionReasons[item.amendment_id] ?? ''}
                          onChange={(event) =>
                            setAmendmentDecisionReasons((current) => ({
                              ...current,
                              [item.amendment_id]: event.target.value,
                            }))
                          }
                        />
                      </label>
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          disabled={!amendmentDecisionReasons[item.amendment_id]?.trim()}
                          onClick={() => void decideAmendment(item, 'rejected')}
                        >
                          Reject
                        </button>
                        <button
                          ...
                          disabled={!amendmentDecisionReasons[item.amendment_id]?.trim()}
                          onClick={() => void decideAmendment(item, 'approved')}
                        >
